...
object KinesisReceiverConfig {
  ...
  ): KinesisReceiverConfig = KinesisReceiverConfig(
...
class ThrallConfig(resources: GridConfigResources)
  ...
  val rewindFrom: Option[DateTime] = stringOpt(
    "thrall.kinesis.stream.rewindFrom"
  ).map(ISODateTimeFormat.dateTime.parseDateTime)
  val lowPriorityRewindFrom: Option[DateTime] = stringOpt(
    "thrall.kinesis.lowPriorityStream.rewindFrom"
  ).map(ISODateTimeFormat.dateTime.parseDateTime)

  val isVersionedS3: Boolean = boolean("s3.image.versioned")

  def kinesisConfig: KinesisReceiverConfig =
    KinesisReceiverConfig(thrallKinesisStream, rewindFrom, this)
  def kinesisLowPriorityConfig: KinesisReceiverConfig = KinesisReceiverConfig(
    thrallKinesisLowPriorityStream,
    lowPriorityRewindFrom,
    this
  )
}
