...
object RetryHandler {
  ...
  def handleWithRetryAndTimeout[T](
      f: WithMarkers[T],
      retries: Int,
    ...
    def handleWithTimeout[T](
        f: WithMarkers[T],
        attemptTimeout: FiniteDuration
    ): WithMarkers[T] = (marker) => {
      val timeout = after(attemptTimeout, using = actorSystem.scheduler)(
        Future.failed(
    ...
    def handleWithRetry[T](
      ...
      def attempt = () => {
        count = count + 1
        val markerWithRetry =
          combineMarkers(marker, MarkerMap("retryCount" -> count))
