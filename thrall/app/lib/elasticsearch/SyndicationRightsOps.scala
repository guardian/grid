...
class SyndicationRightsOps(es: ElasticSearch)(implicit ex: ExecutionContext)
  ...
  private def newRightsUpsert(
    ...
    image.syndicationRights match {
      case Some(_) =>
        for {
          _ <- Future.sequence(
            es.updateImageSyndicationRights(
              image.id,
              image.syndicationRights,
              lastModified
            )
          )
          _ <- refreshCurrentPhotoshoot(
            image,
  ...
  private def refreshPreviousPhotoshoot(
      image: Image,
    ...
    previousPhotoshootOpt match {
      case Some(photoshoot) =>
        refreshPhotoshoot(image, photoshoot, Some(image.id), lastModified)
      case None => Future.successful(())
    }

  private def refreshCurrentPhotoshoot(
      image: Image,
      currentPhotoshootOpt: Option[Photoshoot],
      lastModified: DateTime
  )(implicit
    ...
    currentPhotoshootOpt match {
      case Some(photoshoot) =>
        refreshPhotoshoot(image, photoshoot, None, lastModified)
      case None =>
        if (image.hasInferredSyndicationRightsOrNoRights)
          Future
            .sequence(es.deleteSyndicationRights(image.id, lastModified))
            .map(_ => ())
        else
          Future.successful(())
    }

  private def refreshPhotoshoot(
      image: Image,
    ...
    for {
      latestRights <- getLatestSyndicationRights(
        image,
        photoshoot,
        excludedImageId
      )
      inferredImages <- getInferredSyndicationRightsImages(
        image,
        photoshoot,
        excludedImageId
      )
  ...
  private def updateRights(
    ...
    latestRights match {
      case updatedRights @ Some(rights)
          if updateRequired(image, inferredImages) =>
        logger.info(
          s"Using rights ${Json.toJson(rights)} to infer syndication rights for ${inferredImages.length} image id(s) in photoshoot $photoshoot: ${inferredImages
            .map(_.id)}"
        )
        inferredImages.foreach(img =>
          es.updateImageSyndicationRights(
            img.id,
            updatedRights.map(_.copy(isInferred = true)),
            lastModified
          )
        )
      case None if image.hasNonInferredRights =>
        logger.info(
          s"Removing rights from images (photoshoot $photoshoot): ${inferredImages
            .map(_.id)} (total = ${inferredImages.length})."
        )
        inferredImages.foreach(img =>
          es.updateImageSyndicationRights(img.id, None, lastModified)
        )
      case _ =>
   ...
   */
  private def updateRequired(
      image: Image,
      inferredImages: List[Image]
  ): Boolean = image.hasNonInferredRights || inferredImages.exists(
    _.syndicationRights.isDefined
  )
   ...
   */
  private def getLatestSyndicationRights(
      image: Image,
      photoshoot: Photoshoot,
      excludedImageId: Option[String] = None
  )(implicit
    ...
    excludedImageId match {
      case Some(_) =>
        es.getLatestSyndicationRights(photoshoot, excludedImageId)
          .map(_.flatMap(_.syndicationRights))
      case None =>
        val hasInferredRights: Boolean =
          image.hasInferredSyndicationRightsOrNoRights
        es.getLatestSyndicationRights(photoshoot, None).map {
          case Some(dbImage) =>
            if (!hasInferredRights) mostRecentSyndicationRights(dbImage, image)
            else dbImage.syndicationRights
          case None =>
            if (!hasInferredRights) image.syndicationRights else None
        }
    }

  def mostRecentSyndicationRights(
      image1: Image,
      image2: Image
  ): Option[SyndicationRights] =
    (image1.rcsPublishDate, image2.rcsPublishDate) match {
      case (Some(date1), Some(date2)) =>
        if (date1.isAfter(date2)) image1.syndicationRights
        else image2.syndicationRights
      case (Some(_), None) => image1.syndicationRights
      case (None, Some(_)) => image2.syndicationRights
      case (None, None)    => None
    }

  private def getInferredSyndicationRightsImages(
      image: Image,
      photoshoot: Photoshoot,
      excludedImageId: Option[String] = None
  )(implicit
    ...
    excludedImageId match {
      case Some(_) =>
        es.getInferredSyndicationRightsImages(photoshoot, excludedImageId)
      case None =>
        val imageId =
          if (!image.hasInferredSyndicationRightsOrNoRights) Some(image.id)
          else None
        es.getInferredSyndicationRightsImages(photoshoot, imageId).map {
          images =>
            if (image.hasInferredSyndicationRightsOrNoRights)
              images :+ image
            else
