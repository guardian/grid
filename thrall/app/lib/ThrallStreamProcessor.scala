...
case class TaggedRecord(record: KinesisRecord, priority: Priority)
    extends LogMarker {
  override def markerContents = Map(
    "recordPriority" -> priority.toString,
    "recordArrivalTime" -> DateTimeUtils.toString(
...
class ThrallStreamProcessor(
  ...
  val mergedKinesisSource: Source[TaggedRecord, NotUsed] =
    Source.fromGraph(GraphDSL.create() { implicit g =>
      import GraphDSL.Implicits._
      val highPriorityKinesisSource =
        highPrioritySource.map(TaggedRecord(_, HighPriority))
      val lowPriorityKinesisSource =
        lowPrioritySource.map(TaggedRecord(_, LowPriority))
  ...
  def createStream()
      : Source[(TaggedRecord, Stopwatch, Option[UpdateMessage]), NotUsed] = {
    mergedKinesisSource
      .map { taggedRecord =>
        taggedRecord -> ThrallEventConsumer.parseRecord(
          taggedRecord.record.data.toArray,
          taggedRecord.record.approximateArrivalTimestamp
        )
      }
      .mapAsync(1) { result =>
  ...
  def run(): Future[Done] = {
    val stream = this.createStream().runForeach {
      case (taggedRecord, stopwatch, maybeUpdateMessage) =>
        ...
        val markers = maybeUpdateMessage
          .map(combineMarkers(basicMakers, _))
          .getOrElse(basicMakers)
    ...
    stream.onComplete {
      case Failure(exception) =>
        logger.error("stream completed with failure", exception)
      case Success(_) =>
        logger.info("Stream completed with done, probably shutting down")
    }
