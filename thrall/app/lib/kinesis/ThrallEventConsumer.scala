...
class ThrallEventConsumer(
  ...
  private val messageProcessor = new MessageProcessor(
    es,
    store,
  ...
  def processUpdateMessage(
    ...
    RetryHandler
      ...
      .transform {
        ...
        case Failure(
              processorNotFoundException: ProcessorNotFoundException
            ) => {
          logger.error(
        ...
        case Failure(e: Throwable) => {
          logger.error(
            combineMarkers(marker, stopwatch.elapsed),
            s"Failed to process ${updateMessage.subject} message; message will be ignored:",
            e
          )
          Failure(e)
        }
    ...
    Try(JsonByteArrayUtil.fromByteArray[UpdateMessage](r)) match {
      case Success(Some(updateMessage: UpdateMessage)) => {
        logger.info(
          ...
          s"Received ${updateMessage.subject} message at $timestamp"
        )
        Some(updateMessage)
      }
      case Success(None) => {
        logger.warn(
          s"No message present in record at $timestamp",
          new String(r)
        )
