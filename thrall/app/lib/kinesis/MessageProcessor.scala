...
class MessageProcessor(
  ...
  def process(updateMessage: UpdateMessage, logMarker: LogMarker)(implicit
      ec: ExecutionContext
  ): Future[Any] = {
    updateMessage.subject match {
      ...
      case "delete-image-exports" =>
        deleteImageExports(updateMessage, logMarker)
      case "update-image-exports" =>
        updateImageExports(updateMessage, logMarker)
      case "update-image-user-metadata" =>
        updateImageUserMetadata(updateMessage, logMarker)
      case "update-image-usages" => updateImageUsages(updateMessage, logMarker)
      case "replace-image-leases" =>
        replaceImageLeases(updateMessage, logMarker)
      ...
      case "set-image-collections" =>
        setImageCollections(updateMessage, logMarker)
      case "delete-usages" => deleteAllUsages(updateMessage, logMarker)
      case "upsert-rcs-rights" =>
        upsertSyndicationRights(updateMessage, logMarker)
      case "update-image-photoshoot" =>
        updateImagePhotoshoot(updateMessage, logMarker)
      case unknownSubject =>
        Future.failed(ProcessorNotFoundException(unknownSubject))
    }
  }

  def updateImageUsages(message: UpdateMessage, logMarker: LogMarker)(implicit
      ec: ExecutionContext
  ): Future[List[ElasticSearchUpdateResponse]] = {
    ...
    withId(message) { id =>
      withUsageNotice(message) { usageNotice =>
        ...
        Future.traverse(es.updateImageUsages(id, usages, message.lastModified))(
          _.recoverWith { case ElasticNotFoundException =>
  ...
  private def reindexImage(message: UpdateMessage, logMarker: LogMarker)(
      implicit ec: ExecutionContext
  ) = {
    logger.info(
      logMarker,
      s"Reindexing image: ${message.image.map(_.id).getOrElse("image not found")}"
    )
    indexImage(message, logMarker)
  }

  private def indexImage(message: UpdateMessage, logMarker: LogMarker)(implicit
      ec: ExecutionContext
  ) =
    withImage(message)(i =>
      Future.sequence(
  ...
  private def updateImageExports(message: UpdateMessage, logMarker: LogMarker)(
      implicit ec: ExecutionContext
  ) =
    withId(message)(id =>
  ...
  private def deleteImageExports(message: UpdateMessage, logMarker: LogMarker)(
      implicit ec: ExecutionContext
  ) =
    withId(message)(id =>
      Future.sequence(
  ...
  private def updateImageUserMetadata(
    ...
    withEdits(message)(edits =>
      withId(message)(id =>
        Future.sequence(
          es.applyImageMetadataOverride(id, edits, message.lastModified)(
            ec,
            logMarker
          )
        )
  ...
  private def replaceImageLeases(message: UpdateMessage, logMarker: LogMarker)(
      implicit ec: ExecutionContext
  ) =
    withId(message)(id =>
      withLeases(message)(leases =>
        Future.sequence(
          es.replaceImageLeases(id, leases, message.lastModified)(ec, logMarker)
        )
      )
    )

  private def addImageLease(message: UpdateMessage, logMarker: LogMarker)(
      implicit ec: ExecutionContext
  ) =
    withId(message)(id =>
      withLease(message)(mediaLease =>
        Future.sequence(
          es.addImageLease(id, mediaLease, message.lastModified)(ec, logMarker)
        )
      )
    )

  private def removeImageLease(message: UpdateMessage, logMarker: LogMarker)(
      implicit ec: ExecutionContext
  ): Future[List[ElasticSearchUpdateResponse]] =
    withId(message)(id =>
      withLeaseId(message)(leaseId =>
        Future.sequence(
          es.removeImageLease(id, Some(leaseId), message.lastModified)(
            ec,
            logMarker
          )
        )
  ...
  private def setImageCollections(message: UpdateMessage, logMarker: LogMarker)(
      implicit ec: ExecutionContext
  ) =
    withId(message)(id =>
      withCollections(message)(collections =>
        Future.sequence(
          es.setImageCollections(id, collections, message.lastModified)(
            ec,
            logMarker
          )
        )
  ...
  private def deleteImage(updateMessage: UpdateMessage, logMarker: LogMarker)(
      implicit ec: ExecutionContext
  ) = {
    Future.sequence(
      withId(updateMessage) { id =>
        implicit val marker: LogMarker =
          logMarker ++ logger.imageIdMarker(ImageId(id))
        ...
        es.deleteImage(id).map { requests =>
          requests.map { _: ElasticSearchDeleteResponse =>
          ...
          } recoverWith { case ImageNotDeletable =>
  ...
  private def deleteAllUsages(message: UpdateMessage, logMarker: LogMarker)(
      implicit ec: ExecutionContext
  ) =
    withId(message)(id =>
      Future.sequence(
        es.deleteAllImageUsages(id, message.lastModified)(ec, logMarker)
      )
    )

  def upsertSyndicationRights(message: UpdateMessage, logMarker: LogMarker)(
      implicit ec: ExecutionContext
  ): Future[Any] =
    withId(message) { id =>
      implicit val marker: LogMarker =
      ...
      withSyndicationRights(message)(syndicationRights =>
        es.getImage(id) map {
          case Some(image) =>
            ...
            logger.info(
              marker,
              s"Upserting syndication rights for image $id in photoshoot $photoshoot with rights ${Json
                .toJson(syndicationRights)}"
            )
  ...
  def updateImagePhotoshoot(message: UpdateMessage, logMarker: LogMarker)(
      implicit ec: ExecutionContext
  ): Future[Unit] = {
    withId(message) { id =>
      implicit val marker: LogMarker =
      ...
      withEdits(message) { upcomingEdits =>
        for {
          ...
          prevPhotoshootOpt = imageOpt.flatMap(
            _.userMetadata.flatMap(_.photoshoot)
          )
          _ <- updateImageUserMetadata(message, logMarker)
          _ <- {
            logger.info(
              marker,
              s"Upserting syndication rights for image $id. Moving from photoshoot $prevPhotoshootOpt to ${upcomingEdits.photoshoot}."
            )
            syndicationRightsOps.upsertOrRefreshRights(
              image = imageOpt.get,
          ...
          marker,
          s"Moved image $id from $prevPhotoshootOpt to ${upcomingEdits.photoshoot}"
        )
      }
    }
  }
  ...
  private def withCollections[A](
      message: UpdateMessage
  )(f: Seq[Collection] => A): A = {
  ...
  private def withLeases[A](
      message: UpdateMessage
  )(f: Seq[MediaLease] => A): A = {
  ...
  private def withSyndicationRights[A](
      message: UpdateMessage
  )(f: SyndicationRights => A): A = {
    message.syndicationRights.map(f).getOrElse {
      sys.error(
        s"No syndication rights present on data field message: $message"
      )
    }
  }

  private def withUsageNotice[A](
      message: UpdateMessage
  )(f: UsageNotice => A): A = {
...
case class ProcessorNotFoundException(unknownSubject: String)
    extends Exception(s"Could not find processor for $unknownSubject message")
