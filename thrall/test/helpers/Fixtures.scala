...
trait Fixtures {
  ...
  def createImage(
    ...
    Image(
      ...
      userMetadata = optPhotoshoot.map(_ =>
        Edits(metadata = ImageMetadata(), photoshoot = optPhotoshoot)
      ),
      metadata = ImageMetadata(
        dateTaken = None,
        title = Some(s"Test image $id"),
        keywords = List("test", "es")
      ),
  ...
  def createImageForSyndication(
    ...
    createImage(
      id,
      StaffPhotographer("Tom Jenkins", "The Guardian"),
      Some(syndicationRights),
      leaseByMedia,
      usages,
  ...
  def someSyndRights = Some(
    SyndicationRights(
      ...
      rights = List(
        Right(rightCode = "code", acquired = Some(true), properties = Seq.empty)
      ),
      isInferred = false
    )
  )

  def imageWithNoSyndRights: Image = createImage(
    id = UUID.randomUUID().toString,
    usageRights = StaffPhotographer("Tom Jenkins", "The Guardian")
  )
  def imageWithSyndRights: Image = createImage(
    id = UUID.randomUUID().toString,
    usageRights = StaffPhotographer("Tom Jenkins", "The Guardian"),
    syndicationRights = someSyndRights
  )

  def imageWithPhotoshoot(photoshoot: Photoshoot): Image = createImage(
    id = UUID.randomUUID().toString,
    StaffPhotographer("Tom Jenkins", "The Guardian"),
    optPhotoshoot = Some(photoshoot)
  )
  ...
  def usage(id: String = UUID.randomUUID().toString) = Usage(
    id,
    List.empty,
    DigitalUsage,
    "test",
