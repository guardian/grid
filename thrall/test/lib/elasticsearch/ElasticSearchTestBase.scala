...
trait ElasticSearchTestBase
  ...
  val elasticSearchConfig =
    ElasticSearchConfig("writealias", esTestUrl, "media-service-test", 1, 0)
  ...
  override protected def afterEach(): Unit = {
    ...
    Await.ready(
      ES.client.execute(
        ElasticDsl
          .deleteByQuery(ES.initialImagesIndex, ElasticDsl.matchAllQuery())
      ),
      fiveSeconds
    )
  ...
  def asJsLookup(d: DateTime): JsLookupResult = JsDefined(
