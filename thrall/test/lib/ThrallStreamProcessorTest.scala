...
private class ThrallStreamProcessorTest
    extends FunSpec
  ...
  def createRecord: KinesisRecord = KinesisRecord(
    ByteString.empty,
    "",
    None,
    "",
  ...
  val highPrioritySource: Source[KinesisRecord, Future[Done.type]] = Source
    .repeat(createRecord)
    .mapMaterializedValue(_ => Future.successful(Done))
    .take(100)
  val lowPrioritySource: Source[KinesisRecord, Future[Done.type]] = Source
    .repeat(createRecord)
    .mapMaterializedValue(_ => Future.successful(Done))
    .take(100)
  ...
  describe("Stream merging strategy") {
    it("should process high priority events first") {
      ...
      val prioritiesFromMessages =
        Await.result(stream.take(200).runWith(Sink.seq), 5.minutes).map {
      ...
      val expected: Seq[Priority] = alternatingSegment ++ (1 to 98).map(_ =>
        HighPriority
      ) ++ alternatingSegment ++ (1 to 96).map(_ => LowPriority)
