...
class EditsController(
  ...
  def setArchived(id: String) = auth.async(parse.json) { implicit req =>
    (req.body \ "data")
      ...
      .fold(
        ...
        archived =>
          store
            .booleanSetOrRemove(id, "archived", archived)
            .map(publish(id))
  ...
  def getPhotoshoot(id: String) = auth.async {
    store
      ...
      respondNotFound("No photoshoot found")
    }
  }
  ...
  def setPhotoshoot(id: String) = auth.async(parse.json) { req =>
    {
      (req.body \ "data")
        ...
        .map(photoshoot => {
          store
            .jsonAdd(id, "photoshoot", caseClassToMap(photoshoot))
            .map(publish(id, "update-image-photoshoot"))
            .map(_ => respond(photoshoot))
        })
        .getOrElse(
          Future.successful(
            respondError(BadRequest, "invalid-form-data", "Invalid form data")
          )
        )
    }
  }
  ...
  def removeLabel(id: String, label: String) = auth.async {
    store
      .setDelete(id, "labels", decodeUriParam(label))
      .map(publish(id))
      .map(edits => labelsCollection(id, edits.labels.toSet))
  ...
  def setMetadata(id: String) = auth.async(parse.json) { req =>
    (req.body \ "data")
      ...
      .fold(
        ...
        metadata =>
          store
            .jsonAdd(id, "metadata", metadataAsMap(metadata))
            .map(publish(id))
  ...
  def setMetadataFromUsageRights(id: String) = auth.async { req =>
    store.get(id) flatMap { dynamoEntry =>
      ...
      metadataOpt map { metadata =>
        ...
        store
          .jsonAdd(id, "metadata", metadataAsMap(mergedMetadata))
          .map(publish(id))
      ...
      respondError(NotFound, "item-not-found", "Could not find image")
    }
  }
  ...
  def setUsageRights(id: String) = auth.async(parse.json) { req =>
    (req.body \ "data")
      ...
      .map(usageRight => {
        store
          .jsonAdd(id, "usageRights", caseClassToMap(usageRight))
          .map(publish(id))
  ...
  def caseClassToMap[T](caseClass: T)(implicit
      tjs: Writes[T]
  ): Map[String, JsValue] =
    Json.toJson[T](caseClass).as[JsObject].as[Map[String, JsValue]]

  def labelsCollection(
      id: String,
      labels: Set[String]
  ): (URI, Seq[EmbeddedEntity[String]]) =
    (labelsUri(id), labels.map(setUnitEntity(id, "labels", _)).toSeq)

  def publish(id: String, subject: String = "update-image-user-metadata")(
      metadata: JsObject
  ): Edits = {
    val edits = metadata.as[Edits]
    val updateMessage =
      UpdateMessage(subject = subject, id = Some(id), edits = Some(edits))
