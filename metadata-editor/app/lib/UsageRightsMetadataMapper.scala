      ...
      (ur: UsageRights) =>
        ur match {
          ...
          case u: ContractPhotographer =>
            ImageMetadata(byline = Some(u.photographer), credit = u.publication)
          case u: CommissionedPhotographer =>
            ImageMetadata(byline = Some(u.photographer), credit = u.publication)
          case u: ContractIllustrator =>
            ImageMetadata(byline = Some(u.creator), credit = u.publication)
          case u: StaffIllustrator =>
            ImageMetadata(byline = Some(u.creator), credit = Some(u.creator))
