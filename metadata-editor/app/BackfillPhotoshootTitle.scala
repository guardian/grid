...
import lib.{EditsConfig, EditsStore, Syndication, SyndicationStore}
...
object BackfillPhotoshootTitleApp extends App with Syndication with GridLogging {

  val editsTable = "media-service-PROD-EditsDynamoTable-1A65A93S9F15R"
//  val editsTable = "media-service-TEST-EditsDynamoTable-ZME1PCUPGQ3R"

  implicit private val system: ActorSystem = ActorSystem()
  def underlying = new UnderlyingPhotoshoot(editsTable)
  ...
  override def syndicationStore = ???
  override def config = new EditsConfig(gridConfigResources)
  override def editsStore = new EditsStore(editsConfig)
  override def notifications = ???

  // Fetch all dynamo entries from Edits table
  var record = 0
  val objectsWithPhotoshoot = editsStore
    .scanRaw()
    .foreach(stream =>
      stream
      .foreach(item => {
          ...
          Option(item.getMap("photoshoot")) match {
            case None => println(s"$record: $id, Nothing")
            case Some(photoshoot) => {
              val title = photoshoot.get("title").toString
              println(s"$record: $id, ${title}")
              setPhotoshootAndPublish(id, Photoshoot(title))
            }
          }
        }
      )
//          .map(dynamoEntry => ((page.item \ "id").toString, dynamoEntry.as[Edits]))
//          // Ignore any without photoshoot
//          .collect{
//            case (id, Edits(_, _, _, _, Some(photoshoot))) => (id, photoshoot)
//          }
//          .foreach(
//            edit => {
//
//              // Set Photoshoot and write back.
//              println(edit._1)
//              println(edit._2)
//  //            setPhotoshootAndPublish(edit._1, edit._2)
//            }
//          )
    )
  system.terminate()
...
class UnderlyingPhotoshoot(editsTable: String) extends Config {
  val params = List(
    "grid.stage" -> "LOCAL",
...
case class BackfillPhotoshootConfigValue(path: String, value: Option[String]) extends ConfigValue {
...
case class BackfillPhotoshootConfigOrigin(path: String) extends ConfigOrigin {
