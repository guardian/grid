...
class MetadataEditorComponents(context: Context)
    extends GridComponents(context, new EditsConfig(_)) {
  ...
  val editsController = new EditsController(
    auth,
    store,
    notifications,
    config,
  ...
  override val router =
    new Routes(httpErrorHandler, controller, editsController, management)
}
