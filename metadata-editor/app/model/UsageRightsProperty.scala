...
object UsageRightsProperty {
  ...
  import MetadataConfig.{
    contractPhotographersMap,
    staffPhotographersMap,
    contractIllustratorsMap,
    staffIllustrators,
  ...
  implicit val jsonWrites: Writes[UsageRightsProperty] =
    Json.writes[UsageRightsProperty]
  ...
  def getPropertiesForSpec(u: UsageRightsSpec): List[UsageRightsProperty] =
    props.flatMap(f => f(u))
  ...
  private def illustratorField(illustrators: OptionsMap, key: String) =
    requiredStringField(
      "creator",
      "Illustrator",
      optionsMap = Some(illustrators),
      optionsMapKey = Some(key)
    )

  private def restrictionProperties(
      u: UsageRightsSpec
  ): List[UsageRightsProperty] = u match {
    case NoRights => List()
    case _ =>
      List(
        UsageRightsProperty(
    ...
    case StaffIllustrator =>
      List(
        requiredStringField(
          "creator",
          "Illustrator",
          Some(sortList(staffIllustrators))
        )
      )

    case CommissionedIllustrator =>
      List(
        publicationField(false),
        requiredStringField(
          "creator",
          "Illustrator",
          examples = Some("Ellie Foreman Peck, Matt Bors")
        )
      )
    ...
    case CreativeCommons =>
      List(
        ...
        requiredStringField(
          "source",
          "Source",
          examples = Some("Wikimedia Commons")
        ),
        ...
        requiredStringField(
          ...
          "Link to content",
          examples = Some(
            "https://commons.wikimedia.org/wiki/File:Foreign_and_Commonwealth_Office_-_Durbar_Court.jpg"
          )
        )
      )
    ...
    case Composite =>
      List(
        requiredStringField(
          "suppliers",
          "Suppliers",
          examples = Some("REX/Getty Images/Corbis, Corbis/Reuters")
        )
      )
    ...
    case Screengrab =>
      List(
        requiredStringField(
          "source",
          "Source",
          examples = Some("BBC News, HBO, ITV")
        )
      )
