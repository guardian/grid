...
class ImageUploadTest extends AsyncFunSuite with Matchers with MockitoSugar {
  ...
  val mockConfig: ImageUploadOpsCfg = ImageUploadOpsCfg(
    tempDir,
    256,
    85d,
    List(Tiff),
  ...
  private def imageUpload(
      ...
      expectOptimisedFile: Boolean = false
  ): Future[Assertion] = {
    ...
    futureImage.map(i => {
      ...
      assert(
        i.source.mimeType.contains(expectedOriginalMimeType),
      ...
      assert(
        i.thumbnail.get.mimeType.get == Jpeg,
        "Should have correct thumb mime type"
      )

      // Assertions on optional generated optimised png image
      assert(
        i.optimisedPng.isDefined == expectOptimisedFile,
        "Should have optimised file"
      )
      assert(
        !expectOptimisedFile || i.optimisedPng
          .flatMap(p => p.mimeType)
          .contains(Png),
        "Should have correct optimised mime type"
      )
    })
  }
  ...
  ignore(
    "A layered tiff file (will require renaming extracted file) which requires optimising for UI"
  ) {
    imageUpload(
      "tiff_8bpc_layered_withTransparency.tif",
      Tiff,
      expectOptimisedFile = true
    )
  }
  ...
  ignore(
    "A png which is not suitable (too many colours + transparency) for UI viewing"
  ) {
    imageUpload(
      "bgan6a16_TrueColorWithAlpha_16bit.png",
      Png,
      expectOptimisedFile = true
    )
  }
  ...
  ignore("not an image but looks like one") {
    imageUpload("thisisnotanimage.jpg", Png, expectOptimisedFile = true)
      .transformWith {
        case Success(_) => fail("Should have thrown an error")
        case Failure(e) =>
          e match {
            case e: ImageProcessingException =>
              assert(e.getMessage == "File format could not be determined")
          }
      }
  }
