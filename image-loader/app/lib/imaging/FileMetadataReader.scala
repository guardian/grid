...
object FileMetadataReader {
  ...
  for ((prefix, namespaceUri) <- namespaces)
  ...
  def fromIPTCHeaders(image: File, imageId: String): Future[FileMetadata] =
    ...
    } yield getMetadataWithIPTCHeaders(
      metadata,
      imageId
  ...
  def fromIPTCHeadersWithColorInfo(
      image: File,
      imageId: String,
      mimeType: MimeType
  ): Future[FileMetadata] =
    for {
      metadata <- readMetadata(image)
      colourModelInformation <- getColorModelInformation(
        image,
        metadata,
        mimeType
      )
    } yield getMetadataWithIPTCHeaders(metadata, imageId)
      .copy(colourModelInformation = colourModelInformation)

  private def getMetadataWithIPTCHeaders(
      metadata: Metadata,
      imageId: String
  ): FileMetadata =
    FileMetadata(
  ...
  private def exportDirectory[T <: Directory](
      metadata: Metadata,
    ...
    Option(metadata.getFirstDirectoryOfType(directoryClass)) map { directory =>
      ...
      directory match {
        case d: IptcDirectory =>
          val dateTimeCreated =
            Option(d.getDateCreated)
              .map(d => dateToUTCString(new DateTime(d)))
              .map("Date Time Created Composite" -> _)

          val digitalDateTimeCreated =
            Option(d.getDigitalDateCreated)
              .map(d => dateToUTCString(new DateTime(d)))
              .map("Digital Date Time Created Composite" -> _)

          metaTagsMap ++ dateTimeCreated ++ digitalDateTimeCreated

        case d: ExifSubIFDDirectory =>
          val dateTimeCreated = Option(d.getDateOriginal)
            .map(d => dateToUTCString(new DateTime(d)))
            .map("Date/Time Original Composite" -> _)
  ...
  private def xmpDirectoryToMap(
      directory: XmpDirectory,
      imageId: String
  ): Map[String, String] = {
    directory.getXmpProperties.asScala.toMap
      .mapValues(nonEmptyTrimmed)
      .collect {
        case (datePattern(key), Some(value)) =>
          key -> ImageMetadataConverter.cleanDate(value, key, imageId)
        case (key, Some(value)) => key -> value
      }
  }
  private def exportRawXmpProperties(
      metadata: Metadata,
      imageId: String
  ): Map[String, String] = {
    val directories =
      metadata.getDirectoriesOfType(classOf[XmpDirectory]).asScala.toList
    val props: Map[String, String] =
      directories.foldLeft[Map[String, String]](Map.empty)((acc, dir) => {
  ...
  private def exportXmpPropertiesInTransformedSchema(
      metadata: Metadata,
      imageId: String
  ): Map[String, JsValue] = {
  ...
  private def exportGettyDirectory(
      metadata: Metadata,
      imageId: String
  ): Map[String, String] = {
    ...
    def readAssetId: Option[String] =
      readProperty("GettyImagesGIFT:AssetId").orElse(
        readProperty("GettyImagesGIFT:AssetID")
      )
    ...
    Map(
      ...
      "Original Create Date Time" -> readProperty(
        "GettyImagesGIFT:OriginalCreateDateTime"
      ),
  ...
  private def dateToUTCString(date: DateTime): String =
    ISODateTimeFormat.dateTime.print(date.withZone(DateTimeZone.UTC))

  def dimensions(
      image: File,
      mimeType: Option[MimeType]
  ): Future[Option[Dimensions]] =
    for {
      metadata <- readMetadata(image)
    } yield {
      ...
      mimeType match {
        ...
        case Some(Jpeg) =>
          for {
            jpegDir <- Option(
              metadata.getFirstDirectoryOfType(classOf[JpegDirectory])
            )
        ...
        case Some(Png) =>
          for {
            pngDir <- Option(
              metadata.getFirstDirectoryOfType(classOf[PngDirectory])
            )
        ...
        case Some(Tiff) =>
          for {
            exifDir <- Option(
              metadata.getFirstDirectoryOfType(classOf[ExifIFD0Directory])
            )
  ...
  def getColorModelInformation(
    ...
    runIdentifyCmd(formatter)
      .map { imageType =>
        getColourInformation(metadata, imageType.headOption, mimeType)
      }
      .recover { case _ => getColourInformation(metadata, None, mimeType) }
  }

  private def getColourInformation(
      metadata: Metadata,
      maybeImageType: Option[String],
      mimeType: MimeType
  ): Map[String, String] = {

    val hasAlpha = maybeImageType.map(imageType =>
      if (imageType.contains("Matte")) "true" else "false"
    )
    ...
    mimeType match {
      case Png =>
        ...
        Map(
          "hasAlpha" -> hasAlpha,
          "colorType" -> Option(
            metaDir.getDescription(PngDirectory.TAG_COLOR_TYPE)
          ),
          "bitsPerSample" -> Option(
            metaDir.getDescription(PngDirectory.TAG_BITS_PER_SAMPLE)
          ),
          "paletteHasTransparency" -> Option(
            metaDir.getDescription(PngDirectory.TAG_PALETTE_HAS_TRANSPARENCY)
          ),
          "paletteSize" -> Option(
            metaDir.getDescription(PngDirectory.TAG_PALETTE_SIZE)
          ),
          "iccProfileName" -> Option(
            metaDir.getDescription(PngDirectory.TAG_ICC_PROFILE_NAME)
          )
        ).flattenOptions
      case _ =>
        val metaDir = Option(
        ...
        Map(
          ...
          "photometricInterpretation" -> metaDir.map(
            _.getDescription(ExifDirectoryBase.TAG_PHOTOMETRIC_INTERPRETATION)
          ),
          "bitsPerSample" -> metaDir.map(
            _.getDescription(ExifDirectoryBase.TAG_BITS_PER_SAMPLE)
          )
        ).flattenOptions
    }
