...
import com.gu.mediaservice.lib.config.{MetadataConfig, UsageRightsConfig, UsageRightsStore}
...
class Uploader(val store: ImageLoaderStore,
  ...
  def fromUploadRequest(uploadRequest: UploadRequest)
    ...
    val sideEffectDependencies = ImageUploadOpsDependencies(toImageUploadOpsCfg(config), imageOps,
      metadataCleaners, supplierProcessors, usageRightsStore, storeSource, storeThumbnail, storeOptimisedPng)
...
object Uploader {
    ...
    createOptimisedFileFuture(uploadRequest, deps).flatMap(uploadRequest => {
      ...
      bracket(thumbFuture)(_.delete) { thumb =>
        ...
        val finalImage = toFinalImage(
          ...
          optimisedPng,
          uploadRequest,
          deps.metadataCleaners,
          deps.supplierProcessors,
          deps.usageRightsStore.get
        )
  ...
  private def toFinalImage(sourceStoreFuture: Future[S3Object],
    ...
    metadataCleaners: MetadataCleaners,
    supplierProcessors: SupplierProcessors,
    usageRightsConfig: UsageRightsConfig)
    (implicit ec: ExecutionContext, logMarker: LogMarker): Future[Image] = {
    Logger.info("Starting image ops")
    for {
      ...
      processedImage = supplierProcessors.process(baseImage, usageRightsConfig)
