...
object OptimiseWithPngQuant extends OptimiseOps {
  ...
  def toOptimisedFile(file: File, imageWrapper: ImageWrapper, tempDir: File)(
      ...
      ec: ExecutionContext,
      logMarker: LogMarker
  ): Future[(File, MimeType)] = Future {

    val optimisedFilePath =
      tempDir.getAbsolutePath + "/optimisedpng - " + imageWrapper.id + optimiseMimeType.fileExtension
    Stopwatch("pngquant") {
      val result = Seq(
        "pngquant",
        "--quality",
      ...
      if (result > 0)
        throw new Exception(
          s"pngquant failed to convert to optimised png file (rc = $result)"
        )
    }
    ...
    if (optimisedFile.exists()) {
      ...
      throw new Exception(
        s"Attempted to optimise PNG file ${optimisedFile.getPath}"
      )
    }
  }
