...
object Projector {
  ...
  def apply(config: ImageLoaderConfig, imageOps: ImageOperations, metadataCleaners: MetadataCleaners, supplierProcessors: SupplierProcessors)(implicit ec: ExecutionContext): Projector
  = new Projector(toImageUploadOpsCfg(config), S3Ops.buildS3Client(config), imageOps, metadataCleaners, supplierProcessors)
}
  ...
  def projectImage(srcFileDigest: DigestedFile, extractedS3Meta: S3FileExtractedMetadata, requestId: UUID)
    ...
    MimeTypeDetection.guessMimeType(tempFile_) match {
      ...
      case util.Right(mimeType) =>
        ...
        imageUploadProjectionOps.projectImageFromUploadRequest(uploadRequest, metadataCleaners, supplierProcessors)
    }
  }
}
...
class ImageUploadProjectionOps(config: ImageUploadOpsCfg,
  ...
  def projectImageFromUploadRequest(uploadRequest: UploadRequest, metadataCleaners: MetadataCleaners, supplierProcessors: SupplierProcessors)
                                   (implicit ec: ExecutionContext, logMarker: LogMarker): Future[Image] = {
    val dependenciesWithProjectionsOnly = ImageUploadOpsDependencies(config, imageOps, metadataCleaners, supplierProcessors,
