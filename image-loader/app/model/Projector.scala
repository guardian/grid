...
object Projector {
  ...
  def apply(config: ImageLoaderConfig, imageOps: ImageOperations)(implicit
      ec: ExecutionContext
  ): Projector = new Projector(
    toImageUploadOpsCfg(config),
    S3Ops.buildS3Client(config),
    imageOps,
...
object S3FileExtractedMetadata {
  def apply(s3ObjectMetadata: ObjectMetadata): S3FileExtractedMetadata = {
    ...
    val uploadedTimeRaw =
      fileUserMetadata.getOrElse("upload_time", lastModified)
...
class Projector(
  ...
  private val imageUploadProjectionOps =
    new ImageUploadProjectionOps(config, imageOps, processor)

  def projectS3ImageById(
      imageUploadProjector: Projector,
      imageId: String,
      tempFile: File,
      requestId: UUID
  )(implicit
    ...
    Future {
      ...
      Logger.info(
        s"object exists, getting s3 object at s3://${config.originalFileBucket}/$s3Key to perform Image projection"
      )

      val s3Source = s3.getObject(config.originalFileBucket, s3Key)
      val digestedFile =
        getSrcFileDigestForProjection(s3Source, imageId, tempFile)
      val extractedS3Meta = S3FileExtractedMetadata(s3Source.getObjectMetadata)

      val finalImageFuture = imageUploadProjector.projectImage(
        digestedFile,
        extractedS3Meta,
        requestId
      )
  ...
  private def getSrcFileDigestForProjection(
      s3Src: S3Object,
      imageId: String,
      tempFile: File
  ) = {
...
class ImageUploadProjectionOps(
    config: ImageUploadOpsCfg,
    imageOps: ImageOperations,
    processor: ImageProcessor
) {
  ...
  def projectImageFromUploadRequest(
    ...
    fromUploadRequestShared(
      uploadRequest,
      dependenciesWithProjectionsOnly,
      processor
    )
  }

  private def projectOriginalFileAsS3Model(
      storableOriginalImage: StorableOriginalImage
  ...
  private def projectThumbnailFileAsS3Model(
      storableThumbImage: StorableThumbImage
  )(implicit ec: ExecutionContext) = Future {
    val key = ImageIngestOperations.fileKeyFromId(storableThumbImage.id)
    val thumbMimeType = Some(
      OptimiseWithPngQuant.optimiseMimeType
    ) // this IS what we will generate.
    S3Ops.projectFileAsS3Object(
      config.thumbBucket,
      key,
  ...
  private def projectOptimisedPNGFileAsS3Model(
    ...
    val key =
      ImageIngestOperations.optimisedPngKeyFromId(storableOptimisedImage.id)
    val optimisedPngMimeType =
      Some(ImageOperations.thumbMimeType) // this IS what we will generate.
    S3Ops.projectFileAsS3Object(
      config.originalFileBucket,
      key,
