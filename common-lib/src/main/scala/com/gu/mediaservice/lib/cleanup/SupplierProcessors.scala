        ...
        case p: StaffPhotographer =>
          image.copy(
            ...
            metadata = image.metadata
              .copy(credit = Some(p.publication), byline = Some(p.photographer))
          )
        case p: ContractPhotographer =>
          image.copy(
            usageRights = p,
            metadata = image.metadata
              .copy(credit = p.publication, byline = Some(p.photographer))
          )
        case _ => image
      }
    }
...
object AapParser extends ImageProcessor {
  def apply(image: Image): Image = image.metadata.credit match {
    case Some("AAPIMAGE") | Some("AAP IMAGE") | Some("AAP") =>
      image.copy(
...
object ActionImagesParser extends ImageProcessor {
  def apply(image: Image): Image = image.metadata.credit match {
    case Some("Action Images") | Some("Action Images/Reuters") =>
      image.copy(
...
object AlamyParser extends ImageProcessor {
  def apply(image: Image): Image = image.metadata.credit match {
    case Some(credit)
        if credit.contains("Alamy") && !credit.contains("Alamy Live News") =>
      image.copy(
        usageRights = Agencies.get("alamy"),
        metadata = image.metadata.copy(credit =
          Some(credit.replace("Alamy Stock Photo", "Alamy"))
        )
      )
    case _ => image
  }
...
object AllStarParser extends ImageProcessor {
  ...
  def asAllstarAgency(image: Image, suppliersCollection: Option[String]) =
    image.copy(
      usageRights = Agency("Allstar Picture Library", suppliersCollection)
    )

  def stripAllstarFromByline(image: Image) = image.copy(
    metadata = image.metadata.copy(byline =
      image.metadata.byline.map(stripAllstarSuffix)
    )
  )
  ...
  def stripDuplicateByline(image: Image) =
    (image.usageRights, image.metadata.byline) match {
      case (agency @ Agency(supplier, Some(supplColl), _), Some(byline))
          if supplColl.toLowerCase == byline.toLowerCase => {
        image.copy(
          usageRights =
            agency.copy(suppliersCollection = image.metadata.byline),
          metadata = image.metadata.copy(
            credit = image.metadata.credit.map(credit =>
              credit.replace(supplColl, byline)
            ),
            byline = None
          )
        )
...
object ApParser extends ImageProcessor {
  ...
  def apply(image: Image): Image =
    image.metadata.credit.map(_.toLowerCase) match {
      case Some("ap") | Some("associated press") =>
        image.copy(
          usageRights = Agency("AP"),
          metadata = image.metadata.copy(credit = Some("AP"))
        )
      case Some("invision") | Some("invision/ap") | Some(InvisionFor(_)) |
          Some(PersonInvisionAp(_)) =>
        image.copy(
...
object EpaParser extends ImageProcessor {
  def apply(image: Image): Image = image.metadata.credit match {
    case Some(x) if x.matches(".*\\bEPA\\b.*") =>
      image.copy(
...
object GettyXmpParser extends ImageProcessor with GettyProcessor {
  def apply(image: Image): Image = {
    ...
    val isExcludedByCredit =
      image.metadata.credit.exists(isExcluded(_, excludedCredit))
    val isExcludedBySource =
      image.metadata.source.exists(isExcluded(_, excludedSource))
    ...
    if (!hasGettyMetadata || isExcludedByCredit || isExcludedBySource) {
      ...
      image.copy(
        ...
        metadata = image.metadata.copy(credit =
          Some(image.metadata.credit.getOrElse("Getty Images"))
        )
      )
    }
  }
...
object PaParser extends ImageProcessor {
  ...
  def apply(image: Image): Image = {
    val isPa =
      List(image.metadata.credit, image.metadata.source).flatten.exists {
        creditOrSource =>
...
object ReutersParser extends ImageProcessor {
  def apply(image: Image): Image = image.metadata.credit match {
    ...
    case Some("REUTERS") | Some("Reuters") | Some("RETUERS") | Some("REUETRS") |
        Some("REUTERS/") | Some("via REUTERS") | Some("VIA REUTERS") |
        Some("via Reuters") =>
      image.copy(
...
object RonaldGrantParser extends ImageProcessor {
  def apply(image: Image): Image = image.metadata.credit match {
    case Some("www.ronaldgrantarchive.com") | Some("Ronald Grant Archive") =>
      image.copy(
