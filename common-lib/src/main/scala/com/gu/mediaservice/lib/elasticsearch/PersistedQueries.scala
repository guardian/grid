...
object PersistedQueries extends ImageFields {
  ...
  def existedPreGrid(persistenceIdentifier: String) = filters.exists(
    NonEmptyList(identifierField(persistenceIdentifier))
  )
  ...
  val hasPhotographerUsageRights = filters.bool.must(
    filters.terms(usageRightsField("category"), photographerCategories)
  )
  val hasIllustratorUsageRights = filters.bool.must(filters.terms(usageRightsField("category"), illustratorCategories))
  val hasAgencyCommissionedUsageRights = filters.bool.must(
    filters.terms(usageRightsField("category"), agencyCommissionedCategories)
  )

  def addedGNMArchiveOrPersistedCollections(persistedRootCollections: List[String]) = filters.bool.must(
    filters.terms(collectionsField("path"), persistedRootCollections.toNel.get)
  )
  ...
  def persistedFilter(persistedRootCollections: List[String], persistenceIdentifier: String) = filters.or(
