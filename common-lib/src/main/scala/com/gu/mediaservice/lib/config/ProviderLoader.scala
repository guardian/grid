...
case class ProviderResources[Resources](
    configuration: Configuration,
    resources: Resources
)

class ProviderLoader[ProviderType, ResourcesType](providerDescription: String)(
    implicit
    providerTag: ClassTag[ProviderType],
    resourcesTag: ClassTag[ResourcesType]
) extends StrictLogging {

  private case class ConfigDetails(
      className: String,
  ...
  def seqConfigLoader(
      resources: ResourcesType
  ): ConfigLoader[Seq[ProviderType]] = (config: Config, path: String) => {
    config
      .getList(path)
      .iterator()
      .asScala
  ...
  def singletonConfigLoader(
      resources: ResourcesType
  ): ConfigLoader[ProviderType] = (config: Config, path: String) => {
    val configDetails = parseConfigValue(config.getValue(path), path, resources)
    loadProvider(configDetails)
  }

  private def parseConfigValue(
    ...
    configValue match {
      case plainClass if plainClass.valueType == ConfigValueType.STRING =>
        ConfigDetails(
          plainClass.unwrapped.asInstanceOf[String],
          None,
          resources,
          plainClass.origin,
          path
      ...
      case withConfig: ConfigObject if validConfigObject(withConfig) =>
        ...
        ConfigDetails(
          className,
          Some(Configuration(processorConfig)),
          resources,
          withConfig.origin,
          path
        )
      case _ =>
        throw new BadValue(
          configValue.origin,
          path,
          s"A ${providerDescription} can either be a class name (string) or object with className (string) and config (object) fields. This ${configValue.valueType} is not valid."
        )
    }
  }
  ...
  private def loadProvider(details: ConfigDetails): ProviderType = {
    logger.info(
      s"Dynamically loading provider from ${details.className} as specified by config path ${details.path}"
    )
    val config = ProviderResources(
      details.config.getOrElse(Configuration.empty),
      details.resources
    )
    loadProvider(details.className, config) match {
      ...
      case Left(error) =>
        val configError =
          s"Unable to instantiate ${providerDescription} from config: $error"
  ...
  def loadProvider(
      className: String,
      config: ProviderResources[ResourcesType]
  ): Either[String, ProviderType] = {
    for {
  ...
  private def loadClass(className: String): Either[String, Class[_]] =
    catchNonFatal(Class.forName(className)) {
      case _: ClassNotFoundException =>
        s"Unable to find ${providerDescription} class $className"
      case other =>
  ...
  case class ProviderCompanionObject(companionObject: AnyRef)
      extends ProviderClassType
  case class ProviderConstructor(ctor: Constructor[_]) extends ProviderClassType

  private def instantiate(
      clazz: Class[_],
      resources: ProviderResources[ResourcesType]
  ): Either[String, ProviderType] = {
    for {
      providerClassType <- discoverProviderClassType(
        clazz,
        resources.configuration.keys.nonEmpty
      )
  ...
  private def discoverProviderClassType(
      clazz: Class[_],
      configProvided: Boolean
  ): Either[String, ProviderClassType] = {
    Try(clazz.getField("MODULE$").get(clazz)).toOption match {
      case Some(companionObject) if configProvided =>
        Left(
          s"Configuration provided but ${clazz.getCanonicalName} is a companion object and doesn't take configuration."
        )
      case Some(companionObject) =>
        Right(ProviderCompanionObject(companionObject))
      case None =>
        for {
  ...
  private def findConstructor(
    ...
    val validConstructors: List[Constructor[_]] =
      allConstructors.filter(validProviderConstructor)

    validConstructors match {
      case configViolationConstructor :: Nil
          if configViolation(configViolationConstructor) =>
        Left(
          s"Configuration provided but constructor of ${clazz.getCanonicalName} with args ${constructorParamsString(configViolationConstructor)} doesn't take it."
        )
      case singleValidConstructor :: Nil =>
        Right(singleValidConstructor)
      case otherCombinations =>
        Left(
          s"""A provider must have one and only one valid constructors taking arguments of type
                |${resourcesTag.runtimeClass.getCanonicalName} or ${classOf[
            Configuration
          ].getCanonicalName}.
                |${clazz.getCanonicalName} has ${otherCombinations.length} constructors:
                |${otherCombinations.map(
  ...
  private def validProviderConstructor: Constructor[_] => Boolean = {
    constructor =>
      ...
      val onlyKnownTypes = paramTypes.forall { paramType =>
        paramType == resourcesTag.runtimeClass || paramType == classOf[
          Configuration
        ]
      }
  ...
  private def getProviderInstance(
      providerType: ProviderClassType,
      resources: ProviderResources[ResourcesType]
  ): Either[String, ProviderType] = {
    for {
      instance <- providerType match {
        case ProviderCompanionObject(companionObject) => Right(companionObject)
        case ProviderConstructor(ctor) =>
          catchNonFatal(ctor.newInstance(paramsFor(ctor, resources): _*)) {
            case ite: InvocationTargetException =>
              val cause = Option(ite.getCause)
              val error =
                s"${cause.map(_.getClass.getName).getOrElse("Unknown exception")} thrown when executing constructor ${ctor.getClass.getCanonicalName}${constructorParamsString(ctor)}. Search logs for stack trace."
              logger.error(error, cause.getOrElse(ite))
              error
            case NonFatal(other) =>
              val error =
                s"${other.getClass.getName} thrown whilst creating a new instance using constructor ${ctor.getClass.getCanonicalName}${constructorParamsString(ctor)}. Search logs for stack trace."
  ...
  private def castProvider(instance: Any): Either[String, ProviderType] = {
    if (providerTag.runtimeClass.isAssignableFrom(instance.getClass)) {
      ...
      Left(
        s"Failed to cast ${instance.getClass.getCanonicalName} to a ${providerTag.runtimeClass.getCanonicalName}"
      )
    }
  }

  private def constructorParamsString(ctor: Constructor[_]): String =
    ctor.getParameterTypes.map(_.getCanonicalName).mkString("(", ", ", ")")

  private def catchNonFatal[T](
      block: => T
  )(error: Throwable => String): Either[String, T] = {
    try {
      Right(block)
    } catch {
