...
object ItemToMediaUsage {
  ...
  private def buildFront(
      metadataMap: Map[String, Any]
  ): Option[FrontUsageMetadata] = {
    Try {
      FrontUsageMetadata(
  ...
  private def buildSyndication(
      metadataMap: Map[String, Any]
  ): Option[SyndicationUsageMetadata] = {
    Try {
  ...
  private def buildDigital(
      metadataMap: Map[String, Any]
  ): Option[DigitalUsageMetadata] = {
    Try {
      DigitalUsageMetadata(
        ...
        metadataMap
          .get("composerUrl")
          .map(x => URI.create(x.asInstanceOf[String]))
      )
    }.toOption
  }
  ...
  private def buildDownload(
      metadataMap: Map[String, Any]
  ): Option[DownloadUsageMetadata] = {
    Try {
