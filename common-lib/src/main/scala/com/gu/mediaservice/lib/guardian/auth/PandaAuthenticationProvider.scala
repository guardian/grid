...
import com.gu.pandomainauth.model.{
  AuthenticatedUser,
  User,
  Authenticated => PandaAuthenticated,
  Expired => PandaExpired,
  GracePeriod => PandaGracePeriod,
  InvalidCookie => PandaInvalidCookie,
  NotAuthenticated => PandaNotAuthenticated,
  NotAuthorized => PandaNotAuthorised
}
...
class PandaAuthenticationProvider(
  ...
  final override def authCallbackUrl: String =
    s"${resources.commonConfig.services.authBaseUri}/oauthCallback"
  ...
  override def controllerComponents: ControllerComponents =
    resources.controllerComponents
  ...
  override def authenticateRequest(
    ...
    val providerStatus = pandaStatus match {
      ...
      case PandaInvalidCookie(e) =>
        Invalid("error checking user's auth, clear cookie and re-auth", Some(e))
      case PandaExpired(authedUser) =>
        Expired(gridUserFrom(authedUser.user, request))
      case PandaGracePeriod(authedUser) =>
        Authenticated(gridUserFrom(authedUser.user, request))
      case PandaNotAuthorised(authedUser) =>
        NotAuthorised(
          s"${authedUser.user.email} not authorised to use application"
        )
      case PandaAuthenticated(authedUser) =>
        Authenticated(gridUserFrom(authedUser.user, request))
    }
    logger.info(
      s"Authenticating request ${request.uri}. Panda $pandaStatus Provider $providerStatus"
    )
    providerStatus
  }
  ...
  /** If this provider supports sending a user that is not authorised to a federated auth provider then it should
    ...
    */
  override def sendForAuthenticationCallback
      : Option[(RequestHeader, Option[RedirectUri]) => Future[Result]] =
    Some({ (requestHeader: RequestHeader, maybeUri: Option[RedirectUri]) =>
      ...
      Future
        .fromTry(Try(processOAuthCallback()(requestHeader)))
        .flatten
        .recover {
          ...
          case e: IllegalArgumentException
              if e.getMessage == "The anti forgery token did not match" => {
        ...
        .map {
          ...
          case overrideRedirect
              if overrideRedirect.header.headers
                .contains(LOCATION) && maybeUri.nonEmpty =>
            val uri = maybeUri.get
            Redirect(uri).copy(
              newCookies = overrideRedirect.newCookies,
              newSession = overrideRedirect.newSession
            )
          case other => other
        }
    })
  ...
  /** A function that allows downstream API calls to be made using the credentials of the inflight request
    ...
    */
  override def onBehalfOf(
      request: Principal
  ): Either[String, WSRequest => WSRequest] = {
    val cookieName = panDomainSettings.settings.cookieSettings.cookieName
    request.attributes.get(PandaCookieKey) match {
      case Some(cookie) =>
        Right { wsRequest: WSRequest =>
          wsRequest.addCookies(DefaultWSCookie(cookieName, cookie.value))
        }
      case None =>
        Left(s"Pan domain cookie $cookieName is missing in principal.")
    }
  }

  private def gridUserFrom(
      pandaUser: User,
      request: RequestHeader
  ): UserPrincipal = {
    val maybePandaCookie: Option[TypedEntry[Cookie]] = request.cookies
      .get(panDomainSettings.settings.cookieSettings.cookieName)
      .map(TypedEntry[Cookie](PandaCookieKey, _))
  ...
  private def buildPandaSettings() = {
    new PanDomainAuthSettingsRefresher(
      ...
      system = providerConfiguration
        .getOptional[String]("panda.system")
        .getOrElse("media-service"),
      bucketName = providerConfiguration
        .getOptional[String]("panda.bucketName")
        .getOrElse("pan-domain-auth-settings"),
      settingsFileKey = providerConfiguration
        .getOptional[String]("panda.settingsFileKey")
        .getOrElse(s"${resources.commonConfig.services.domainRoot}.settings"),
      s3Client = S3Ops.buildS3Client(
        resources.commonConfig,
        localstackAware = resources.commonConfig.useLocalAuth
      )
    )
  }

  private val userValidationEmailDomain = resources.commonConfig
    .stringOpt("panda.userDomain")
    .getOrElse("guardian.co.uk")

  final override def validateUser(authedUser: AuthenticatedUser): Boolean = {
    PandaAuthenticationProvider.validateUser(
      authedUser,
      userValidationEmailDomain,
      multifactorChecker
    )
  }
}
...
object PandaAuthenticationProvider {
  def validateUser(
    ...
    val isValidDomain =
      authedUser.user.email.endsWith("@" + userValidationEmailDomain)
    val passesMultifactor = if (multifactorChecker.nonEmpty) {
