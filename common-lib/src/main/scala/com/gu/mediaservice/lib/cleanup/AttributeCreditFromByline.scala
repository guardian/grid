...
/** A generally useful cleaner that assigns credits based on bylines.
  ...
  * @param credit
  */
case class AttributeCreditFromByline(bylines: List[String], credit: String)
  ...
  override def clean(metadata: ImageMetadata): ImageMetadata =
    metadata.byline match {
      case Some(byline) if lowercaseBylines.contains(byline.toLowerCase) =>
        metadata.copy(credit = Some(credit))
      case _ => metadata
    }
...
object AttributeCreditFromByline {
  def fromCreditBylineMap(
      creditBylineMap: Map[String, List[String]]
  ): ImageProcessor = {
    ImageProcessor.compose(
      "AttributeCreditFromBylines",
      creditBylineMap.map { case (credit, bylines) =>
