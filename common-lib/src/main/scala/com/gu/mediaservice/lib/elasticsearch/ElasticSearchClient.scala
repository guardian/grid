...
trait ElasticSearchClient extends ElasticSearchExecutions with GridLogging {
  ...
  def waitUntilHealthy(): Unit = {
    ...
    val clusterHealthResponse = Await.result(
      client.execute(
        clusterHealth().waitForStatus(HealthStatus.Green).timeout("25s")
      ),
      thirtySeconds
    )
    ...
    if (clusterHealthResponse.isError) {
      throw new RuntimeException(
        "cluster health could not be confirmed as green"
      ) // TODO Exception isn't great but our callers aren't looking at our return value
    }
  }
  ...
  def healthCheck(): Future[Boolean] = {
    ...
    executeAndLog(request, "Healthcheck").map { _ => true }.recover { case _ =>
      false
    }
  }
  ...
  def countImages(): Future[ElasticSearchImageCounts] = {
    implicit val logMarker = MarkerMap()
    val queryCatCount = catCount(
      "images"
    ) // document count only of index including live documents, not deleted documents which have not yet been removed by the merge process
    val queryImageSearch =
      search(
        "images"
      ) limit 0 // hits that match the query defined in the request
    val queryStats = indexStats(
      "images"
    ) // total accumulated values of an index for both primary and replica shards
    ...
    } yield ElasticSearchImageCounts(
  ...
  def ensureIndexExists(index: String): Unit = {
    ...
    val eventualIndexExistsResponse: Future[Response[IndexExistsResponse]] =
      client.execute {
        indexExists(index)
      }

    val indexExistsResponse =
      Await.result(eventualIndexExistsResponse, tenSeconds)
  ...
  def createImageIndex(index: String): Unit = {
    logger.info(
      s"Creating image index '$index' with $shards shards and $replicas replicas"
    )

    val eventualCreateIndexResponse: Future[Response[CreateIndexResponse]] =
      client.execute {
        ...
        val maximumFieldsOverride =
          Map("mapping.total_fields.limit" -> Integer.MAX_VALUE)
        ...
        val nonRecommendenedIndexSettingOverrides =
          maximumFieldsOverride ++ maximumPaginationOverride
        logger.warn(
          "Applying non recommended index setting overrides; please consider altering the application " +
    ...
    val createIndexResponse =
      Await.result(eventualCreateIndexResponse, tenSeconds)
  ...
  def assignAliasTo(index: String): Unit = {
    ...
    val aliasActionResponse = Await.result(
      client.execute {
        aliases(
