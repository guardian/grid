...
object PhotographersList {
  ...
  def creditBylineMap(store: Store): CreditBylineMap = store
    ...
    .map { case (publication, photographers) =>
      publication -> photographers
        .map(_.name)
        .sortWith(_.toLowerCase < _.toLowerCase)
    }

  def list(store: Store) =
    store.map(_.name).sortWith(_.toLowerCase < _.toLowerCase)

  def caseInsensitiveLookup(store: Store, lookup: String) =
    store.reverse.find { case KnownPhotographer(name, _) =>
      name.toLowerCase == lookup.toLowerCase
    }

  def getPhotographer(photographer: String): Option[Photographer] = {
    caseInsensitiveLookup(staffPhotographers, photographer)
      ...
      .orElse(caseInsensitiveLookup(contractedPhotographers, photographer).map {
        case KnownPhotographer(name, publication) =>
...
object MetadataConfig {
  ...
  type Store =
    List[
      KnownPhotographer
    ] // not a Map at this point to allow for duplicate keys (as some photographers take pics for multiple publications)

  implicit class KnownPhotographerOps(name: String) {
    def ->(publication: String): KnownPhotographer =
      KnownPhotographer(name, publication)
  }
  ...
  val staffPhotographers: Store =
    externalStaffPhotographers ++ internalStaffPhotographers
  ...
  val externalPhotographersMap =
    PhotographersList.creditBylineMap(externalStaffPhotographers)
  val staffPhotographersMap =
    PhotographersList.creditBylineMap(staffPhotographers)
  val contractPhotographersMap =
    PhotographersList.creditBylineMap(contractedPhotographers)
  val allPhotographersMap = PhotographersList.creditBylineMap(allPhotographers)
  val contractIllustratorsMap =
    PhotographersList.creditBylineMap(contractIllustrators)
