...
object CollectionsManager {
  ...
  def onlyLatest(collections: List[Collection]): List[Collection] =
    collections filter { collection =>
      ...
      !collections.exists { col =>
        {
          col.path == collection.path && col.actionData.date.isAfter(
            collection.actionData.date
          )
        }
      }
    }
  ...
  def isValidPathBit(s: String) =
    if (s.contains(delimiter) || s.contains(doublequotes)) false else true
  ...
  def getCssColour(path: List[String]) =
    path.headOption.map(_.toLowerCase).flatMap(getCollectionColour)
}
