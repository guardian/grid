...
class Authentication(config: CommonConfig,
  ...
  def getOnBehalfOfPrincipal(principal: Principal, originalRequest: RequestHeader): OnBehalfOfPrincipal = {
    val maybeEnrichFn: Either[String, WSRequest => WSRequest] = principal match {
      case _:ApiKeyAccessor => providers.apiProvider.onBehalfOf(originalRequest)
      case _:PandaUser      => providers.userProvider.onBehalfOf(originalRequest)
    }
    val enrichFn: WSRequest => WSRequest = maybeEnrichFn.fold(error => throw new IllegalStateException(error), identity)
    new OnBehalfOfPrincipal {
      override def enrich: WSRequest => WSRequest = enrichFn
    }
  }
}
