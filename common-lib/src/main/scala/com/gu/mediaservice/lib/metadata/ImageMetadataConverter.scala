...
object ImageMetadataConverter extends GridLogging {
  ...
  private def extractXMPArrayStrings(
      field: String,
      fileMetadata: FileMetadata
  ): Seq[String] = fileMetadata.xmp.get(field) match {
  ...
  private def extractPeople(fileMetadata: FileMetadata): Set[String] = {
    val xmpIptcPeople =
      extractXMPArrayStrings("Iptc4xmpExt:PersonInImage", fileMetadata)
    val xmpGettyPeople =
      extractXMPArrayStrings("GettyImagesGIFT:Personality", fileMetadata)
    (xmpIptcPeople ++ xmpGettyPeople).toSet
  }

  def fromFileMetadata(
      fileMetadata: FileMetadata,
      latestAllowedDateTime: Option[DateTime] = None
  ): ImageMetadata = {
    ...
    ImageMetadata(
      dateTaken = (fileMetadata.exifSub.get(
        "Date/Time Original Composite"
      ) flatMap (parseRandomDate(_, latestAllowedDateTime))) orElse
        (fileMetadata.iptc.get(
          "Date Time Created Composite"
        ) flatMap (parseRandomDate(_, latestAllowedDateTime))) orElse
        (readXmpHeadStringProp(
      ...
      suppliersReference =
        readXmpHeadStringProp("photoshop:TransmissionReference") orElse
      ...
      specialInstructions =
        readXmpHeadStringProp("photoshop:Instructions") orElse
          fileMetadata.iptc.get("Special Instructions"),
      // FIXME: Read XMP dc:subject array:
      keywords = fileMetadata.iptc.get("Keywords") map (_.split(
        Array(';', ',')
      ).distinct.map(_.trim).toList) getOrElse Nil,
  ...
  private lazy val dateTimeFormatters: List[DateTimeFormatter] = List(
     ...
     */
    DateTimeFormat
      .forPattern("E MMM dd HH:mm:ss.SSS 'BST' yyyy")
      .withZone(DateTimeZone.forOffsetHours(1)),
    DateTimeFormat
      .forPattern("E MMM dd HH:mm:ss 'BST' yyyy")
      .withZone(DateTimeZone.forOffsetHours(1)),
  ...
  private[metadata] def parseRandomDate(
      str: String,
      maxDate: Option[DateTime] = None
  ): Option[DateTime] = {
    dateTimeFormatters
      .foldLeft[Option[DateTime]](None) {
        ...
        case (None, formatter) =>
          safeParsing(formatter.parseDateTime(str))
  ...
  def cleanDate(
      ...
      imageId: String = "none"
  ): String = parseRandomDate(dirtyDate) match {
    case Some(cleanDate) => cleanDateFormat.print(cleanDate)
    case None => {
      logger.info(
        s"Unable to parse date $dirtyDate from field $fieldName for image $imageId"
      )
      dirtyDate
    }
  }
