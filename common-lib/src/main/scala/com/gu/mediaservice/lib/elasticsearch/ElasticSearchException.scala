  ...
  def apply(e: ElasticError): Exception with ElasticSearchError = {
    e match {
      case ElasticError(
            ...
            _,
            _
          ) => // No root causes provided.
        new Exception(s"query failed because: $r type: $t")
      ...
      case ElasticError(t, r, _, _, _, Seq(), Some(c), _, _, _) =>
        new Exception(s"query failed because: $r type: $t caused by $c")
          with ElasticSearchError {
          override def error: ElasticError = e
          override def markerContents: Map[String, Any] =
            (List("reason" -> r, "type" -> t) ::: causes(c)).toMap
        }
      case ElasticError(t, r, _, _, _, s, None, _, _, _) =>
        new Exception(
          s"query failed because: $r type: $t root cause ${s.mkString(",\n ")}"
        ) with ElasticSearchError {
          override def error: ElasticError = e

          override def markerContents: Map[String, Any] =
            Map("reason" -> r, "type" -> t, "rootCause" -> s.mkString(",\n"))
        }
      case ElasticError(t, r, _, _, _, s, Some(c), _, _, _) =>
        new Exception(
          s"query failed because: $r type: $t root cause ${s.mkString(", ")}, caused by $c"
        ) with ElasticSearchError {
          override def error: ElasticError = e

          override def markerContents: Map[String, Any] = (List(
            "reason" -> r,
            "type" -> t,
            "rootCause" -> s.mkString(",\n"),
            "causedBy" -> c.toString()
          ) ::: causes(c)).toMap
        }
      case _ =>
        new Exception(s"query failed because: unknown error")
          with ElasticSearchError {
          override def error: ElasticError = e

          override def markerContents: Map[String, Any] = Map(
            "reason" -> "unknown Elastic Search error"
          )
        }
    }
  }

  def unapply(arg: ElasticSearchError): Option[(ElasticError, LogMarker)] =
    Some((arg.error, MarkerMap(arg.markerContents)))
}

case object ElasticNotFoundException
    extends Exception(s"Elastic Search Document Not Found")
