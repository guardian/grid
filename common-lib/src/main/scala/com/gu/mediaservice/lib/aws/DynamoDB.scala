...
class DynamoDB(config: CommonConfig, tableName: String) {
  ...
  lazy val client: AmazonDynamoDBAsync = config
    .withAWSCredentials(AmazonDynamoDBAsyncClientBuilder.standard())
    .build()
  ...
  def exists(id: String)(implicit ex: ExecutionContext): Future[Boolean] =
    Future {
      table.getItem(new GetItemSpec().withPrimaryKey(IdKey, id))
    } map (Option(_).isDefined)

  def get(id: String)(implicit ex: ExecutionContext): Future[JsObject] =
    Future {
      table.getItem(
        new GetItemSpec().withPrimaryKey(IdKey, id)
      )
    } flatMap itemOrNotFound map asJsObject

  private def get(id: String, key: String)(implicit
      ex: ExecutionContext
  ): Future[Item] = Future {
    table.getItem(
      new GetItemSpec()
  ...
  def removeKey(id: String, key: String)(implicit
      ex: ExecutionContext
  ): Future[JsObject] =
    update(
      id,
  ...
  def deleteItem(id: String)(implicit ex: ExecutionContext): Future[Unit] =
    Future {
      table.deleteItem(new DeleteItemSpec().withPrimaryKey(IdKey, id))
    }

  def booleanGet(id: String, key: String)(implicit
      ex: ExecutionContext
  ...
  def booleanSet(id: String, key: String, value: Boolean)(implicit
      ex: ExecutionContext
  ): Future[JsObject] =
    update(
      id,
  ...
  def booleanSetOrRemove(id: String, key: String, value: Boolean)(implicit
      ex: ExecutionContext
  ): Future[JsObject] =
    if (value) booleanSet(id, key, value)
    else removeKey(id, key)

  def stringSet(id: String, key: String, value: JsValue)(implicit
      ex: ExecutionContext
  ): Future[JsObject] =
    update(
      id,
  ...
  def setGet(id: String, key: String)(implicit
      ex: ExecutionContext
  ): Future[Set[String]] =
    get(id, key).map { item =>
      Option(item.getStringSet(key)) match {
        case Some(set) => set.asScala.toSet
        case None      => Set()
      }
    }

  def setAdd(id: String, key: String, value: String)(implicit
      ex: ExecutionContext
  ): Future[JsObject] =
    update(
      id,
  ...
  def setAdd(id: String, key: String, value: List[String])(implicit
      ex: ExecutionContext
  ): Future[JsObject] =
    update(
      id,
  ...
  def jsonGet(id: String, key: String)(implicit
      ex: ExecutionContext
  ): Future[JsValue] =
    get(id, key).map(item => asJsObject(item))

  // We cannot update, so make sure you send over the WHOLE document
  def jsonAdd(id: String, key: String, value: Map[String, JsValue])(implicit
      ex: ExecutionContext
  ): Future[JsObject] =
    update(
      id,
  ...
  def setDelete(id: String, key: String, value: String)(implicit
      ex: ExecutionContext
  ): Future[JsObject] =
    update(
      id,
  ...
  def listGet[T](id: String, key: String)(implicit
      ex: ExecutionContext,
      reads: Reads[T]
  ): Future[List[T]] = {
  ...
  def listAdd[T](id: String, key: String, value: T)(implicit
      ex: ExecutionContext,
      tjs: Writes[T],
      rjs: Reads[T]
  ): Future[List[T]] = {
    ...
    def append =
      update(
        id,
        s"SET $key = list_append($key, :value)",
  ...
  def listRemoveIndexes[T](id: String, key: String, indexes: List[Int])(implicit
      ex: ExecutionContext,
      rjs: Reads[T]
  ): Future[List[T]] =
    update(
      id,
      s"REMOVE ${indexes.map(i => s"$key[$i]").mkString(",")}"
    ) map (j => (j \ key).as[List[T]])

  def objPut[T](id: String, key: String, value: T)(implicit
      ex: ExecutionContext,
      wjs: Writes[T],
      rjs: Reads[T]
  ): Future[T] = Future {
    ...
    val item = new Item()
      .withPrimaryKey(IdKey, id)
      .withJSON(key, Json.toJson(value).toString)
  ...
  def update(id: String, expression: String, valueMap: ValueMap)(implicit
      ex: ExecutionContext
  ): Future[JsObject] =
  ...
  def update(id: String, expression: String, valueMap: Option[ValueMap] = None)(
    ...
    val updateSpec =
      valueMap.map(baseUpdateSpec.withValueMap(_)) getOrElse baseUpdateSpec
  ...
  def mapJsValue(jsValue: JsValue)(f: JsValue => JsValue): JsValue =
    jsValue match {
      case JsObject(items) =>
        JsObject(items.map { case (k, v) => k -> mapJsValue(v)(f) })
      case JsArray(items) => JsArray(items.map(f))
      case value          => f(value)
    }

  def jsonWithNullAsEmptyString(jsValue: JsValue): JsValue =
    mapJsValue(jsValue) {
  ...
  def valueMapWithNullForEmptyString(value: Map[String, JsValue]) = {
    ...
    value
      .map { case (k, v) => (k, if (v == JsNull) null else v) }
