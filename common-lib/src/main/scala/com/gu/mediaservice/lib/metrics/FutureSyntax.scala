...
trait FutureSyntax {
  ...
  implicit class FutureOps[A](self: Future[A])(implicit ex: ExecutionContext) {
    ...
    def incrementOnSuccess[N](metric: Option[Metric[N]])(implicit
        N: Numeric[N]
    ): Future[A] =
      toMetric(metric)(_ => N.fromInt(1))

    def incrementOnFailure[B](metric: Option[Metric[B]])(
        pfn: PartialFunction[Throwable, Boolean]
    ...
    def toMetric[B](metric: Option[Metric[B]], dims: List[Dimension] = List())(
        f: A => B
    ): Future[A] = {
