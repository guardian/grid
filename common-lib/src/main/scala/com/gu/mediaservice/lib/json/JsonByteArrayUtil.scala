...
object JsonByteArrayUtil extends PlayJsonHelpers with GridLogging {
  ...
  def hasCompressionMarker(bytes: Array[Byte]) =
    bytes.head == compressionMarkerByte

  def toByteArray[T](obj: T)(implicit writes: Writes[T]): Array[Byte] =
    compress(Json.toBytes(Json.toJson(obj)))
