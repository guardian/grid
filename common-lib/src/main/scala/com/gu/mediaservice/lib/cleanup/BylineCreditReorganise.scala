...
object BylineCreditReorganise extends MetadataCleaner {
  ...
  def removeBylineFromCredit(bylineField: Field, creditField: Field) =
    bylineField
      .map { byline =>
        ...
        val ambiguousBylineCredit =
          bylineParts.length == 0 || (bylineParts.length == 1 && creditParts.length == 1)
        ...
        if (ambiguousBylineCredit) {
          ...
          val outputCredit = (bylineParts.tail.filter(
            !creditParts.contains(_)
          ) ++ creditParts.filter(_ != outputByline)).distinct.mkString("/")
  ...
  def condenseSpaceySlashes(s: String): String =
    SpaceySlashes.replaceAllIn(s, "/")
}
