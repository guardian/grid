...
class ImageOperations(playPath: String) extends GridLogging {
  ...
  private def profileLocation(
      colourModel: String,
      optimised: Boolean = false
  ): String = colourModel match {
    ...
    case model =>
      throw new Exception(s"Profile for invalid colour model requested: $model")
  }
  ...
  private def applyOutputProfile(
      base: IMOperation,
      optimised: Boolean = false
  ) = profile(base)(profileLocation("RGB", optimised))
  ...
  private def correctColour(
      base: IMOperation
  )(iccColourSpace: Option[String], colourModel: Option[String]) = {
    (iccColourSpace, colourModel) match {
      ...
      case (_, Some(model)) =>
        profile(stripProfile(base)("icm,icc"))(profileLocation(model))
    }
  }
  ...
  def cropImage(
    ...
    for {
      ...
      _ <- runConvertCmd(
        addOutput,
        useImageMagick = sourceMimeType.contains(Tiff)
      )
  ...
  def optimiseImage(resizedFile: File, mediaType: MimeType): File =
    mediaType match {
      case Png =>
        ...
        val optimisedImageName: String =
          fileName.split('.')(0) + "optimised.png"
        Seq(
          "pngquant",
          "--quality",
          "1-85",
      ...
      case Tiff =>
        logger.error(
          "Attempting to optimize a Tiff crop. Cropping as Tiff is not supported."
        )
  ...
  def createThumbnail(
    ...
    val unsharpened = unsharp(profiled)(
      thumbUnsharpRadius,
      thumbUnsharpSigma,
      thumbUnsharpAmount
    )
...
object ImageOperations {
  ...
  def identifyColourModel(sourceFile: File, mimeType: MimeType)(implicit
      ec: ExecutionContext
