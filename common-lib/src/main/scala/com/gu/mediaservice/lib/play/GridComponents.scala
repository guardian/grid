...
import com.gu.mediaservice.lib.config.{
  ApiAuthenticationProviderLoader,
  CommonConfig,
  GridConfigResources,
  UserAuthenticationProviderLoader
}
...
abstract class GridComponents[Config <: CommonConfig](
    context: Context,
    val loadConfig: GridConfigResources => Config
  ...
  val config: Config = loadConfig(
    GridConfigResources(configuration, actorSystem)
  )
  ...
  final override lazy val corsConfig: CORSConfig = CORSConfig
    ...
    .copy(
      allowedOrigins = Origins.Matching(
        Set(
          config.services.kahunaBaseUri,
          config.services.apiBaseUri
        ) ++ config.services.corsAllowedDomains
      )
    )
  ...
  val providers: AuthenticationProviders = AuthenticationProviders(
    userProvider = config.configuration
      .get[UserAuthenticationProvider]("authentication.providers.user")(
        UserAuthenticationProviderLoader.singletonConfigLoader(
          authProviderResources
        )
      ),
    apiProvider = config.configuration
      .get[MachineAuthenticationProvider]("authentication.providers.machine")(
        ApiAuthenticationProviderLoader.singletonConfigLoader(
          authProviderResources
        )
      )
  )
