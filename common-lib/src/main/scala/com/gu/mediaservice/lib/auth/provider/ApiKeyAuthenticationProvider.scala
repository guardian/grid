...
object ApiKeyAuthenticationProvider {
  val ApiKeyHeader: TypedKey[(String, String)] =
...
class ApiKeyAuthenticationProvider(
...
) extends MachineAuthenticationProvider
    with StrictLogging {
  implicit val executionContext: ExecutionContext =
    resources.controllerComponents.executionContext
  var keyStorePlaceholder: Option[KeyStore] = _

  override def initialise(): Unit = {
    val store = new KeyStore(
      configuration.get[String]("authKeyStoreBucket"),
  ...
  def keyStore: KeyStore = keyStorePlaceholder.getOrElse(
      ...
      case Some(key) =>
        keyStore.lookupIdentity(key) match {
          ...
          case Some(apiKey) =>
            // valid api key
            if (
              ApiAccessor.hasAccess(
                apiKey,
                request,
                resources.commonConfig.services
              )
            ) {
              ...
              val accessor = MachinePrincipal(
                apiKey,
                TypedMap(
                  ApiKeyAuthenticationProvider.ApiKeyHeader -> (ApiKeyAuthenticationProvider.apiKeyHeaderName -> key)
                )
              )
              logger.info(
                s"Using api key with name ${apiKey.identity} and tier ${apiKey.tier}",
                apiKey
              )
              ...
              NotAuthorised(
                s"API key ${apiKey.identity} valid but not authorised for this request"
              )
            }
  ...
  override def onBehalfOf(
    ...
    principal.attributes.get(ApiKeyAuthenticationProvider.ApiKeyHeader) match {
      case Some(apiKeyHeaderTuple) =>
        Right { wsRequest: WSRequest =>
          wsRequest.addHttpHeaders(apiKeyHeaderTuple)
        }
      case None =>
        Left(
          s"API key not found in request, no header ${ApiKeyAuthenticationProvider.apiKeyHeaderName}"
        )
    }
  }
}
