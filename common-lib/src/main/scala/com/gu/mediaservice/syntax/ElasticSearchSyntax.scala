...
trait ElasticSearchSyntax {
  ...
  final implicit class ListenableActionFutureSyntax[A](
      self: ListenableActionFuture[A]
  ) {
    def asScala: Future[A] = FutureConversions(self)
  }

  final implicit class GetResponseSyntax(self: GetResponse) {
    def sourceOpt: Option[JsValue] =
      Option(self.getSourceAsString) map Json.parse
  }
  ...
  final implicit class ActionRequestBuilderSyntax[A <: ActionResponse](
    ...
    def executeAndLog(
      ...
      result.foreach { _ =>
        ...
        Logger.info(s"$message - query returned successfully in $elapsed ms")(
          markers
        )
      }
      ...
      result.failed.foreach { e =>
        ...
        Logger.error(
          s"$message - query failed after $elapsed ms: ${e.getMessage} cs: ${e.getCause}"
        )(markers)
      }
