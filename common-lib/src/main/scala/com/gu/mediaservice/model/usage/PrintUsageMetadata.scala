...
case class PrintUsageMetadata(
  ...
  override def toMap = Map(
  ...
  ) ++ size.foldLeft[MapStringIntElement](Nil)((_, m) =>
    List("size" -> m.toMap.asJava)
  ) ++
    ...
    notes.foldLeft[StringElement](Nil)((_, s) =>
      if (s.isEmpty) Nil else List("notes" -> s)
    ) ++
    source.foldLeft[StringElement](Nil)((_, s) =>
      if (s.isEmpty) Nil else List("source" -> s)
    )
}
object PrintUsageMetadata {
  ...
  implicit val writes: Writes[PrintUsageMetadata] =
    Json.writes[PrintUsageMetadata]
}
