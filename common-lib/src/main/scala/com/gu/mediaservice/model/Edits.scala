...
object Edits {
  ...
  implicit val EditsReads: Reads[Edits] = (
    (__ \ "archived").readNullable[Boolean].map(_ getOrElse false) ~
      ...
      (__ \ "metadata")
        .readNullable[ImageMetadata]
        .map(_ getOrElse emptyMetadata) ~
  ...
  implicit val EditsWrites: Writes[Edits] = (
    (__ \ "archived").write[Boolean] ~
      ...
      (__ \ "metadata")
        .writeNullable[ImageMetadata]
        .contramap(noneIfEmptyMetadata) ~
...
trait EditsResponse {
  ...
  def editsEntity(id: String): Writes[Edits] = (
    (__ \ "archived")
      .write[ArchivedEntity]
      .contramap(archivedEntity(id, _: Boolean)) ~
      (__ \ "labels")
        .write[SetEntity]
        .contramap(setEntity(id, "labels", _: List[String])) ~
      (__ \ "metadata")
        .write[MetadataEntity]
        .contramap(metadataEntity(id, _: ImageMetadata)) ~
      (__ \ "usageRights")
        .write[UsageRightsEntity]
        .contramap(usageRightsEntity(id, _: Option[UsageRights])) ~
      (__ \ "photoshoot")
        .write[PhotoshootEntity]
        .contramap(photoshootEntity(id, _: Option[Photoshoot]))
  )(unlift(Edits.unapply))

  def photoshootEntity(
      id: String,
      photoshoot: Option[Photoshoot]
  ): PhotoshootEntity =
  ...
  def metadataEntity(id: String, m: ImageMetadata): MetadataEntity =
    EmbeddedEntity(
      entityUri(id, "/metadata"),
      Some(m),
      actions = List(
        Action(
          "set-from-usage-rights",
          entityUri(id, "/metadata/set-from-usage-rights"),
          "POST"
        )
      )
    )
  ...
  def setEntity(id: String, setName: String, labels: List[String]): SetEntity =
    EmbeddedEntity(
      entityUri(id, s"/$setName"),
      Some(labels.map(setUnitEntity(id, setName, _)))
    )
  ...
  def setUnitEntity(
      ...
      setName: String,
      name: String
  ): EmbeddedEntity[String] =
    EmbeddedEntity(
      entityUri(id, s"/$setName/${URLEncoder.encode(name, "UTF-8")}"),
      Some(name)
    )
