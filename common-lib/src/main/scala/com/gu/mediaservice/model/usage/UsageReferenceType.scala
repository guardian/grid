...
object UsageReferenceType {
  implicit val reads: Reads[UsageReferenceType] =
    JsPath.read[String].map(UsageReferenceType(_))
  implicit val writer: Writes[UsageReferenceType] =
    (usageReferenceType: UsageReferenceType) =>
      JsString(usageReferenceType.toString)

  def apply(usageReferenceType: String): UsageReferenceType =
    usageReferenceType.toLowerCase match {
