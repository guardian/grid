...
case class Image(
  ...
  def hasInferredSyndicationRightsOrNoRights: Boolean =
    syndicationRights.forall(_.isInferred)
  ...
  def syndicationStatus: SyndicationStatus = {
    ...
    if (!isRightsAcquired) {
      ...
      if (hasSyndicationUsage) {
        ...
        val allowSyndicationLease =
          leases.leases.find(_.access == AllowSyndicationLease)
        val denySyndicationLease =
          leases.leases.find(_.access == DenySyndicationLease)
  ...
  implicit val ImageReads: Reads[Image] = (
    (__ \ "id").read[String] ~
      ...
      (__ \ "identifiers")
        ...
        .map(_ getOrElse Map()) ~
      (__ \ "uploadInfo")
        .readNullable[UploadInfo]
        .map(_ getOrElse UploadInfo()) ~
      (__ \ "source").read[Asset] ~
      (__ \ "thumbnail").readNullable[Asset] ~
      (__ \ "optimisedPng").readNullable[Asset] ~
      (__ \ "fileMetadata")
        .readNullable[FileMetadata]
        .map(_ getOrElse FileMetadata()) ~
