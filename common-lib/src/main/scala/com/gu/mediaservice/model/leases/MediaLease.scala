...
object MediaLeaseType {
  ...
  implicit val writer: Writes[MediaLeaseType] =
    (mediaLeaseType: MediaLeaseType) => JsString(mediaLeaseType.name)
...
case object AllowSyndicationLease extends MediaLeaseType {
  val name = "allow-syndication"
}
case object DenySyndicationLease extends MediaLeaseType {
  val name = "deny-syndication"
}
...
case class MediaLease(
  ...
  private def afterStart =
    startDate.forall(start => new DateTime().isAfter(start))
  private def beforeEnd = endDate.forall(end => new DateTime().isBefore(end))

  private def withValidNotesField: MediaLease = notes match {
    case Some(note) if note.trim.length == 0 =>
      this.copy(notes = None) // cannot save empty string in dynamo
    case _ => this
  }

  private def withValidEndDateField: MediaLease =
    if (access == AllowSyndicationLease)
      this.copy(endDate = None) // an allow-syndication cannot end
    else this

  private def withValidStartDateField: MediaLease =
    if (access == DenySyndicationLease)
      this.copy(startDate = None) // a deny-syndication cannot start
    else this

  def prepareForSave: MediaLease =
    this.withValidNotesField.withValidStartDateField.withValidEndDateField

  def active = afterStart && beforeEnd

  def isSyndication =
    access == AllowSyndicationLease || access == DenySyndicationLease
...
object MediaLease {
  ...
  implicit val MediaLeaseWrites: Writes[MediaLease] =
    (mediaLease: MediaLease) =>
      Json
        .toJson(mediaLease)(MediaLeasePlainWrites)
        .as[JsObject] + ("active" -> JsBoolean(mediaLease.active))
