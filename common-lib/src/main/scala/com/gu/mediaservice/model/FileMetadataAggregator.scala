...
object FileMetadataAggregator {
  ...
  private def entryToAggregatedKeyAndJsValue(
      k: String,
      v: MetadataEntry
    ...
    def toCustomObjectKeyAndValue(k: String, v: MetadataEntry) = {
      ...
      val stringifiedObj = Json
        .stringify(JsObject(Seq((objectFieldName, v.jsValue))))
        .replace("\"", "'")
    ...
    if (isArrayKey(k)) toArrayKeyAndValue(k, v)
    else if (isCustomObjectKey(k)) toCustomObjectKeyAndValue(k, v)
    else (k, v)
  }

  private def getIdxBetweenArrayBrackets(k: String): Int =
    k.substring(k.lastIndexOf("[") + 1, k.lastIndexOf("]")).trim.toInt

  private def aggregateCurrentMetadataLevel(
      nodes: Map[String, MetadataEntry]
    ...
    val entriesWithIndexes: Map[String, MetadataEntry] =
      toEntriesWithUpdatedIndexes(nodes)

    val mutableMap = scala.collection.mutable
      .Map[String, Either[MetadataEntry, List[MetadataEntry]]]()

    entriesWithIndexes.foreach { case (k, v) =>
      val (aggregatedKey, newMetadataEntry) =
        entryToAggregatedKeyAndJsValue(k, v)
    ...
    val mapWithSortedValuesAtCurrentLevel = mutableMap.mapValues {
      ...
      case scala.util.Right(value) => {
        ...
        val (jsArrays, jsStrings) =
          sortedList.map(_.jsValue).partition(_.isInstanceOf[JsArray])

        val aggJsArrays: JsArray = jsArrays
          .map(_.as[JsArray])
          .foldLeft(JsArray.empty)((acc, arrayItem) => acc ++ arrayItem)
        val aggJsStrings: JsArray = jsStrings
          .map(_.as[JsString])
          .foldLeft(JsArray.empty)((acc, item) => acc.append(item))
  ...
  def aggregateMetadataMap(
      flatProperties: Map[String, String]
  ): Map[String, JsValue] = {
    ...
    val initialMetadataStructure = toInitialEntriesWithIndexes(
      flatProperties.mapValues(JsString)
    )
    ...
    def anyKeyIsDynamicObjectKey(keys: Set[String]) =
      keys.exists(isCustomObjectKey)

    while (
      anyKeyIsArrayKey(aggMetadata.keySet) || anyKeyIsDynamicObjectKey(
        aggMetadata.keySet
      )
    ) aggMetadata = aggregateCurrentMetadataLevel(aggMetadata)
