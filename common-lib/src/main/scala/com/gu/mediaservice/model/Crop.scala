...
object Crop {
  ...
  def createFromCropSource(
      by: Option[String],
      timeRequested: Option[DateTime],
      specification: CropSpec,
      master: Option[Asset] = None,
      cropSizings: List[Asset] = Nil
  ): Crop =
    Crop(
      Some(getCropId(specification.bounds)),
      by,
      timeRequested,
      specification,
      master,
...
object ExportType {
  ...
  implicit val exportTypeWrites: Writes[ExportType] =
    Writes[ExportType](t => JsString(t.name))
...
case class CropSpec(
    ...
    `type`: ExportType = ExportType.default
)
...
object CropSpec {
  ...
  implicit val cropSpecReads: Reads[CropSpec] = (
    (__ \ "uri").read[String] ~
      ...
      (__ \ "type")
        .readNullable[ExportType]
        .map(_.getOrElse(ExportType.default))
  )(CropSpec.apply _)
}
