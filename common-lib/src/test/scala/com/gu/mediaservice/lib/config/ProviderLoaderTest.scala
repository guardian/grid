...
case class ResourceTestProvider(resources: TestProviderResources)
...
case class ConfigResourceTestProvider(
    config: Configuration,
    resources: TestProviderResources
) extends TestProvider {
  override def info: String =
    s"config-resource-test-provider ${config.hashCode} ${resources.aResource}"
}

case class ResourceConfigTestProvider(
    resources: TestProviderResources,
    config: Configuration
) extends TestProvider {
  override def info: String =
    s"resource-config-test-provider ${resources.aResource} ${config.hashCode}"
}

case class BadTestProvider(
    resources: TestProviderResources,
    config: Configuration
) extends TestProvider {
...
class TestProviderWithStringConstructor(configString: String)
    extends TestProvider {
  def info: String = s"not-test-provider $configString"
}

object TestProviderLoader
    extends ProviderLoader[TestProvider, TestProviderResources]("test provider")
...
class ProviderLoaderTest extends FreeSpec with Matchers with EitherValues {
  ...
  "The class reflector" - {
    ...
    "should successfully load a no arg TestProvider instance" in {
      val instance = TestProviderLoader.loadProvider(
        classOf[NoArgTestProvider].getCanonicalName,
        providerResources
      )
    ...
    "should successfully load a companion object TestProvider" in {
      val instance = TestProviderLoader.loadProvider(
        ObjectTestProvider.getClass.getCanonicalName,
        providerResources
      )
    ...
    "should successfully load a config arg TestProvider instance" in {
      val instance = TestProviderLoader.loadProvider(
        classOf[ConfigTestProvider].getCanonicalName,
        providerResources
      )
    ...
    "should successfully load a resource arg TestProvider instance" in {
      val instance = TestProviderLoader.loadProvider(
        classOf[ResourceTestProvider].getCanonicalName,
        providerResources
      )
    ...
    "should successfully load a config, resource arg TestProvider instance" in {
      val instance = TestProviderLoader.loadProvider(
        classOf[ConfigResourceTestProvider].getCanonicalName,
        providerResources
      )
    ...
    "should successfully load a resource, config arg TestProvider instance" in {
      val instance = TestProviderLoader.loadProvider(
        classOf[ResourceConfigTestProvider].getCanonicalName,
        providerResources
      )
    ...
    "should fail to load something that isn't an TestProvider" in {
      val instance = TestProviderLoader.loadProvider(
        classOf[NotATestProvider].getCanonicalName,
        providerResources
      )
    ...
    "should fail to load something that doesn't have a suitable constructor" in {
      val instance = TestProviderLoader.loadProvider(
        classOf[TestProviderWithStringConstructor].getCanonicalName,
        providerResources
      )
    ...
    "should fail to load something that doesn't exist" in {
      val instance = TestProviderLoader.loadProvider(
        "com.gu.mediaservice.lib.config.TestProviderThatDoesntExist",
        providerResources
      )
    ...
    val nonEmptyConfigProviderResources =
      ProviderResources(nonEmptyConfig, resources)
    ...
    "should fail to load a provider if the constructor throws an exception" in {
      val instance = TestProviderLoader.loadProvider(
        classOf[BadTestProvider].getCanonicalName,
        providerResources
      )
  ...
  "The config loader" - {
    ...
    implicit val testProviderConfigLoader: ConfigLoader[TestProvider] =
      TestProviderLoader.singletonConfigLoader(resources)
    implicit val testProvidersConfigLoader: ConfigLoader[Seq[TestProvider]] =
      TestProviderLoader.seqConfigLoader(resources)
    ...
    "should load an image processor which has configuration" in {
      ...
      inside(processor) { case ConfigTestProvider(config) =>
        config.get[String]("parameter") shouldBe "value"
      }
    }
    ...
    "should fail to load multiple image processors if they don't meet the spec" in {
      ...
      thrown.getMessage should include(
        "A test provider can either be a class name (string) or object with className (string) and config (object) fields. This OBJECT is not valid."
      )
    }
    ...
    "should fail to load an image processors if the config isn't a string" in {
      ...
      thrown.getMessage should include(
        "A test provider can either be a class name (string) or object with className (string) and config (object) fields. This LIST is not valid"
      )
    }
