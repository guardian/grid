...
class RedundantTokenRemoverTest
    extends FunSpec
    with Matchers
    with MetadataHelper {
  ...
  separators.foreach { s =>
    ...
    it(
      s"Clean partially redundant byline, clean partially redundant credit - '$s' separator"
    ) {
    ...
    it(
      s"Keep good byline, simplify redundant credit (use the rightmost) - '$s' separator"
    ) {
