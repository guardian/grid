...
import com.gu.mediaservice.lib.auth.provider.{
  ApiKeyAuthenticationProvider,
  Authenticated,
  AuthenticationProviderResources,
  Invalid,
  NotAuthenticated,
  NotAuthorised
}
...
class ApiKeyAuthenticationProviderTest
  ...
  private val config = new CommonConfig(
    Configuration.load(Environment.simple())
  ) {}
  private val providerConfig = Configuration.empty
  private val controllerComponents: DefaultControllerComponents =
    DefaultControllerComponents(null, null, null, null, null, global)
  private val resources = AuthenticationProviderResources(
    config,
    actorSystem,
    wsClient,
  ...
  private val provider =
    new ApiKeyAuthenticationProvider(providerConfig, resources) {
      ...
      override def keyStore: KeyStore =
        new KeyStore("not-used", resources.commonConfig) {
  ...
  "requestAuthentication" - {
    "should return Authenticated if the key is valid" in {
      val testHeader =
        ApiKeyAuthenticationProvider.apiKeyHeaderName -> "key-chuckle"
      val status =
        provider.authenticateRequest(FakeRequest().withHeaders(testHeader))
      inside(status) {
        case Authenticated(MachinePrincipal(apiAccessor, attributes)) =>
          apiAccessor shouldBe ApiAccessor("brothers", Internal)
          attributes.contains(
            ApiKeyAuthenticationProvider.ApiKeyHeader
          ) shouldBe true
          attributes.get(
            ApiKeyAuthenticationProvider.ApiKeyHeader
          ) shouldBe Some(testHeader)
      }
    }
    ...
    "should return Invalid if the key is invalid" in {
      val testHeader =
        ApiKeyAuthenticationProvider.apiKeyHeaderName -> "key-banana"
      val status =
        provider.authenticateRequest(FakeRequest().withHeaders(testHeader))
      inside(status) { case Invalid(message, _) =>
        message shouldBe "API key not valid"
      }
    }
    "should return NotAuthorised if the key doesn't have enough permissions" in {
      val testHeader =
        ApiKeyAuthenticationProvider.apiKeyHeaderName -> "key-limited"
      val status = provider.authenticateRequest(
        FakeRequest().withHeaders(testHeader).withMethod("POST")
      )
      inside(status) { case NotAuthorised(message) =>
