...
class SupplierProcessorsTest extends FunSpec with Matchers with MetadataHelper {
  ...
  it(
    "should leave supplier and suppliersCollection empty if credit doesn't match"
  ) {
  ...
  describe("Photographer") {
    it("should match StaffPhotographer byline") {
      ...
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(
        StaffPhotographer("Graham Turner", "The Guardian")
      )
    ...
    it("should match ContractPhotographer byline") {
      ...
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(
        ContractPhotographer("Linda Nylind", Option("The Guardian"))
      )
      processedImage.metadata.credit should be(Some("The Guardian"))
    }
    ...
    it("should correct casing of photographer") {
      ...
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(
        ContractPhotographer("Murdo MacLeod", Option("The Guardian"))
      )
  ...
  describe("Alamy") {
    ...
    it(
      "should match 'Alamy Stock Photo' credit, and replace 'Alamy Stock Photo' with 'Alamy'"
    ) {
    ...
    it("should match credit with Alamy as a suffix with '/'") {
      val image =
        createImageFromMetadata("credit" -> "Prod.DB/Alamy Stock Photo")
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(Agency("Alamy"))
    ...
    it(
      "should not match credit with Alamy when the credit contains 'Alamy Live News', because we only have rights after 48 hours, and there's no provision to add a 'deny' lease for that period yet"
    ) {
      val image =
        createImageFromMetadata("credit" -> "Alamy Live News/Alamy Live News")
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(NoRights)
      processedImage.metadata.credit should be(
        Some("Alamy Live News/Alamy Live News")
      )
    }
  }
  ...
  describe("Allstar") {
    ...
    it("should match 'Sportsphoto Ltd./Allstar' credit") {
      val image =
        createImageFromMetadata("credit" -> "Sportsphoto Ltd./Allstar")
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(
    ...
    it("should match 'Allstar/UNIVERSAL' credit") {
      ...
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(
        Agency("Allstar Picture Library", Some("UNIVERSAL"))
      )
      processedImage.metadata.credit should be(Some("Allstar/UNIVERSAL"))
    }

    it(
      "should strip redundant byline but use it as canonical casing for credit"
    ) {
      val image = createImageFromMetadata(
        "credit" -> "Allstar/UNIVERSAL PICTURES",
        "byline" -> "Universal Pictures"
      )
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(
        Agency("Allstar Picture Library", Some("Universal Pictures"))
      )
      processedImage.metadata.credit should be(
        Some("Allstar/Universal Pictures")
      )
  ...
  describe("Getty Images") {
    it(
      "should detect getty file metadata and use source as suppliersCollection"
    ) {
      val image =
        createImageFromMetadata("credit" -> "AFP/Getty", "source" -> "AFP")
      val gettyImage = image.copy(fileMetadata =
        FileMetadata(getty = Map("Original Filename" -> "lol.jpg"))
      )
    ...
    it(
      "should exclude images that have Getty metadata that aren't from Getty"
    ) {
      val image = createImageFromMetadata("credit" -> "NEWSPIX INTERNATIONAL")
      val notGettyImage = image.copy(fileMetadata =
        FileMetadata(getty = Map("Composition" -> "Headshot"))
      )
    ...
    it(
      "should exclude images that have Getty metadata that also have 'Pinnacle Photo Agency Ltd' as source"
    ) {
      val image =
        createImageFromMetadata("source" -> "Pinnacle Photo Agency Ltd")
      val notGettyImage = image.copy(fileMetadata =
        FileMetadata(getty = Map("dummy" -> "metadata"))
      )
      val processedImage = applyProcessors(notGettyImage)
      processedImage.usageRights should be(NoRights)
    }
    ...
    it(
      "should use 'Getty Images' as credit if missing from the file metadata"
    ) {
      val image = createImageFromMetadata()
      val gettyImage = image.copy(fileMetadata =
        FileMetadata(getty = Map("Original Filename" -> "lol.jpg"))
    ...
    it("should match 'The LIFE Images Collection/Getty' credit") {
      val image = createImageFromMetadata(
        "credit" -> "The LIFE Images Collection/Getty",
        "source" -> "The LIFE Images Collection"
      )
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(
        Agency("Getty Images", Some("The LIFE Images Collection"))
      )
      processedImage.metadata.credit should be(
        Some("The LIFE Images Collection/Getty")
      )
    }
    ...
    it("should match 'Getty Images/Ikon Images' credit") {
      ...
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(
        Agency("Getty Images", Some("Ikon Images"))
      )
    ...
    it("should match 'Bloomberg/Getty Images' credit") {
      ...
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(
        Agency("Getty Images", Some("Bloomberg"))
      )
    ...
    it("should match 'Some Long Provider/Getty Im' credit") {
      val image = createImageFromMetadata(
        "credit" -> "Some Long Provider/Getty Im",
        "source" -> "Some Long Provider"
      )
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(
        Agency("Getty Images", Some("Some Long Provider"))
      )
      processedImage.metadata.credit should be(
        Some("Some Long Provider/Getty Im")
      )
    }

    it("should match 'Getty Images for Apple' credit") {
      val image = createImageFromMetadata(
        "credit" -> "Getty Images for Apple",
        "source" -> "Getty Images Europe"
      )
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(
        Agency("Getty Images", Some("Getty Images Europe"))
      )
    ...
    it("should match 'WireImage' credit") {
      ...
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(
        Agency("Getty Images", Some("WireImage"))
      )
    ...
    it("should match 'Hulton' credit") {
      ...
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(
        Agency("Getty Images", Some("Hulton"))
      )
  ...
  describe("PA") {
    ...
    it("should match PA source if credit doesn't match") {
      val image =
        createImageFromMetadata("credit" -> "BBC/PA", "source" -> "PA")
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(Agency("PA"))
    }
    ...
    it("should match 'Press Association Images' credit") {
      val image =
        createImageFromMetadata("credit" -> "Press Association Images")
      val processedImage = applyProcessors(image)
      processedImage.usageRights should be(Agency("PA"))
    }
  ...
  describe("Rex Features") {
    it("should match Rex Features source") {
      val image = createImageFromMetadata(
        "credit" -> "Tim Ireland/REX Shutterstock",
        "source" -> "Rex Features"
      )
      val processedImage = applyProcessors(image)
      ...
      processedImage.metadata.credit should be(
        Some("Tim Ireland/REX Shutterstock")
      )
    ...
    it("should match '*/ Rex Features' credit") {
      val image =
        createImageFromMetadata("credit" -> "Bleddyn Butcher / Rex Features")
      val processedImage = applyProcessors(image)
  ...
  describe("Ronald Grant") {
    it("should match www.ronaldgrantarchive.com credit") {
      val image =
        createImageFromMetadata("credit" -> "www.ronaldgrantarchive.com")
      val processedImage = applyProcessors(image)
