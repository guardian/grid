...
class StripCopyrightPrefixTest
    extends FunSpec
    with Matchers
    with MetadataHelper {
  ...
  it("should strip Copyright of prefix") {
    val metadata =
      createImageMetadata("credit" -> "Copyright of Acme Corporation")
    val cleanedMetadata = StripCopyrightPrefix.clean(metadata)
    cleanedMetadata.credit should be(Some("Acme Corporation"))
  }
  ...
  it("should strip copyright followed by colon prefix") {
    val metadata =
      createImageMetadata("credit" -> "Copyright : Acme Corporation")
    val cleanedMetadata = StripCopyrightPrefix.clean(metadata)
    cleanedMetadata.credit should be(Some("Acme Corporation"))
  }
  ...
  it("should strip a combination of copyright prefixes") {
    val metadata =
      createImageMetadata("credit" -> "Copyright (c) Acme Corporation")
    val cleanedMetadata = StripCopyrightPrefix.clean(metadata)
    cleanedMetadata.credit should be(Some("Acme Corporation"))
  }
