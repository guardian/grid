...
class DynamoDBTest extends FunSpec with Matchers {
  ...
  describe("jsonToValueMap") {
    it("should convert a simple JsObject to a valueMap") {
      val json = Json
        .toJson(SimpleDynamoDBObj("this is a string", 100, true, List("list")))
        .as[JsObject]
    ...
    it("should convert a nested JsObject to a valueMap") {
      val nestedObj = NestedDynamoDBObj(
        "string",
        100,
        false,
        SimpleDynamoDBObj("strang", 500, true, List("list"))
      )
      ...
      val a: List[String] = List(
        simpleMap.get("a").asInstanceOf[java.util.ArrayList[String]].toArray: _*
      ).asInstanceOf[List[String]]
    ...
    it("should convert a Collection to ValueMap") {
      val collection = Collection.build(
        List("g2", "art", "batik"),
        ActionData("mighty.mouse@guardian.co.uk", DateTime.now)
      )
...
object SimpleDynamoDBObj {
  implicit def formats: Format[SimpleDynamoDBObj] =
    Json.format[SimpleDynamoDBObj]
}

case class NestedDynamoDBObj(
    ss: String,
    dd: BigDecimal,
    bb: Boolean,
    simple: SimpleDynamoDBObj
)
object NestedDynamoDBObj {
  implicit def formats: Format[NestedDynamoDBObj] =
    Json.format[NestedDynamoDBObj]
}
