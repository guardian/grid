...
class AuthenticationTest
  ...
  private def makeCookie(
      firstName: String = "Test",
      lastName: String = "User",
      email: String = "test@user",
      expired: Boolean = false,
      veryExpired: Boolean = false
  ): Cookie = {
  ...
  def makeAuthenticationInstance(
      testProviders: AuthenticationProviders
  ): Authentication = {
  ...
  "authenticationStatus" - {
    val testProviders = AuthenticationProviders(
      new UserAuthenticationProvider {
          ...
          request.cookies.get(COOKIE_NAME) match {
            ...
            case Some(cookie) =>
              parseCookie(cookie) match {
                ...
                case Some(token @ AuthToken(_, _, _, _, true)) =>
                  Expired(token.user)
                case Some(token) if token.email == "test@user" =>
                  Authenticated(token.user)
                case Some(token) =>
                  NotAuthorised(s"${token.email} not authorised")
              }
          }
        }

        override def sendForAuthentication
            : Option[RequestHeader => Future[Result]] = ???
        ...
        override def flushToken: Option[(RequestHeader, Result) => Result] =
          Some({ (_: RequestHeader, result: Result) =>
      ...
      new MachineAuthenticationProvider {
        override def authenticateRequest(
          ...
          request.headers.get(HEADER_NAME) match {
            case None => NotAuthenticated
            case Some(key)
                if key.startsWith("key-") && key.endsWith("-blocked") =>
              NotAuthorised(s"$key is blocked")
            case Some(key) if key.startsWith("key-") =>
              Authenticated(MachinePrincipal(ApiAccessor(key, Internal)))
    ...
    "should return unauthorised if request is empty (no headers)" in {
      ...
      authStatus.left.value.map { result =>
        ...
        Helpers
          ...
          .\("errorKey")
          .as[String] shouldBe "authentication-failure"
      }
    }
    ...
    "should return 419 when expired" in {
      ...
      authStatus.left.value.map { result =>
        ...
        Helpers
          .contentAsJson(Future.successful(result))
          .\("errorKey")
          .as[String] shouldBe "authentication-expired"
      }
    }
    "should return forbidden when user is not authorised by provider" in {
      ...
      authStatus.left.value.map { result =>
        ...
        Helpers
          .contentAsJson(Future.successful(result))
          .\("errorKey")
          .as[String] shouldBe "principal-not-authorised"
      }
    }
    "should authenticate with an API key" in {
      val request = FakeRequest().withHeaders(HEADER_NAME -> "key-client")
      val authStatus = auth.authenticationStatus(request)
      authStatus.right.value shouldBe MachinePrincipal(
        ApiAccessor("key-client", Internal)
      )
    }
    ...
    "should prioritise machine authentication over user authentication" in {
      val request = FakeRequest()
        .withCookies(makeCookie())
        .withHeaders(HEADER_NAME -> "key-client")
      val authStatus = auth.authenticationStatus(request)
      authStatus.right.value shouldBe MachinePrincipal(
        ApiAccessor("key-client", Internal)
      )
  ...
  "getOnBehalfOfPrincipal" - {
    ...
    val testProviders = AuthenticationProviders(
      new UserAuthenticationProvider {
        ...
        override def onBehalfOf(
            request: Authentication.Principal
        ): Either[String, WSRequest => WSRequest] = request match {
          case UserPrincipal(_, _, _, attributes)
              if attributes.contains(CookieKey) =>
            Right(req =>
              req.addCookies(
                DefaultWSCookie(
                  COOKIE_NAME,
                  attributes.get(CookieKey).get.value
                )
              )
            )
          case UserPrincipal(_, _, email, _) =>
            Left(s"Unable to build onBehalfOf function for $email")
        }
      },
      new MachineAuthenticationProvider {
        override def authenticateRequest(
        ...
        override def onBehalfOf(
          ...
          case MachinePrincipal(_, attributes)
              if attributes.contains(HeaderKey) =>
            Right(req => req.addHttpHeaders(attributes.get(HeaderKey).get))
          case MachinePrincipal(ApiAccessor(identity, _), _) =>
            Left(s"Unable to build onBehalfOf function for $identity")
        }
      }
    )
    ...
    "return function for user principal" in {
      val testUser = UserPrincipal(
        "Test",
        "User",
        "test@user",
        TypedMap(CookieKey -> Cookie(COOKIE_NAME, "this is my cookie value"))
      )
      val onBehalfOfFn: OnBehalfOfPrincipal =
        auth.getOnBehalfOfPrincipal(testUser)
    ...
    "return function for machine principal" in {
      val apiAccessor = MachinePrincipal(
        ApiAccessor("my-client-id", Internal),
        TypedMap(HeaderKey -> (HEADER_NAME -> "my-client-id-key"))
      )
      val onBehalfOfFn: OnBehalfOfPrincipal =
        auth.getOnBehalfOfPrincipal(apiAccessor)
      WsTestClient.withClient { client =>
        val req = client.url("https://example.com")
        val modifiedReq = onBehalfOfFn(req)
