  ...
  describe("CollectionManager") {
    ...
    describe("serialisation") {
      it("should convert path to string with /") {
        CollectionsManager.pathToString(
          List("g2", "art", "film")
        ) shouldBe "g2/art/film"
      }

      it("should convert a string to a path") {
        CollectionsManager
          .stringToPath("g2/art/film") shouldBe List("g2", "art", "film")
      }

      it("should convert a URI to a path") {
        CollectionsManager.uriToPath("g2/art/rhythm+blues") shouldBe List(
          "g2",
          "art",
          "rhythm blues"
        )
      ...
      it("should convert a path to a URI") {
        CollectionsManager.pathToUri(
          List("g2", "art", "rhythm&blues")
        ) shouldBe "g2/art/rhythm%26blues"
      }
    }
    ...
    describe("create") {
      ...
      it("should lowercase pathId on creation") {
        val col = Collection.build(
          List("G2", "ArT", "CasECrazY"),
          ActionData("me@you.com", DateTime.now)
        )
    ...
    it("should only show the latest collection with same ID") {
      ...
      val collection1 =
        Collection.build(List("g2"), ActionData("me@you.com", date))
      val collection2 =
        Collection.build(List("g2"), ActionData("you@me.com", laterDate))
      val collection3 =
        Collection.build(List("g2"), ActionData("them@they.com", evenLaterDate))
    ...
    it("should find the index of a collection in a list") {
      ...
      val noIndex =
        CollectionsManager.findIndexes(List("not", "there"), collections)
      val multiIndex =
        CollectionsManager.findIndexes(List("observer", "feature"), collections)
