...
class ThrallMessageSenderTest extends FunSpec with Matchers {
  ...
  describe("json to message and back") {
    ...
    it(
      "should convert a message from an external source which does not have last modified"
    ) {
      val subject = "test"
    ...
    it(
      "should convert a message last modified with an offset timezone to UTC"
    ) {
