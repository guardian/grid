...
class UsageRightsTest extends FunSpec with Matchers {
  ...
  val invalidJson =
    Json.parse(s"""{ "category": "$invalidCategory", "fps": "∞" }""")
  ...
  it("should serialise to JSON correctly") {
    ...
    val usageRights: UsageRights =
      Agency(supplier, Some(suppliersCollection), restrictions = restrictions)
  ...
  it(
    "should through a `JsResultException` if you try to deserialise thr JSON with `as`"
  ) {
    ...
    jsError.errors.headOption.foreach { case (path, errors) =>
      errors.head.message should be(
        s"No such usage rights category: $invalidCategory"
      )
    }
  }
  ...
  it("should deserialise as a property of a case class") {
    ...
    (Json.toJson(agency) \ "usageRights" \ "supplier").as[String] should be(
      "Getty Images"
    )
  }
  ...
  it("should serialise as a property of a case class") {
    val noRightsJson =
      Json.parse("""{ "name": "Test Image", "usageRights": {} }""")
    val agencyJson = Json.parse(
      """{ "name": "Test Image", "usageRights": { "category": "agency", "supplier": "Getty Images" } }"""
    )
