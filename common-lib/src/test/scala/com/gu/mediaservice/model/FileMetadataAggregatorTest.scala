...
class FileMetadataAggregatorTest extends FlatSpec with Matchers {
  ...
  it should "organise flat metadata into aggregated form and preserve the order of properties if they are stored in arrays" in {
    ...
    val expected = Map(
      "schema:imageHasSubject" -> JsArray(
        Seq(
      ...
      "xmpMM:OriginalDocumentID" -> JsString(
        "xmp.did:65d63b5e-a24e-4e51-89bd-6693ce193404"
      ),
      ...
      "photoshop:DocumentAncestors" -> JsArray(
        Seq(
          "0",
      ...
      "xmpMM:DocumentID" -> JsString(
        "adobe:docid:photoshop:b55c9154-805d-a14a-a383-6b3945315d73"
      ),
      "tiff:Orientation" -> JsString("1"),
      "dc:creator" -> JsArray(Seq(JsString("tmp"))),
      "exif:ColorSpace" -> JsString("1"),
      "xmpMM:History" -> JsArray(
        Seq(
          JsArray(
            Seq(
      ...
      "xmpMM:InstanceID" -> JsString(
        "xmp.iid:d9500a13-3c27-401c-a2cc-1fd027b0424f"
      )
    )
