...
class MimeTypeTest extends FunSpec with Matchers {
  ...
  it(
    "should raise an UnsupportedMimeTypeException with an unsupported mime type"
  ) {
  ...
  it(
    "should raise an UnsupportedMimeTypeException when deserialising an unsupported mime type"
  ) {
    an[UnsupportedMimeTypeException] should be thrownBy JsString("audio/mp3")
      .as[MimeType]
  }
}
