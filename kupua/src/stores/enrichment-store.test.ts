...
beforeEach(() => {
  ...
  useEnrichmentStore.setState({ data: new Map() });
});

describe("enrichment-store", () => {
  it("initial state has an empty data map", () => {
    const { data } = useEnrichmentStore.getState();
  ...
  it("API cost overwrites ES baseline (merge direction test)", () => {
    ...
    const result = useEnrichmentStore.getState().data.get("img-4");
