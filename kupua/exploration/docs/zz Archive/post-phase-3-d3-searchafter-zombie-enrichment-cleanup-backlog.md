...
| `dal/grid-api/service-discovery.ts` | DELETE | **Kept** | Constructor dependency of `GridApiDataSource`. The gap-closure roadmap (`../03 Ce n'est pas une pipe dream/media-api-work/media-api-01-capability-inventory.md`) adds more methods to `GridApiDataSource`, and satellite service proxies (leases, usages, collections) will need URL helpers here. Deleting now would require changing the constructor, then recreating nearly the same class. Not dead code — dormant but near-future. |
