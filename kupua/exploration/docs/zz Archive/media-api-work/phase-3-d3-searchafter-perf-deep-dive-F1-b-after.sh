#!/usr/bin/env zsh
# B-after — TEST load generator, AFTER the gzip patch deployed.
#
# Identical structure to B-before. UNTIL is hardcoded to the SAME value used
# in B-before so the corpus is frozen and the only variable is the patch.
#
# Prereq:  - Gzip patch deployed to TEST via Riff-Raff.
#          - Kahuna browser tabs CLOSED (no background polling noise).
#          - GRID_COOKIE_TEST env var still set.
#          - Run from repo root.
#
# Run:     zsh kupua/exploration/docs/03\ Ce\ n\'est\ pas\ une\ pipe\ dream/media-api-work/phase-3-d3-searchafter-perf-deep-dive-F1-b-after.sh
# Safety:  read-only GET against TEST only. Bodies to /dev/null (signed URLs).

set -euo pipefail

if [[ -z "${GRID_COOKIE_TEST:-}" ]]; then
  echo "ERROR: GRID_COOKIE_TEST env var not set."
  echo "  export GRID_COOKIE_TEST='gutoolsAuth-assym=PASTE_TEST_VALUE_HERE'"
  exit 1
fi

MEDIA_API="https://api.media.test.dev-gutools.co.uk"

# PINNED — must be identical to B-before for valid comparison.
UNTIL_RAW="2026-06-27T16:41:07.000Z"
UNTIL_ENC="2026-06-27T16%3A41%3A07.000Z"

REPS=50
PAUSE=15

echo "╔══════════════════════════════════════════════════════╗"
echo "║  B-after — TEST load (WITH gzip patch)              ║"
echo "╚══════════════════════════════════════════════════════╝"
echo ""
echo "  UNTIL (pinned, same as B-before) = $UNTIL_RAW"
echo ""

# Confirm cookie
HTTP=$(curl -s -o /dev/null -w '%{http_code}' --max-time 10 \
  -H "Cookie: $GRID_COOKIE_TEST" \
  "${MEDIA_API}/images?q=&length=1&orderBy=-uploadTime&free=true&countAll=true")
if [[ "$HTTP" != "200" ]]; then
  echo "ERROR: got HTTP $HTTP — cookie may have expired."
  exit 1
fi
echo "  Cookie OK (HTTP 200)."
echo ""

typeset -A BATCH_START BATCH_END

printf '%-6s  %-26s  %-26s  %-16s\n' "len" "batch_start (UTC)" "batch_end (UTC)" "tTotal_s(med)"
printf '%-6s  %-26s  %-26s  %-16s\n' "------" "--------------------------" "--------------------------" "----------------"

for LEN in 1 50 200; do
  URL="${MEDIA_API}/images?q=&offset=0&length=${LEN}&orderBy=-uploadTime&until=${UNTIL_ENC}&free=true&countAll=false"

  START_TS=$(date -u +%Y-%m-%dT%H:%M:%S.000Z)
  BATCH_START[$LEN]=$START_TS

  T_MED=$(for i in $(seq $REPS); do
    curl -s -o /dev/null -w '%{time_total}\n' \
      --max-time 30 \
      -H "Cookie: $GRID_COOKIE_TEST" \
      "$URL"
  done | sort -n | awk '{a[NR]=$1} END{print a[int(NR/2)]}')

  END_TS=$(date -u +%Y-%m-%dT%H:%M:%S.000Z)
  BATCH_END[$LEN]=$END_TS

  printf '%-6s  %-26s  %-26s  %-16s\n' "$LEN" "$START_TS" "$END_TS" "$T_MED"

  if [[ "$LEN" != "200" ]]; then
    echo "  (pausing ${PAUSE}s...)"
    sleep $PAUSE
  fi
done

echo ""
echo "══════════════════════════════════════════════════════"
echo "  KIBANA LOOKUP — same KQL as B-before:"
echo '    stage:TEST AND stack:media-service AND message:"image search - query returned successfully"'
echo ""
for LEN in 1 50 200; do
  echo "  len=$LEN  →  $BATCH_START[$LEN]  to  $BATCH_END[$LEN]"
done
echo ""
echo "══════════════════════════════════════════════════════"
echo "  Done. Read Kibana duration p50/p95/p99 per batch window."
echo "  Record as B-after. Compare with B-before."
