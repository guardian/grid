...
|---|---|---|---|---|---|---|---|---|
| 1 | ✅ `normalizeCql` — carry-over from prior audit | [src/dal/adapters/elasticsearch/cql.ts:506](../../src/dal/adapters/elasticsearch/cql.ts) | D1 | ~15 | **High** | `tsc --noEmit` TS6133 at line 506: "declared but never read." `grep -r 'normalizeCql' kupua/src` → definition only. | None. | **Done 2026-05-22.** 856 unit + 239 e2e tests pass. |
| 2 | ✅ `getTunable` — carry-over from prior audit | [src/lib/image-prefetch.ts:144](../../src/lib/image-prefetch.ts) | D1 | ~10 | **High** | `tsc --noEmit` TS6133 at line 144. `grep -r 'getTunable' kupua/src` → definition only. | None. | **Done 2026-05-22.** |
| 3 | ✅ `unregisterShortcut` — carry-over | [src/lib/keyboard-shortcuts.ts:70](../../src/lib/keyboard-shortcuts.ts) | D1 | ~10 | **High** | `tsc --noEmit` TS6133 at line 70. `grep -r 'unregisterShortcut' kupua/src` → definition + one doc comment. No import statement. | None. | **Done 2026-05-22.** |
| 4 | ✅ `getPrefetchLog` — carry-over | [src/lib/image-prefetch.ts:217](../../src/lib/image-prefetch.ts) | D1 | ~7 | **High** | `tsc --noEmit` TS6133 at line 217. `grep -r 'getPrefetchLog' kupua/src` → definition only. | None — ring buffer also cleaned up per Appendix A item 1 (option B). | **Done 2026-05-22.** See Appendix A item 1. |
| 5 | ✅ `fullImagesEnabled` — carry-over | [src/lib/image-urls.ts:282](../../src/lib/image-urls.ts) | D1 | ~5 | **High** | `tsc --noEmit` TS6133 at line 282. `grep -r 'fullImagesEnabled' kupua/src` → definition only. | None. | **Done 2026-05-22.** |
...
| 26 | ⊘ `ToastLifespan` — export-only noise after #20 | [src/stores/toast-store.ts:45](../../src/stores/toast-store.ts) | D1 | 1 (contingent on #20) | **High** | With #20 done, no external file imports `ToastLifespan`. However, the type is still used internally (`ToastItem.lifespan: ToastLifespan` at line 53) and documents the BBC vocabulary alignment for Phase 3. Removing just the `export` keyword falls under the reviewer's "export-keyword noise" exclusion. | N/A — not actioned, same category as removed findings #6–10 etc. | No action needed. |
...
| Toast cleanup | 20, 26 | Delete `useToast.ts` (#20) first; that makes `ToastLifespan` in `toast-store.ts` dead (#26). Two commits or one. | **Done.** #20 executed (file deleted). #26 reclassified as export-keyword noise — `ToastLifespan` still used internally and documents BBC vocabulary alignment. |
...
1. ✅ **`_prefetchLog` ring buffer** (`image-prefetch.ts`) — **Done 2026-05-22, option B.** `PrefetchLogEntry` interface, `PREFETCH_LOG_CAP` const, `_prefetchLog` array, ring-buffer push/trim code inside `prefetchLog`, and `_prefetchLog.length = 0` in `__resetPrefetchForTests` all deleted. `prefetchLog` function retained as a pure `console.debug` wrapper so dev-mode debug output is preserved. Net reclaim ~20 LOC.
