...
**Coupling fix plan** (`coupling-fix-handoff.md`) — progress: Phase 0 ✅, Phase A ✅, Phase B ✅, Phase C ✅

**Next work: focus drift + P8 scroll** — see `exploration/docs/focus-drift-and-scroll-handoff.md` for the full self-contained handoff. Two items:
1. Fix P6 focusDrift=428px (sort-around-focus re-centres instead of preserving position) and P4b focusDrift=−160px (table→grid restore overshoots). Add P17 test (density switch from deep position).
2. Investigate P8 domChurn=~57k — approaches documented in handoff; likely reducible 15–25% but structural root cause cannot be eliminated without skeleton rows.

- [ ] **Phase D refinements** (lower priority, each independent): D.1 density-focus `saved` ref into parent `useRef` (concurrent mode correctness — assessed as low-risk to skip; `ImageTable`/`ImageGrid` never co-render and the module-level pattern is provably safe for this use case); D.2 font strings from CSS custom properties (C5 — low value, constants are stable compile-time values); D.3 viewport-aware panel constraints (C10 — requires design discussion before implementing).
...
- **P6 focusDrift=428px / 0.41 ratio after sort change.** Sort-around-focus places the focused item nearly half a viewport below its pre-sort position. focusVisible=true (on screen) but positioning is significantly off. Most likely cause of the "cell reordering" the user observed. Investigate `_findAndFocusImage()` scroll offset calculation.

- **P4b focusDrift=−160px after table→grid density switch.** Item appears ~5 table rows above expected position. P4a (grid→table) is perfect (0px drift). Investigate `density-focus.ts` ratio/localIndex restore path.

- **Focus drift harness bug fixed.** `focusDriftPx`/`focusDriftRatio`/`focusVisible` were always recorded in `.metrics-tmp.jsonl` but stripped by `aggregateMetrics()` before writing to `audit-log.json`. Fixed in `run-audit.mjs` — future runs will persist these fields.

- **P7 LoAF regression uncertain.** Phase A/B win (133→47ms) appears gone in Phase C re-run (138ms). Single-run; needs `--runs 3` to distinguish real regression from tunnel variance.

- **Phase C perf run incomplete** — P4a/P4b/P6/P7/P9 timed out (ES tunnel instability) in the first Phase C run. Superseded by "Phase C re-run (full)" entry (31 tests). See `exploration/docs/perf-measurement-report.md`.
...
kupua/
  ...
  src/                         # ~16,300 lines total
    ...
    lib/
      ...
      density-focus.ts         # Transient bridge for viewport-position preservation across density switches — saves ratio + localIndex (5 lines)
      scroll-container-ref.ts  # Module-level ref for the active scroll container — registerScrollContainer(el) called by density components on mount/unmount; getScrollContainer() used by Scrubber + scroll-reset. Same pattern as density-focus.ts.
