...
**Scroll effects** (`useScrollEffects.ts`, ~700 lines): Shared hook for all scroll lifecycle — parameterised by `ScrollGeometry` descriptor. Handles: scroll reset orchestration, prepend/forward-evict compensation, seek scroll-to-target (effect #6: reverse-compute + lastVisibleRow buffer-shrink preservation), sort-around-focus scroll, density-focus save/restore (with edge clamping), bufferOffset→0 guard. Post-seek backward extend suppression (`_postSeekBackwardSuppress` flag in `useDataWindow.ts`) blocks `extendBackward` until user scrolls past `EXTEND_THRESHOLD`, preventing swimming. Seek timing constants in `tuning.ts`: `SEEK_COOLDOWN_MS` (200ms, post-arrival extend block), `SEEK_DEFERRED_SCROLL_MS` (derived: cooldown + 100ms, fires synthetic scroll to trigger extends), `SEARCH_FETCH_COOLDOWN_MS` (2000ms, blocks extends during in-flight search/abort). Module-level bridges for density-focus and sort-focus.
...
**Testing:** 186 Vitest unit/integration tests (~5s) — includes 7 null-zone seek/extend tests + 3 total-corruption regression tests using sparse `MockDataSource` (50k images, 20% `lastModified` coverage). 80 Playwright E2E tests (~70s) — scrubber (including flash-prevention golden table with **0px scroll-drift tolerance**), buffer corruption regression suite, density-focus drift, visual baselines. Safety gate refuses real ES. 19 perf tests + experiment infrastructure (16 scenarios). Corpus pinned via `PERF_STABLE_UNTIL`. 11 smoke tests for TEST cluster (`manual-smoke-test.spec.ts`). 10 scroll stability smoke tests (`smoke-scroll-stability.spec.ts`, S12-S21) — write structured JSON report to `test-results/scroll-stability-report.json` for agent consumption; all passed on real data (run 5/6). **Logging pattern:** all diagnostic `console.log` calls use `devLog()` from `src/lib/dev-log.ts` — a thin wrapper guarded by `import.meta.env.DEV` so Vite DCEs them in prod. E2E tests can still read these messages via `KupuaHelpers.getConsoleLogs()` because Playwright runs against the dev server. Use `devLog` for all new diagnostic logging; reserve bare `console.warn` for genuine error paths only.
...
- [x] **Scroll stability on real data** — ✅ ALL FIXED (agent 6). Flash, position preservation, swimming, End key, buffer-bottom freeze. See `exploration/docs/worklog-stale-cells-bug.md`.
