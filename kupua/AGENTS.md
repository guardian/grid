...
| Obscure sorting decision | `exploration/docs/scroll-and-position-preservation-testing-4.2.1-obscure-sorting-decision.md` | One-semantic-sort containment, exact-boundary/approximate-evidence scrubber contract, and exact special-date position maps; I-J remain. |
...
- **Special-date sort correctness** — execute slices I-J of the obscure sorting workplan, then D/J media-api parity. Last used and Added to collection now share maximum-date sorting, an exact populated/null boundary, and exact position maps independent of G's approximate histogram evidence. `countBefore`, range selection and final cross-boundary acceptance still need parity. Materialised scalar dates require a separate backend proposal and performance review.