...
**Scroll effects** (`useScrollEffects.ts`, ~700 lines): Shared hook for all scroll lifecycle — parameterised by `ScrollGeometry` descriptor. Handles: scroll reset orchestration, prepend/forward-evict compensation, seek scroll-to-target (effect #6: reverse-compute + lastVisibleRow buffer-shrink preservation), sort-around-focus scroll, density-focus save/restore (with edge clamping), bufferOffset→0 guard. Seek timing constants in `tuning.ts`: `SEEK_COOLDOWN_MS` (700ms, post-arrival extend block), `SEEK_DEFERRED_SCROLL_MS` (derived: cooldown + 100ms = 800ms, fires synthetic scroll to trigger extends), `SEARCH_FETCH_COOLDOWN_MS` (2000ms, blocks extends during in-flight search/abort). Post-extend cooldown: each `extendBackward` completion sets a 200ms cooldown to prevent cascading prepend compensations (swimming). The old `_postSeekBackwardSuppress` flag in `useDataWindow.ts` has been removed — it prevented swimming but also prevented scrolling up after seek. Module-level bridges for density-focus and sort-focus.
...
**Testing:** 186 Vitest unit/integration tests (~5s) — includes 7 null-zone seek/extend tests + 3 total-corruption regression tests using sparse `MockDataSource` (50k images, 20% `lastModified` coverage). 83 Playwright E2E tests (~70s) — scrubber (including flash-prevention golden table with **0px scroll-drift tolerance**, scroll-up-after-seek grid+table, settle-window visible content stability), buffer corruption regression suite, density-focus drift, visual baselines. Safety gate refuses real ES. 19 perf tests + experiment infrastructure (16 scenarios). Corpus pinned via `PERF_STABLE_UNTIL`. 11 smoke tests for TEST cluster (`manual-smoke-test.spec.ts`). 23 scroll stability smoke tests (`smoke-scroll-stability.spec.ts`, S1-S23) — write structured JSON report to `test-results/scroll-stability-report.json` for agent consumption; all 23 passed on real data (1.3M docs). **Logging pattern:** all diagnostic `console.log` calls use `devLog()` from `src/lib/dev-log.ts` — a thin wrapper guarded by `import.meta.env.DEV` so Vite DCEs them in prod. E2E tests can still read these messages via `KupuaHelpers.getConsoleLogs()` because Playwright runs against the dev server. Use `devLog` for all new diagnostic logging; reserve bare `console.warn` for genuine error paths only.
...
- [x] **Scroll stability on real data** — ✅ ALL FIXED (agents 6+10). Flash, position preservation, swimming, End key, buffer-bottom freeze, scroll-up after seek. 1% cosmetic: tiny ~3-image shift on first `extendBackward` after seek (see `scroll-work-worklog-agent10-final-fix.md` for 5 mitigation ideas, Idea A most promising).
