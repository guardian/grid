...
- [x] **Scroll stability on real data** — ✅ ALL FIXED (agents 6+10+11-14). Flash, position preservation, swimming, End key, buffer-bottom freeze, scroll-up after seek. Bidirectional seek (Idea B) eliminates the ~1% cosmetic swim: deep seek paths now fetch backward items in addition to the forward fetch, placing the user in the buffer middle so extends operate on off-screen content. Position preserved across the entire headroom zone (including the fractional boundary row where `backwardItemCount/cols` is not an integer) via `_seekSubRowOffset` in the store + effect #6 application. Settle-window content shift: 0 items (was COLS+1). All 24 smoke tests pass on TEST (1.3M docs).
...
8. **Windowed buffer + Scrubber** — fixed-capacity buffer (max 1000) with `bufferOffset`. `search_after` + PIT for cursor-based pagination. Extend at edges, evict to keep memory bounded. Seek: shallow (<10k) via `from/size`, deep (≥10k) via percentile estimation + `search_after` + `countBefore`. **Bidirectional seek:** deep seek paths add a backward `search_after` (halfBuffer items) after the forward fetch, placing the user in the buffer middle (~100 items headroom above). Eliminates post-seek swimming by construction — extends operate on off-screen content. Native scrollbar hidden; Scrubber is sole visible scrollbar with two modes (scroll/seek). Full design: `search-after-plan.md`.
...
kupua/
  ...
  scripts/
    ...
    run-smoke.mjs             # Smoke test runner (read-only, against TEST cluster)
    read-results.py           # Read smoke test JSON report: python3 scripts/read-results.py
