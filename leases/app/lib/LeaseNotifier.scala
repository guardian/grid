...
class LeaseNotifier(config: LeasesConfig, store: LeaseStore)
    extends ThrallMessageSender(config.thrallKinesisStreamConfig) {
  ...
  def sendAddLease(mediaLease: MediaLease) = {
    ...
    val updateMessage = UpdateMessage(
      subject = addImageLease,
      mediaLease = Some(mediaLease),
      id = Some(mediaLease.mediaId)
    )
    publish(updateMessage)
  }
