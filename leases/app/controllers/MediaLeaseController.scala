...
class MediaLeaseController(
    auth: Authentication,
    store: LeaseStore,
    config: LeasesConfig,
    notifications: LeaseNotifier,
  ...
  private def clearLease(id: String) = store.get(id).map { lease =>
    store.delete(id).map { _ =>
      notifications.sendRemoveLease(lease.mediaId, id)
    }
  }

  private def clearLeases(id: String) = Future.sequence(
    store
      .getForMedia(id)
      .flatMap(_.id)
      .flatten(clearLease)
  ...
  private def badRequest(e: Seq[(JsPath, Seq[JsonValidationError])]) =
    respondError(
      BadRequest,
      "media-leases-parse-failed",
      JsError.toJson(e).toString
    )

  private def prepareLeaseForSave(
      mediaLease: MediaLease,
      userId: Option[String]
  ): MediaLease =
    mediaLease.prepareForSave.copy(
  ...
  private def addLease(mediaLease: MediaLease, userId: Option[String]) = {
    ...
    if (lease.isSyndication) {
      ...
      replaceLeases(
        leasesWithoutSyndication :+ lease,
        mediaLease.mediaId,
        userId
      )
    } else {
  ...
  private def replaceLeases(
      mediaLeases: List[MediaLease],
      imageId: String,
      userId: Option[String]
  ) = {
  ...
  def postLease = auth.async(parse.json) { implicit request =>
    request.body.validate[MediaLease] match {
      case JsSuccess(mediaLease, _) =>
        addLease(mediaLease, Some(Authentication.getIdentity(request.user)))
          .map(_ => Accepted)
  ...
  def getLease(id: String) = auth.async { _ =>
    Future {
      ...
      leases.foldLeft(notFound)((_, lease) =>
        respond[MediaLease](
  ...
  def validateLeases(leases: List[MediaLease]) = leases.count {
    _.isSyndication
  } <= 1

  def replaceLeasesForMedia(id: String) = auth.async(parse.json) {
    implicit request =>
      Future {
        request.body
          .validate[List[MediaLease]]
          .fold(
            badRequest,
            mediaLeases => {
              if (validateLeases(mediaLeases)) {
                replaceLeases(
                  mediaLeases,
                  id,
                  Some(Authentication.getIdentity(request.user))
                )
                Accepted
              } else {
                respondError(
                  BadRequest,
                  "validation-error",
                  "No more than one syndication lease per image"
                )
              }
            }
          )
