...
class ParserTest
  ...
  describe("text") {
    ...
    it("should match single terms with curly apostrophe") {
      Parser.run("l’apostrophe") should be(
        List(Match(AnyField, Words("l’apostrophe")))
      )
    }
    ...
    it("should match multiple terms") {
      Parser.run("cats dogs") should be(
        List(Match(AnyField, Words("cats dogs")))
      )
    }
    ...
    it("should match multiple terms including 'in'") {
      Parser.run("cats in dogs") should be(
        List(Match(AnyField, Words("cats in dogs")))
      )
    }

    it("should match multiple terms including 'by'") {
      Parser.run("cats by dogs") should be(
        List(Match(AnyField, Words("cats by dogs")))
      )
    }

    it("should match multiple terms including apostrophes") {
      Parser.run("it's a cat") should be(
        List(Match(AnyField, Words("it's a cat")))
      )
    }
    ...
    it("should match a quoted phrase") {
      Parser.run(""""cats dogs"""") should be(
        List(Match(AnyField, Phrase("cats dogs")))
      )
    }

    it("should match faceted terms") {
      Parser.run("credit:cats") should be(
        List(Match(creditField, Words("cats")))
      )
    }

    it("should match multiple faceted terms on the same facet") {
      Parser.run("label:cats label:dogs") should be(
        List(
    ...
    it("should match multiple faceted terms on different facets") {
      Parser.run("credit:cats label:dogs") should be(
        List(
  ...
  describe("date") {
    ...
    describe("exact") {
      ...
      it("should match year") {
        Parser.run("date:2014") should be(
          List(
            Match(
              uploadTimeField,
              DateRange(
      ...
      it("should match month, quoted") {
        Parser.run("""date:"january 2014"""") should be(
          List(
            Match(
              uploadTimeField,
              DateRange(
      ...
      it("should match human date, quoted") {
        Parser.run("""date:"1 january 2014"""") should be(
          List(
            Match(
              uploadTimeField,
              DateRange(
      ...
      it("should match date with slashes") {
        Parser.run("date:1/1/2014") should be(
          List(
            Match(
              uploadTimeField,
              DateRange(
      ...
      it("should match date with slashes and zero-padding") {
        Parser.run("date:01/01/2014") should be(
          List(
            Match(
              uploadTimeField,
              DateRange(
      ...
      it("should match date") {
        Parser.run("date:2014-01-01") should be(
          List(
            Match(
              uploadTimeField,
              DateRange(
    ...
    describe("relative") {
      ...
      it("should match today") {
        Parser.run("date:today") should be(
          List(
            Match(
              uploadTimeField,
              DateRange(
      ...
      it("should match yesterday") {
        Parser.run("date:yesterday") should be(
          List(
            Match(
              uploadTimeField,
              DateRange(
    ...
    describe("nested usage") {
      it("should match nested usage status query") {
        Parser.run("usages@status:pending") should be(
          List(
            Nested(
              SingleField("usages"),
      ...
      it("should match nested usage reference query") {
        Parser.run("usages@reference:foo") should be(
          List(
            Nested(
              SingleField("usages"),
              MultipleField(
      ...
      it("should match nested usage reference query with url supplied") {
        Parser.run("usages@reference:https://generic.cms/1234") should be(
          List(
            Nested(
              SingleField("usages"),
              MultipleField(
    ...
    describe("date constraint") {
      ...
      it("should match date constraint") {
        Parser.run("<date:2012-01-01") should be(
          List(
            Match(
              SingleField("uploadTime"),
              DateRange(
      ...
      describe("nested") {
        ...
        it("should match date constraint with parent field") {
          Parser.run("usages@<added:2012-01-01") should be(
            List(
              Nested(
                SingleField("usages"),
    ...
    describe("shortcut and facets") {
      ...
      it("should match '@' shortcut") {
        Parser.run("@2014-01-01") should be(
          List(
            Match(
              uploadTimeField,
              DateRange(
      ...
      it("should match uploaded facet term") {
        Parser.run("uploaded:2014-01-01") should be(
          List(
            Match(
              uploadTimeField,
              DateRange(
      ...
      it("should match taken facet term") {
        Parser.run("taken:2014-01-01") should be(
          List(
            Match(
              SingleField("dateTaken"),
              DateRange(
                new DateTime("2014-01-01T00:00:00.000Z"),
                new DateTime("2014-01-01T23:59:59.999Z")
              )
    ...
    describe("error") {
      ...
      it("should ignore an invalid date argument") {
        Parser.run("date:NAZGUL") should be(
          List(
  ...
  describe("negation") {
    ...
    it("should match negated single terms") {
      Parser.run("-cats") should be(
        List(Negation(Match(AnyField, Words("cats"))))
      )
    }

    it("should match negated quoted terms") {
      Parser.run("""-"cats dogs"""") should be(
        List(Negation(Match(AnyField, Phrase("cats dogs"))))
      )
    }
  ...
  describe("aliases") {
    ...
    it("should match aliases to multiple fields") {
      Parser.run("in:berlin") should be(
        List(
          Match(
            MultipleField(
  ...
  describe("combination") {
    ...
    it("should combination of terms (negated faceted word, word)") {
      Parser.run("""-credit:cats dogs""") should be(
        List(
    ...
    it("should combination of terms (negated faceted phrase, word)") {
      Parser.run("""-credit:"cats dogs" unicorns""") should be(
        List(
    ...
    it("should combination of terms (multiple words, label)") {
      Parser.run("""cats dogs #unicorns""") should be(
        List(
    ...
    it("should combination of terms (multiple words, label interleaved)") {
      Parser.run("""cats #unicorns dogs""") should be(
        List(
  ...
  describe("has filter") {
    it("should find images with crops") {
      Parser.run("has:crops") should be(
        List(
    ...
    it("should match multiple terms and the has query") {
      Parser.run("cats dogs has:rightsSyndication") should be(
        List(
          Match(AnyField, Words("cats dogs")),
    ...
    it("should match aliases and a has query") {
      Parser.run("by:cats has:paws") should be(
        List(
  ...
  describe("fileType filter") {
    it("should find jpegs images") {
      Parser.run("fileType:jpeg") should be(
        List(
    ...
    it("should find png images") {
      Parser.run("fileType:png") should be(
        List(
    ...
    it("should find tiff images when searching for file type 'tif'") {
      Parser.run("fileType:tif") should be(
        List(
    ...
    it("should find tiff images when searching for file type 'tiff'") {
      Parser.run("fileType:tiff") should be(
        List(
          Match(SingleField(getFieldPath("mimeType")), Words("image/tiff"))
        )
      )
    ...
    it("should match multiple terms and the fileType query") {
      Parser.run("fileType:tiff cats dogs") should be(
        List(
          Match(SingleField(getFieldPath("mimeType")), Words("image/tiff")),
          Match(AnyField, Words("cats dogs"))
        )
    ...
    it("should match aliases and a fileType query") {
      Parser.run("by:cats fileType:tiff") should be(
        List(
          Match(bylineField, Words("cats")),
          Match(SingleField(getFieldPath("mimeType")), Words("image/tiff"))
        )
    ...
    it("should not match unrelated file types") {
      Parser.run("fileType:catsdogs") should be(
        List(
