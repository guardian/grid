...
class ImageExtrasTest extends FunSpec with Matchers with MockitoSugar {
  ...
  val baseValidityMap = Map(
    "paid_image" -> ValidityCheck(
      invalid = true,
      overrideable = true,
      shouldOverride = false
    ),
    "missing_description" -> ValidityCheck(
      invalid = true,
      overrideable = false,
      shouldOverride = false
    ),
    "missing_credit" -> ValidityCheck(
      invalid = true,
      overrideable = false,
      shouldOverride = false
    ),
    "over_quota" -> ValidityCheck(
      invalid = false,
      overrideable = true,
      shouldOverride = false
    ),
    "current_deny_lease" -> ValidityCheck(
      invalid = false,
      overrideable = true,
      shouldOverride = false
    ),
    "no_rights" -> ValidityCheck(
      invalid = true,
      overrideable = true,
      shouldOverride = false
    ),
    "conditional_paid" -> ValidityCheck(
      invalid = false,
      overrideable = true,
      shouldOverride = false
    )
  ...
  describe("Invalid Images") {
    it("should generate validityMaps") {
      baseValidityMap should be(
        ImageExtras.validityMap(baseImage, withWritePermission = false)
      )
    }
    ...
    it("should report overriden validity") {
      ...
      val validityMap =
        ImageExtras.validityMap(overrideableImage, withWritePermission = true)
      val validity = ImageExtras.isValid(validityMap)
    ...
    it("should report invalid when fields cannot be overriden") {
      val validityMap =
        ImageExtras.validityMap(baseImage, withWritePermission = true)
      val validity = ImageExtras.isValid(validityMap)
