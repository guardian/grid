...
trait ConditionFixtures {
  ...
  val anotherFieldPhraseMatchCondition =
    Match(SingleField("anotherfield"), Phrase("anothervalue"))

  val dateRangeStart: DateTime =
    new DateTime(2016, 1, 1, 0, 0).withZone(DateTimeZone.UTC)
  val dateRangeEnd: DateTime = dateRangeStart.plusHours(1)
  val dateMatchCondition =
    Match(SingleField("adatefield"), DateRange(dateRangeStart, dateRangeEnd))

  val hasFieldCondition = Match(HasField, HasValue("foo"))

  val isOwnedPhotoCondition =
    Match(IsField, IsValue(IsOwnedPhotograph.toString))
  val isOwnedIllustrationCondition =
    Match(IsField, IsValue(IsOwnedIllustration.toString))
  val isOwnedImageCondition = Match(IsField, IsValue(IsOwnedImage.toString))
  val isUnderQuotaCondition =
    Match(IsField, IsValue(IsUnderQuota(Nil).toString))
  ...
  val multipleFieldWordsCondition =
    Match(MultipleField(List("foo", "bar")), Phrase("cats and dogs"))

  val nestedCondition: Condition = Nested(
    SingleField("usages"),
    SingleField("usages.status"),
    Words("pending")
  )
  val anotherNestedCondition: Condition = Nested(
    SingleField("something"),
    SingleField("something.field"),
    Phrase("dogs")
  )
