...
trait Fixtures {
  ...
  def createImageUploadedInThePast(id: String): Image =
    createImage(id = id, Handout()).copy(
  ...
  def createPrintUsage(date: DateTime = DateTime.now): Usage =
    createUsage(InDesignUsageReference, PrintUsage, PendingUsageStatus, date)
  ...
  private def createUsage(
      t: UsageReferenceType,
      usageType: UsageType,
      status: Status,
      date: DateTime
  ): Usage = {
