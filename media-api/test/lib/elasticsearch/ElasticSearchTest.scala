...
class ElasticSearchTest extends ElasticSearchTestBase with Eventually with ElasticSearchExecutions with MockitoSugar {
  ...
  describe("has field filter") {
    it("can filter images which have a specific field") {
      ...
      val hasTitleSearch = SearchParams(tier = Internal, structuredQuery = List(SubQuery(List(hasTitleCondition))))
      whenReady(ES.search(hasTitleSearch), timeout, interval) { result =>
        result.total shouldBe expectedNumberOfImages
      }

      val hasUnknownFieldTitleSearch = SearchParams(tier = Internal, structuredQuery = List(SubQuery(List(unknownFieldCondition))))
    ...
    it("should be able to filter images with fileMetadata even though fileMetadata fields are not indexed") {
      ...
      val hasFileMetadataSearch = SearchParams(tier = Internal, structuredQuery = List(SubQuery(List(hasFileMetadataCondition))))
  ...
  describe("is field filter") {
    it("should return no images with an invalid search") {
      val search = SearchParams(tier = Internal, structuredQuery = List(SubQuery(List(isInvalidCondition))))
    ...
    it("should return owned photographs") {
      val search = SearchParams(tier = Internal, structuredQuery = List(SubQuery(List(isOwnedPhotoCondition))), length = 50)
      whenReady(ES.search(search), timeout, interval) { result => {
        val expected = List(
          "iron-suit",
          "green-leaf",
    ...
    it("should return owned illustrations") {
      val search = SearchParams(tier = Internal, structuredQuery = List(SubQuery(List(isOwnedIllustrationCondition))))
      whenReady(ES.search(search), timeout, interval) { result => {
        val expected = List(
          "green-giant",
    ...
    it("should return all owned images") {
      val search = SearchParams(tier = Internal, structuredQuery = List(SubQuery(List(isOwnedImageCondition))), length = 50)
      whenReady(ES.search(search), timeout, interval) { result => {
        val expected = List(
          "iron-suit",
          "green-leaf",
    ...
    it("should return all images when no agencies are over quota") {
      val search = SearchParams(tier = Internal, structuredQuery = List(SubQuery(List(isUnderQuotaCondition))))
    ...
    it("should return any image whose agency is not over quota") {
      ...
      val search = SearchParams(tier = Internal, structuredQuery = List(SubQuery(List(isUnderQuotaCondition))), length = 50)
