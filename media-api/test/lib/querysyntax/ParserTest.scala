...
class ParserTest extends FunSpec with Matchers with BeforeAndAfter with ImageFields {
  ...
  describe("text") {
    it("should match single terms") {
      Parser.run("cats") should be (List(SubQuery(List(Match(AnyField, Words("cats")), Negation(Match(IsField,IsValue("deleted")))))))
    }

    it("should match single terms with accents") {
      Parser.run("séb") should be (List(SubQuery(List(Match(AnyField,Words("séb")), Negation(Match(IsField,IsValue("deleted")))))))
    }
    it("should match single terms with curly apostrophe") {
      Parser.run("l’apostrophe") should be (List(SubQuery(List(Match(AnyField, Words("l’apostrophe")), Negation(Match(IsField,IsValue("deleted")))))))
    }
    ...
    it("should match multiple terms") {
      Parser.run("cats dogs") should be (List(SubQuery(List(Match(AnyField, Words("cats dogs")), Negation(Match(IsField,IsValue("deleted")))))))
    }
    ...
    it("should match multiple terms including 'in'") {
      Parser.run("cats in dogs") should be (List(SubQuery(List(Match(AnyField, Words("cats in dogs")), Negation(Match(IsField,IsValue("deleted")))))))
    }

    it("should match multiple terms including 'by'") {
      Parser.run("cats by dogs") should be (List(SubQuery(List(Match(AnyField, Words("cats by dogs")), Negation(Match(IsField,IsValue("deleted")))))))
    }

    it("should match multiple terms including apostrophes") {
      Parser.run("it's a cat") should be (List(SubQuery(List(Match(AnyField, Words("it's a cat")), Negation(Match(IsField,IsValue("deleted")))))))
    }
    ...
    it("should match multiple terms including single double quotes") {
      Parser.run("5\" cats") should be (List(SubQuery(List(Match(AnyField, Words("5\" cats")), Negation(Match(IsField,IsValue("deleted")))))))
    }
    ...
    it("should match a quoted phrase") {
      Parser.run(""""cats dogs"""") should be (List(SubQuery(List(Match(AnyField, Phrase("cats dogs")), Negation(Match(IsField,IsValue("deleted")))))))
    }

    it("should match faceted terms") {
      Parser.run("credit:cats") should be (List(SubQuery(List(Match(creditField, Words("cats")), Negation(Match(IsField,IsValue("deleted")))))))
    }

    it("should match multiple faceted terms on the same facet") {
      Parser.run("label:cats label:dogs") should be (List(SubQuery(List(
    ...
    it("should match multiple faceted terms on different facets") {
      Parser.run("credit:cats label:dogs") should be (List(SubQuery(List(
  ...
  describe("date") {
    ...
    describe("exact") {
      ...
      it("should match year") {
        Parser.run("date:2014") should be (List(SubQuery(List(
      ...
      it("should match month, quoted") {
        Parser.run("""date:"january 2014"""") should be (List(SubQuery(List(
          Match(uploadTimeField,
            DateRange(
              new DateTime("2014-01-01T00:00:00.000Z"),
      ...
      it("should match human date, quoted") {
        Parser.run("""date:"1 january 2014"""") should be (List(SubQuery(List(
          Match(uploadTimeField,
            DateRange(
              new DateTime("2014-01-01T00:00:00.000Z"),
      ...
      it("should match date with slashes") {
        Parser.run("date:1/1/2014") should be (List(SubQuery(List(
          Match(uploadTimeField,
            DateRange(
              new DateTime("2014-01-01T00:00:00.000Z"),
              new DateTime("2014-01-01T23:59:59.999Z")
      ...
      it("should match date with slashes and zero-padding") {
        Parser.run("date:01/01/2014") should be (List(SubQuery(List(
          Match(uploadTimeField,
            DateRange(
              new DateTime("2014-01-01T00:00:00.000Z"),
              new DateTime("2014-01-01T23:59:59.999Z")
      ...
      it("should match date") {
        Parser.run("date:2014-01-01") should be (List(SubQuery(List(
          Match(uploadTimeField,
            DateRange(
              new DateTime("2014-01-01T00:00:00.000Z"),
              new DateTime("2014-01-01T23:59:59.999Z")
    ...
    describe("relative") {
      ...
      it("should match today") {
        Parser.run("date:today") should be (List(SubQuery(List(
          Match(uploadTimeField,
            DateRange(
      ...
      it("should match yesterday") {
        Parser.run("date:yesterday") should be (List(SubQuery(List(
          Match(uploadTimeField,
            DateRange(
    ...
    describe("nested usage") {
      it("should match nested usage status query") {
        Parser.run("usages@status:pending") should be (List(SubQuery(List(
          Nested(
            SingleField("usages"),
      ...
      it("should match nested usage reference query") {
        Parser.run("usages@reference:foo") should be (List(SubQuery(List(
          Nested(
            SingleField("usages"),
            MultipleField(List(
      ...
      it("should match nested usage reference query with url supplied") {
        Parser.run("usages@reference:https://generic.cms/1234") should be (List(SubQuery(List(
          Nested(
            SingleField("usages"),
            MultipleField(List(
              "usages.references.uri",
    ...
    describe("date constraint") {
      ...
      it("should match date constraint") {
        Parser.run("<date:2012-01-01") should be (List(SubQuery(List(
      ...
      describe("nested") {
        ...
        it("should match date constraint with parent field") {
          Parser.run("usages@<added:2012-01-01") should be (List(SubQuery(List(
    ...
    describe("shortcut and facets") {
      ...
      it("should match '@' shortcut") {
        Parser.run("@2014-01-01") should be (List(SubQuery(List(
          Match(uploadTimeField,
            DateRange(
              new DateTime("2014-01-01T00:00:00.000Z"),
              new DateTime("2014-01-01T23:59:59.999Z")
      ...
      it("should match uploaded facet term") {
        Parser.run("uploaded:2014-01-01") should be (List(SubQuery(List(
          Match(uploadTimeField,
            DateRange(
              new DateTime("2014-01-01T00:00:00.000Z"),
              new DateTime("2014-01-01T23:59:59.999Z")
      ...
      it("should match taken facet term") {
        Parser.run("taken:2014-01-01") should be (List(SubQuery(List(
    ...
    describe("error") {
      ...
      it("should ignore an invalid date argument") {
        Parser.run("date:NAZGUL") should be (List(SubQuery(List(
  ...
  describe("negation") {
    ...
    it("should match negated single terms") {
      Parser.run("-cats") should be (List(SubQuery(List(Negation(Match(AnyField, Words("cats"))), Negation(Match(IsField,IsValue("deleted")))))))
    }

    it("should match negated quoted terms") {
      Parser.run("""-"cats dogs"""") should be (List(SubQuery(List(Negation(Match(AnyField, Phrase("cats dogs"))), Negation(Match(IsField,IsValue("deleted")))))))
    }
  ...
  describe("aliases") {
    ...
    it("should match aliases to a single canonical field") {
      Parser.run("by:cats") should be (List(SubQuery(List(Match(bylineField, Words("cats")), Negation(Match(IsField,IsValue("deleted")))))))
    }

    it("should match aliases to multiple fields") {
      Parser.run("in:berlin") should be (List(SubQuery(List(Match(MultipleField(List("subLocation", "city", "state", "country").map(getFieldPath)),
        Words("berlin")), Negation(Match(IsField,IsValue("deleted")))))))
    }

    it("should match #terms as labels") {
      Parser.run("#cats") should be (List(SubQuery(List(Match(labelsField, Words("cats")), Negation(Match(IsField,IsValue("deleted")))))))
    }
  ...
  describe("combination") {
    ...
    it("should combination of terms (negated faceted word, word)") {
      Parser.run("""-credit:cats dogs""") should be (List(SubQuery(List(
    ...
    it("should combination of terms (negated faceted phrase, word)") {
      Parser.run("""-credit:"cats dogs" unicorns""") should be (List(SubQuery(List(
    ...
    it("should combination of terms (multiple words, label)") {
      Parser.run("""cats dogs #unicorns""") should be (List(SubQuery(List(
    ...
    it("should combination of terms (multiple words, label interleaved)") {
      Parser.run("""cats #unicorns dogs""") should be (List(SubQuery(List(
    ...
    it("should combination of terms (negated word, word)") {
      Parser.run("""-cats dogs""") should be (List(SubQuery(List(
  ...
  describe("has filter") {
    it("should find images with crops") {
      Parser.run("has:crops") should be (List(SubQuery(List(
    ...
    it("should match multiple terms and the has query") {
      Parser.run("cats dogs has:rightsSyndication") should be (List(SubQuery(List(
        Match(AnyField, Words("cats dogs")),
    ...
    it("should match negated has queries") {
      Parser.run("-has:foo") should be (List(SubQuery(List(
    ...
    it("should match aliases and a has query") {
      Parser.run("by:cats has:paws") should be (List(SubQuery(List(
  ...
  describe("fileType filter") {
    it("should find jpegs images") {
      Parser.run("fileType:jpeg") should be (List(SubQuery(List(
    ...
    it("should find png images") {
      Parser.run("fileType:png") should be (List(SubQuery(List(
    ...
    it("should find tiff images when searching for file type 'tif'") {
      Parser.run("fileType:tif") should be (List(SubQuery(List(
    ...
    it("should find tiff images when searching for file type 'tiff'") {
      Parser.run("fileType:tiff") should be (List(SubQuery(List(
        Match(SingleField(getFieldPath("mimeType")), Words("image/tiff")),
        Negation(Match(IsField,IsValue("deleted")))
      ))))
    }
    ...
    it("should match multiple terms and the fileType query") {
      Parser.run("fileType:tiff cats dogs") should be (List(SubQuery(List(
        Match(SingleField(getFieldPath("mimeType")), Words("image/tiff")),
        Match(AnyField, Words("cats dogs")),
        Negation(Match(IsField,IsValue("deleted")))
      ))))
    ...
    it("should match aliases and a fileType query") {
      Parser.run("by:cats fileType:tiff") should be (List(SubQuery(List(
        Match(bylineField, Words("cats")),
        Match(SingleField(getFieldPath("mimeType")), Words("image/tiff")),
        Negation(Match(IsField,IsValue("deleted")))
      ))))
    ...
    it("should not match unrelated file types") {
      Parser.run("fileType:catsdogs") should be (List(SubQuery(List(
  ...
  describe("quoted field search") {
    it("should match a quoted field search") {
      Parser.run(""""fieldDogs":cats""") should be (List(SubQuery(List(
    ...
    it("should match a quoted field search with  colons") {
      Parser.run(""""fieldDogs:dinosaur:lemur":cats""") should be (List(SubQuery(List(
    ...
    it("should match a quoted field search colons, and a search term with quotes and colons") {
      Parser.run(""""fieldDogs":"cats:are:fun"""") should be (List(SubQuery(List(
