...
class ImageResponseTest extends FunSpec with Matchers {
  ...
  it(
    "not cause a stack overflow when many consecutive newline characters are present"
  ) {
    val text = "\n\r\n\n\n\r\r\r\n" * 10000
    val normalisedText = ImageResponse.normaliseNewlineChars(text)
  ...
  it(
    "should indicate if image can be deleted" +
    ...
    val testCrop = Crop(
      ...
      None,
      None,
      CropSpec("test-uri", Bounds(0, 0, 0, 0), None),
      None,
      Nil
    )
    val testUsage = Usage(
    ...
    val imgWithExportsAndUsages =
      img.copy(exports = List(testCrop)).copy(usages = List(testUsage))
