...
class SuggestionController(
    ...
    with ArgoHelpers
    with ImageFields
    with AggregateResponses {

  def suggestMetadataCredit(q: Option[String], size: Option[Int]) =
    suggestion("suggestMetadataCredit", q, size)

  def suggestPhotoshoot(q: Option[String], size: Option[Int]) =
    suggestion(photoshootField("suggest"), q, size)
  ...
  def metadataSearch(field: String, q: Option[String]) = auth.async { request =>
    ...
    elasticSearch.metadataSearch(
      AggregateSearchParams(field, request)
  ...
  private def suggestion(
    ...
    query
      .flatMap(q => if (q.nonEmpty) Some(q) else None)
      .map { q =>
