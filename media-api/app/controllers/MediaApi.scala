...
class MediaApi(
  ...
  private val indexResponse = {
    ...
    val indexLinks = List(
      ...
      Link(
        "metadata-search",
        s"${config.rootUri}/suggest/metadata/{field}{?q}"
      ),
      ...
      Link(
        "witness-report",
        s"${config.services.guardianWitnessBaseUri}/2/report/{id}"
      ),
  ...
  private def ImageCannotBeDeleted = respondError(
    MethodNotAllowed,
    "cannot-delete",
    "Cannot delete persisted images"
  )
  private def ImageDeleteForbidden = respondError(
    Forbidden,
    "delete-not-allowed",
    "No permission to delete this image"
  )
  private def ImageEditForbidden = respondError(
    Forbidden,
    "edit-not-allowed",
    "No permission to edit this image"
  )
  private def ImageNotFound(id: String) = respondError(
    NotFound,
    "image-not-found",
    s"No image found with the given id $id"
  )
  private def ExportNotFound = respondError(
    NotFound,
    "export-not-found",
    "No export found with the given id"
  )

  def index = auth { indexResponse }

  def getIncludedFromParams(
  ...
  def canUserDeleteCropsOrUsages(user: Principal): Boolean =
    hasPermission(user, Permissions.DeleteCrops)

  private def isAvailableForSyndication(image: Image): Boolean =
    image.syndicationRights.exists(_.isAvailableForSyndication)

  private def hasPermission(
      request: Authentication.Request[Any],
      image: Image
  ): Boolean = request.user.accessor.tier match {
  ...
  def getImageExport(imageId: String, exportId: String) = auth.async {
    request =>
      implicit val r = request
      ...
      elasticSearch.getImageById(imageId) map {
        case Some(source) if hasPermission(request, source) =>
          ...
          exportOption.foldLeft(ExportNotFound)((memo, export) =>
            respond(export)
          )
      ...
      case Some(image) if hasPermission(request, image) =>
        ...
        if (imageCanBeDeleted) {
          ...
          if (canDelete) {
            ...
            val updateMessage =
              UpdateMessage(subject = deleteImage, id = Some(id))
  ...
  def downloadOriginalImage(id: String) = auth.async { request =>
    ...
    elasticSearch.getImageById(id) flatMap {
      case Some(image) if hasPermission(request, image) => {
        ...
        logger.info(
          s"Download original image: $id from user: ${Authentication
            .getIdentity(request.user)}",
          apiKey,
          id
        )
        mediaApiMetrics.incrementImageDownload(
          apiKey,
          mediaApiMetrics.OriginalDownloadType
        )
        ...
        val entity = HttpEntity.Streamed(
          file,
          image.source.size,
          image.source.mimeType.map(_.name)
        )

        if (config.recordDownloadAsUsage) {
          postToUsages(
            config.usageUri + "/usages/download",
            auth.getOnBehalfOfPrincipal(request.user),
            id,
            Authentication.getIdentity(request.user)
          )
        }

        Future.successful(
          Result(ResponseHeader(OK), entity).withHeaders(
            "Content-Disposition" -> s3Client.getContentDisposition(
              image,
              Source
            )
          )
  ...
  def downloadOptimisedImage(
    ...
    elasticSearch.getImageById(id) flatMap {
      case Some(image) if hasPermission(request, image) => {
        val apiKey = request.user.accessor
        logger.info(
          s"Download optimised image: $id from user: ${Authentication
            .getIdentity(request.user)}",
          apiKey,
          id
        )
        mediaApiMetrics.incrementImageDownload(
          apiKey,
          mediaApiMetrics.OptimisedDownloadType
        )
        ...
        val sourceImageUri =
          new URI(
            s3Client.signUrl(
              config.imageBucket,
              image.optimisedPng.getOrElse(image.source).file,
              image,
              imageType = image.optimisedPng match {
        ...
        Future.successful(
          Redirect(
            config.imgopsUri + List(
              sourceImageUri.getPath,
              sourceImageUri.getRawQuery
            ).mkString("?") + s"&w=$width&h=$height&q=$quality"
          )
        )
      }
      case _ => Future.successful(ImageNotFound(id))
  ...
  def postToUsages(
    ...
    val baseRequest = ws
      .url(uri)
      .withHttpHeaders(
        Authentication.originalServiceHeaderName -> config.appName,
  ...
  def imageSearch() = auth.async { request =>
    ...
    def hitToImageEntity(
        elasticId: String,
        image: Image
    ): EmbeddedEntity[JsValue] = {
      val writePermission = canUserWriteMetadata(request, image)
      val deletePermission = canUserDeleteImage(request, image)
      val deleteCropsOrUsagePermission =
        canUserDeleteCropsOrUsages(request.user)
        ...
        errors =>
          Future.successful(
            respondError(
              ...
              errors.map(_.message).list.reduce(_ + ", " + _),
              List(searchLink)
            )
          ),
  ...
  private def getImageResponseFromES(
      id: String,
      request: Authentication.Request[AnyContent]
  ): Future[Option[(Image, JsValue, List[Link], List[Action])]] = {
  ...
  private def getSearchUrl(
    ...
    val paramMap: Map[String, String] =
      SearchParams.toStringMap(searchParams) ++ Map(
    ...
    paramMap
      .foldLeft(UriTemplate()) { (acc, pair) =>
        acc.expandAny(pair._1, pair._2)
      }
      .toString
  }
  ...
  private def getPrevLink(searchParams: SearchParams): Option[Link] = {
    ...
    if (searchParams.offset > 0) {
      ...
      val prevLength =
        List(searchParams.length, searchParams.offset - prevOffset).min
