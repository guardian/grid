...
class UsageController(
  ...
  def bySupplier = auth.async { request =>
    ...
    Future
      .sequence(
        Agencies.all.keys.map(
          elasticSearch.usageForSupplier(_, numberOfDayInPeriod)
        )
      )
      .map(_.toList)
