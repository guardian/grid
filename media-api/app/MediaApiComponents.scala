...
class MediaApiComponents(context: Context) extends GridComponents(context) with GridCORSAuthentication {
  ...
  val imageResponse = new ImageResponse(services, imageBucket, thumbBucket, cloudFrontDomainThumbBucket, persistenceIdentifier, cloudFrontS3Client, usageQuota)

  val mediaApi = new MediaApi(auth, services, imageBucket, messageSender, elasticSearch, imageResponse, controllerComponents, cloudFrontS3Client, mediaApiMetrics)
