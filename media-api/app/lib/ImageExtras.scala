...
object ImageExtras {
  ...
  private def isCurrent(lease: MediaLease): Boolean =
    lease.active && lease.isUse

  def hasCurrentAllowLease(leases: LeasesByMedia): Boolean =
    leases.leases.exists(lease =>
      lease.access == AllowUseLease && isCurrent(lease)
    )

  def hasCurrentDenyLease(leases: LeasesByMedia): Boolean =
    leases.leases.exists(lease =>
      lease.access == DenyUseLease && isCurrent(lease)
    )

  def validityMap(image: Image, withWritePermission: Boolean)(implicit
      cost: CostCalculator,
      quotas: UsageQuota
  ): ValidMap = {

    val shouldOverride =
      validityOverrides(image, withWritePermission).exists(_._2 == true)
    ...
    Map(
      ...
      "missing_credit" -> createCheck(
        !hasCredit(image.metadata),
        overrideable = false
      ),
      "missing_description" -> createCheck(
        !hasDescription(image.metadata),
        overrideable = false
      ),
  ...
  def isValid(validityMap: ValidMap): Boolean =
