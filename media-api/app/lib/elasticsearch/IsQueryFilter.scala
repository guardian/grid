...
object IsQueryFilter {
  ...
  def apply(value: String, overQuotaAgencies: () => List[Agency], config: MediaApiConfig): Option[IsQueryFilter] = {
    ...
    value.toLowerCase match {
      ...
      case "agency-pick" => Some(IsAgencyPick(config.agencyPickQueries.values.toList))
      case _ => None
    }
  }
}
...
case class IsAgencyPick(queries: List[Query]) extends IsQueryFilter {
  override def query: Query = filters.or(queries: _*)
}
