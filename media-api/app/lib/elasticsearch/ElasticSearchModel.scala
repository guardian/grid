...
case class CompletionSuggestionResults(
    results: List[CompletionSuggestionResult]
)
...
object SearchParams {
  ...
  def listToCommas(list: List[String]): Option[String] =
    list.toNel.map(_.list.mkString(","))

  // TODO: return descriptive 400 error if invalid
  def parseIntFromQuery(s: String): Option[Int] = Try(s.toInt).toOption
  def parsePayTypeFromQuery(s: String): Option[PayType.Value] =
    PayType.create(s)
  def parseBooleanFromQuery(s: String): Option[Boolean] = Try(
    s.toBoolean
  ).toOption
  def parseSyndicationStatus(s: String): Option[SyndicationStatus] = Some(
    SyndicationStatus(s)
  )

  def apply(request: Authentication.Request[Any]): SearchParams = {

    def commaSep(key: String): List[String] =
      request.getQueryString(key).toList.flatMap(commasToList)
  ...
  def validate(searchParams: SearchParams): SearchParamValidations = {
    ...
    (validateLength(searchParams).toValidationNel |@| validateOffset(
      searchParams
    ).toValidationNel)((s1, s2) => s1)
  }

  def validateOffset(searchParams: SearchParams): SearchParamValidation = {
    if (searchParams.offset < 0)
      InvalidUriParams("offset cannot be less than 0").failure
    else searchParams.success
  }

  def validateLength(searchParams: SearchParams): SearchParamValidation = {
    if (searchParams.length > maxSize)
      InvalidUriParams(s"length cannot exceed $maxSize").failure
    else searchParams.success
  }
