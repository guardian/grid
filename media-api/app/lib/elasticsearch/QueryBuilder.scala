...
import com.gu.mediaservice.model.{Agency, GnmOwned, PrAndThirdParty}
...
class QueryBuilder(matchFields: Seq[String], overQuotaAgencies: () => List[Agency], config: MediaApiConfig) extends ImageFields with GridLogging {
  ...
  private val legacyCategoryExpansions: Map[String, NonEmptyList[String]] = Map(
    ...
    GnmOwned.category -> GnmOwned.allCategories
  )
  ...
  private def makeQueryBit(condition: Match): Query = condition.field match {
    ...
    case SingleField(field) => condition.value match {
      case Words(value) if field == usageRightsField("category") && legacyCategoryExpansions.contains(value) =>
