...
class QueryBuilder(
  ...
  private def multiMatchPhraseQuery(
      value: String,
      fields: Seq[String]
  ): MultiMatchQuery =
    ElasticDsl
  ...
  private def makeMultiQuery(
      value: Value,
      fields: Seq[String]
  ): MultiMatchQuery = value match {
    case Words(value) =>
      ElasticDsl
        .multiMatchQuery(value)
        .fields(fields)
  ...
  private def makeQueryBit(condition: Match): Query = condition.field match {
    ...
    case HierarchyField =>
      condition.value match {
        ...
        case _ =>
          throw InvalidQuery(
            "Cannot accept non-Phrase value for HierarchyField Match"
          )
      }
    case HasField =>
      condition.value match {
        case HasValue(value) =>
          boolQuery().filter(existsQuery(getFieldPath(value)))
        case _ =>
          throw InvalidQuery(s"Cannot perform has field on ${condition.value}")
      }
    case IsField =>
      condition.value match {
        case IsValue(value) =>
          IsQueryFilter.apply(value, overQuotaAgencies) match {
        ...
        .map {
          case (parent: SingleField, n: List[Nested]) => {
            ...
            val nested = n.foldLeft(boolQuery) {
              case (query, Nested(_, f, v)) =>
                query.withMust(makeQueryBit(Match(f, v)))
              case (query, _) => query
            }
          ...
          case _ =>
            throw InvalidQuery(
              "Can only accept SingleField for Nested Query parent"
            )
