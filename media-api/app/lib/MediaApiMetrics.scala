...
class MediaApiMetrics(config: MediaApiConfig)
    extends CloudWatchMetrics(s"${config.stage}/MediaApi", config) {
  ...
  def incrementImageDownload(
    ...
    val dimension = new Dimension()
      .withName(downloadType.metricName)
