...
trait DateParser {
  ...
  def parseDate(expr: String): Option[DateTime] = Try(
    DateTime.parse(expr, formatter)
  ).toOption
}

case class DateAliasParser(alias: String, start: DateTime, end: DateTime)
    extends DateParser {
  val format = ""
...
case class DateFormatParser(
    format: String,
    calculateEnd: Option[(DateTime) => DateTime] = None
) extends DateParser {
