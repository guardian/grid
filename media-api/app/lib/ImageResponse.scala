...
class ImageResponse(
  ...
  private val imgPersistenceReasons = ImagePersistenceReasons.apply(
    config.persistedRootCollections,
    config.persistenceIdentifier
  )

  def imagePersistenceReasons(image: Image): List[String] =
    imgPersistenceReasons.getImagePersistenceReasons(image)
  ...
  def create(
    ...
    val source = Try {
      ...
      logger.error(
        s"Failed to read ElasticSearch response $id into Image object: ${e.getMessage}"
      )
      Failure(e)
    }.get
    ...
    val imageUrl =
      s3Client.signUrl(config.imageBucket, fileUri, image, imageType = Source)
    val pngUrl: Option[String] = pngFileUri
      .map(
        s3Client.signUrl(config.imageBucket, _, image, imageType = OptimisedPng)
      )

    def s3SignedThumbUrl = s3Client.signUrl(
      config.thumbBucket,
      fileUri,
      image,
      imageType = Thumbnail
    ...
    val data = source
      .transform(addSecureSourceUrl(imageUrl))
    ...
    val links: List[Link] = tier match {
      case Internal =>
        imageLinks(id, imageUrl, pngUrl, withWritePermission, valid)
    ...
    val actions: List[Action] =
      if (tier == Internal)
        imageActions(
          ...
          isDeletable,
          withWritePermission,
          withDeleteCropsOrUsagePermission
        )
      else Nil
  ...
  def downloadLink(id: String) =
    Link("download", s"${config.rootUri}/images/$id/download")
  def downloadOptimisedLink(id: String) = Link(
    "downloadOptimised",
    s"${config.rootUri}/images/$id/downloadOptimised?{&width,height,quality}"
  )

  def imageLinks(
      id: String,
      secureUrl: String,
      securePngUrl: Option[String],
      withWritePermission: Boolean,
      valid: Boolean
  ) = {
    ...
    val optimisedPngLink = securePngUrl match {
      case Some(secureUrl) =>
        Some(Link("optimisedPng", makeImgopsUri(new URI(secureUrl))))
      case _ => None
    }
    ...
    val fileMetadataLink =
      Link("fileMetadata", s"${config.rootUri}/images/$id/fileMetadata")
  ...
  def addUsageCost(source: JsValue): Reads[JsObject] = {
    ...
    __.json.update(
      __.read[JsObject].map(_ ++ Json.obj("cost" -> cost.toString))
    )
  }

  def addSyndicationStatus(image: Image): Reads[JsObject] = {
    __.json
      .update(__.read[JsObject])
      .map(
        _ ++ Json.obj(
  ...
  def addPersistedState(
      isPersisted: Boolean,
      persistenceReasons: List[String]
  ): Reads[JsObject] =
    __.json
      .update(__.read[JsObject])
      .map(
        _ ++ Json.obj(
          "persisted" -> Json
            .obj("value" -> isPersisted, "reasons" -> persistenceReasons)
        )
      )
  ...
  def addSecureSourceUrl(url: String): Reads[JsObject] =
    (__ \ "source").json.update(
      __.read[JsObject].map(_ ++ Json.obj("secureUrl" -> url))
    )

  def addSecureOptimisedPngUrl(url: String): Reads[JsObject] =
    (__ \ "optimisedPng").json.update(
      __.read[JsObject].map(_ ++ Json.obj("secureUrl" -> url))
    )

  def addSecureThumbUrl(url: String): Reads[JsObject] =
    (__ \ "thumbnail").json.update(
      __.read[JsObject].map(_ ++ Json.obj("secureUrl" -> url))
    )
  ...
  def addInvalidReasons(reasons: Map[String, String]): Reads[JsObject] =
    __.json
      .update(__.read[JsObject])
      .map(_ ++ Json.obj("invalidReasons" -> Json.toJson(reasons)))

  def makeImgopsUri(uri: URI): String =
    config.imgopsUri + List(uri.getPath, uri.getRawQuery).mkString(
      "?"
    ) + "{&w,h,q}"
  ...
  def imageResponseWrites(
      id: String,
      expandFileMetaData: Boolean
  ): Writes[Image] = (
    (__ \ "id").write[String] ~
      ...
      (__ \ "fileMetadata")
        .write[FileMetadataEntity]
        .contramap(
          fileMetadataEntity(id, expandFileMetaData, _: FileMetadata)
        ) ~
      (__ \ "userMetadata").writeNullable[Edits] ~
      (__ \ "metadata").write[ImageMetadata](
        ImageResponse.newlineNormalisingImageMetadataWriter
      ) ~
      ...
      (__ \ "leases")
        .write[MediaLeasesEntity]
        .contramap(leasesEntity(id, _: LeasesByMedia)) ~
      (__ \ "collections")
        .write[List[EmbeddedEntity[CollectionResponse]]]
        .contramap((collections: List[Collection]) =>
          collections.map(c => collectionsEntity(id, c))
        ) ~
      (__ \ "syndicationRights").writeNullable[SyndicationRights] ~
      (__ \ "usermetaDataLastModified").writeNullable[DateTime]
  )(unlift(Image.unapply))

  def fileMetaDataUri(id: String) =
    URI.create(s"${config.rootUri}/images/$id/fileMetadata")

  def usagesUri(id: String) = URI.create(s"${config.usageUri}/usages/media/$id")

  def usageUri(id: String) = {
    URI.create(
      s"${config.usageUri}/usages/${UriEncoding.encodePathSegment(id, "UTF-8")}"
    )
  }

  def leasesUri(id: String) =
    URI.create(s"${config.leasesUri}/leases/media/$id")

  def usageEntity(usage: Usage) =
    EmbeddedEntity[Usage](usageUri(usage.id), Some(usage))

  def usagesEntity(id: String, usages: List[Usage]) =
    EmbeddedEntity[List[UsageEntity]](
      usagesUri(id),
      Some(usages.map(usageEntity))
    )

  def leasesEntity(id: String, leaseByMedia: LeasesByMedia) =
    EmbeddedEntity[LeasesByMedia](leasesUri(id), Some(leaseByMedia))

  def collectionsEntity(
      id: String,
      c: Collection
  ): EmbeddedEntity[CollectionResponse] =
  ...
  def collectionEntity(rootUri: String, imageId: String, c: Collection) = {
    ...
    val uri = URI.create(
      s"$rootUri/images/$imageId/${CollectionsManager.pathToUri(c.path)}"
    )
  ...
  def fileMetadataEntity(
      id: String,
      expandFileMetaData: Boolean,
      fileMetadata: FileMetadata
  ) = {
    val displayableMetadata =
      if (expandFileMetaData) Some(fileMetadata) else None
...
object ImageResponse {
  ...
  val newlineNormalisingImageMetadataWriter: Writes[ImageMetadata] =
    (input: ImageMetadata) => {
      Json.toJson(normaliseNewLinesInImageMeta(input))
    }

  def normaliseNewLinesInImageMeta(
  ...
  def normaliseNewlineChars(string: String): String =
...
case class CollectionResponse private (
    path: List[String],
    pathId: String,
    description: String,
    cssColour: Option[String],
...
object CollectionResponse {
  ...
  def build(c: Collection) =
    CollectionResponse(
      ...
      c.pathId,
      c.description,
      CollectionsManager.getCssColour(c.path),
      c.actionData
    )
}
