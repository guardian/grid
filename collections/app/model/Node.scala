...
object Node {
  def fromList[T](
      list: List[T],
      getPath: T => List[String],
      getCorrectPathBit: T => String
  ): Node[T] = {
    ...
    def loop(
      ...
      ts
        ...
        .map { case (currentSlug, tsWithThisSlug) =>
          ...
          val thisCorrectPath = thisLevel.headOption
            .map(getCorrectPathBit)
            .map(c => correctPath :+ c)
            .getOrElse(Nil)
          ...
          Node(
            currentSlug,
            loop(children, thisFullPath, thisCorrectPath),
            thisFullPath,
            thisCorrectPath,
            thisLevel.headOption
          )
        }
  ...
  implicit def nodeFormat[T: Format]: Format[Node[T]] = (
    (__ \ "basename").format[String] ~
      (__ \ "children").lazyFormat(
        Reads.list(nodeFormat[T]),
        Writes.list(nodeFormat[T])
      ) ~
