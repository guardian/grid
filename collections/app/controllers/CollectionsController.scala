...
class CollectionsController(
  ...
  val appIndex =
    AppIndex("media-collections", "The one stop shop for collections")
  val indexLinks = List(Link("collections", collectionUri.toString))

  def addChildAction(pathId: List[String] = Nil): Option[ArgoAction] = Some(
    ArgoAction("add-child", collectionUri(pathId), "POST")
  )
  def addChildAction(n: Node[Collection]): Option[ArgoAction] = addChildAction(
    n.fullPath
  )
  def removeNodeAction(n: Node[Collection]): Option[ArgoAction] = if (
    n.children.nonEmpty
  ) None
  else
    Some(
  ...
  def collectionNotFound(path: String) =
    respondError(
      ...
      s"Could not find collection: $path"
    )
  ...
  def invalidJson(json: JsValue) =
    respondError(
      BadRequest,
      "invalid-json",
      s"Could not parse json: ${json.toString}"
    )
  ...
  def correctedCollections = authenticated.async { req =>
    store.getAll flatMap { collections =>
      ...
      val correctTree = tree hackmap { node =>
        val correctedCollection =
          node.data.map(c => c.copy(path = node.correctPath))
        Node(
          node.basename,
          node.children,
          node.fullPath,
  ...
  def getCollections = authenticated.async { req =>
    ...
    } recover { case e: CollectionsStoreError =>
  ...
  def addChildToCollection(collectionPathId: String) = addChildTo(
    Some(collectionPathId)
  )
  def addChildTo(collectionPathId: Option[String]) =
    authenticated.async(parse.json) { req =>
      (req.body \ "data").asOpt[String] map { child =>
        if (isValidPathBit(child)) {
          ...
          val collection = Collection.build(
            path,
            ActionData(getIdentity(req.user), DateTime.now)
          )
          ...
          store.add(collection).map { collection =>
            ...
            logger.info(
              req.user.accessor,
              s"Adding collection ${path.mkString("/")}"
            )
          ...
          Future.successful(
            respondError(
              BadRequest,
              "invalid-input",
              "You cannot have slashes or double quotes in your path name"
            )
          )
        }
  ...
  def removeCollection(collectionPath: String) = authenticated.async { req =>
    ...
    hasChildren(path).flatMap { noRemove =>
      if (noRemove) {
        ...
        logger.info(
          req.user.accessor,
          s"Deleting collection ${path.mkString("/")}"
        )
  ...
  implicit def collectionEntityWrites
      : Writes[Node[EmbeddedEntity[Collection]]] = (
    (__ \ "name").write[String] ~
      (__ \ "children").lazyWrite(
        Writes.seq[Node[EmbeddedEntity[Collection]]](collectionEntityWrites)
      ) ~
  ...
  implicit def asArgo: Writes[Node[Collection]] = (
    (__ \ "basename").write[String] ~
      (__ \ "children")
        .lazyWrite[CollectionsEntity](
          Writes[CollectionsEntity]
          // This is so we don't have to rewrite the Write[Seq[T]]
          (seq => Json.toJson(seq))
        )
        .contramap(collectionsEntity) ~
      (__ \ "fullPath").write[List[String]] ~
