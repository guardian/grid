...
class ImageCollectionsController(
  ...
  def getCollections(id: String) = authenticated.async { req =>
    dynamo.listGet[Collection](id, "collections").map { collections =>
      ...
      respondNotFound("No collections found")
    }
  }
  ...
  def addCollection(id: String) = authenticated.async(parse.json) { req =>
    (req.body \ "data").asOpt[List[String]].map { path =>
      val collection = Collection
        .build(path, ActionData(getIdentity(req.user), DateTime.now()))
      dynamo
        .listAdd(id, "collections", collection)
        .map(publish(id))
  ...
  def removeCollection(id: String, collectionString: String) =
    authenticated.async { req =>
      val path =
        CollectionsManager.uriToPath(UriOps.encodePlus(collectionString))
      ...
      dynamo.listGet[Collection](id, "collections") flatMap { collections =>
        CollectionsManager.findIndexes(path, collections) match {
          case Nil =>
            Future.successful(
              respondNotFound(s"Collection $collectionString not found")
            )
          case indexes =>
            dynamo
              .listRemoveIndexes[Collection](id, "collections", indexes)
              .map(publish(id))
  ...
  def publish(id: String)(collections: List[Collection]): List[Collection] = {
    ...
    val updateMessage = UpdateMessage(
      subject = "set-image-collections",
      id = Some(id),
      collections = Some(onlyLatestCollections)
    )
