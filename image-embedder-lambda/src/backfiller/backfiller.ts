...
import {checkQueueSize, sendSqsMessages} from "./sqs";
...
export const handler = async (
  ...
  const queueSize = await checkQueueSize(BACKFILL_SQS_QUEUE!);
  ...
  if (sqsMessages.length > 0) {
    await sendSqsMessages(sqsMessages, BACKFILL_SQS_QUEUE!);
