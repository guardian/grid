...
export interface ImageResolver {
	fetchImage(message: SQSMessageBody): Promise<FetchedImage>;
}
...
export class S3ImageResolver implements ImageResolver {
	...
	async fetchImage(message: SQSMessageBody): Promise<FetchedImage> {
...
export class CachedImageResolver implements ImageResolver {
	...
	async fetchCachedDownscaledImage(
		...
		if (!this.downScaledImageBucket) {
			console.log(
				`No downscaled image bucket configured, skipping cache lookup`,
			);
			return undefined;
		}
	...
	async fetchImage(message: SQSMessageBody): Promise<FetchedImage> {
		...
		const cachedBytes = await this.fetchCachedDownscaledImage(
			message.imageId,
		...
		if (cachedBytes) {
			...
			if (revalidated !== cachedBytes) {
				await this.cacheDownscaledImage(
					message.imageId,
					revalidated,
					processedMimeType,
				);
