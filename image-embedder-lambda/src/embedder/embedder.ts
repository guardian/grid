...
export const computeEmbeddingForSQSEvent = async (
	...
	const s3VectorStore = new S3VectorStore(
		clients.s3VectorsClient,
		`image-embeddings-${environment.stage}`.toLowerCase(),
	);
	...
	if (vectors.length > 0) {
		const shortenedVectors =
			thrallEventPublisher.matryoshkaEmbeddingToElasticsearchDimensions(
				vectors,
			);
...
export const handler = async (
	...
	if (!process.env.DOWNSCALED_IMAGE_BUCKET) {
		console.error(
			'DOWNSCALED_IMAGE_BUCKET environment variable is not set. Caching of downscaled images will be disabled.',
		);
	}
