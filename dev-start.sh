...
setupLocalKinesis() {
  ...
  aws --profile media-service --region=eu-west-1 --endpoint-url=http://localhost:4568 kinesis create-stream --shard-count 1 --stream-name "${stream_name}" | cat
  aws --profile media-service --region=eu-west-1 --endpoint-url=http://localhost:4568 kinesis list-streams | cat
}
