...
import lib.{EnableMyCrops, EnableAISearch, ExampleSwitch, FeatureSwitches, KahunaConfig, UseCqlChips}
...
class KahunaController(
  ...
  def index(ignored: String) = withOptionalLoginRedirect { request =>
    ...
    val featureSwitches = new FeatureSwitches(
      List(ExampleSwitch, UseCqlChips, EnableAISearch, EnableMyCrops)
    )
