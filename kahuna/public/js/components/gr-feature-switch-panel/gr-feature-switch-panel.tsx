...
export const getFeatureSwitchActive = (key: string): boolean => {
  // Server-side kill switch for AI search: takes priority over the sticky per-browser cookie below,
  // so it can be turned off for every user immediately via config, however a user's browser is set up.
  if (key === "enable-ai-search" && !window._clientConfig.aiSearchEnabled) {
    return false;
  }
