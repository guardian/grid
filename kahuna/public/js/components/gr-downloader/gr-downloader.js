...
downloader.controller('DownloaderCtrl', [
  ...
  function Controller($window, $q, $scope, inject$, imageDownloadsService) {
    ...
    ctrl.downloadableImagesArray = () => restrictDownload ? ctrl.imagesArray().filter(({data}) => data.isDownloadable && data.softDeletedMetadata === undefined) : ctrl.imagesArray();
    ...
    $scope.$watch('ctrl.images', function () {
      ...
      if (restrictDownload) {
        ...
        ctrl.singleImageSelected = singleImageSelected && ctrl.imagesArray()[0].data.isDownloadable && ctrl.imagesArray()[0].data.softDeletedMetadata === undefined;
