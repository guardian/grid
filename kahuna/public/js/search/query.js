...
query.controller("SearchQueryCtrl", [
  ...
  function (
    ...
    ctrl.costFilterFalseValue = ctrl.costFilterChargeable
      ? undefined
      : "'true'";
    ...
    ctrl.initialShowPaidEvent =
      $stateParams.nonFree === undefined && ctrl.usePermissionsFilter
        ? false
        : true;

    ctrl.shouldDisplayAISearchOption =
      getFeatureSwitchActive("enable-ai-search");
    if (!ctrl.shouldDisplayAISearchOption) {
      ctrl.useAISearch = false;
    } else {
      ctrl.useAISearch =
        $stateParams.useAISearch === "true" || $stateParams.useAISearch === true
          ? true
          : false;
    }
    ...
    function manageUploadedBy(filter, sender) {
      ...
      if (!ctrl.usePermissionsFilter) {
        ...
        const shouldOverwriteUploadedBy =
          !filter.uploadedBy ||
          filter.uploadedBy === ctrl.user.email ||
          myUploadsCheckbox;
        if (shouldOverwriteUploadedBy) {
          ctrl.filter.uploadedBy = filter.uploadedByMe
            ? ctrl.user.email
            : undefined;
        }
      } else {
        if (sender === "selectMyUploads") {
          ...
          if (shouldOverwriteUploadedBy) {
            ctrl.filter.uploadedBy =
              ctrl.user && ctrl.filterMyUploads ? ctrl.user.email : undefined;
    ...
    function manageOrgOwnedSetting(filter) {
      ...
      const orgOwnedIndexInQuery = structuredQuery.findIndex(
      ...
      if (ctrl.filter.orgOwned && !queryHasOrgOwned) {
        ...
        !ctrl.filter.orgOwned &&
        queryHasOrgOwned &&
        !ctrl.usePermissionsFilter
      ) {
    ...
    function getCollection() {
      const collection = storage.getJs("currentCollection")
        ? storage.getJs("currentCollection")
        : "";
      return collection;
    }

    function getPriorOrderBy() {
      const prior = storage.getJs("priorOrderBy")
        ? storage.getJs("priorOrderBy")
        : "";
      return prior;
    }
    ...
    function priorRevisedOrderBy(
      ...
      if (
        collectionSearch &&
        (oldCollection !== newCollection || "" !== priorOrderBy)
      ) {
        if (priorOrderBy != "") {
          setPriorOrderBy("");
          return priorOrderBy;
    ...
    function watchSearchChange(newFilter, sender) {
      ...
      ctrl.collectionSearch = newFilter.query
        ? checkForCollection(newFilter.query)
        : false;
      ...
      if (ctrl.usePermissionsFilter) {
        ...
        if (
          $stateParams.orderBy &&
          $stateParams.orderBy.includes(TAKEN_SORT) &&
          (!newFilter.query || !newFilter.query.includes(HAS_DATE_TAKEN))
        ) {
          ctrl.ordering["orderBy"] = revisedOrderBy(ctrl.collectionSearch);
        } else {
          const prior = priorRevisedOrderBy(
            ctrl.collectionSearch,
            newCollection,
            oldCollection,
          );
      ...
      let sortBy = ctrl.ordering["orderBy"]
      ...
      if (ctrl.usePermissionsFilter && nonFreeCheck === undefined) {
        ...
        !ctrl.usePermissionsFilter &&
        (nonFreeCheck === "false" || nonFreeCheck === false)
      ) {
    ...
    function setAndWatchParam(key) {
      ...
      ctrl.collectionSearch = ctrl.filter.query
        ? checkForCollection(ctrl.filter.query)
        : false;
    ...
    mediaApi.getSession().then((session) => {
      ...
      //-default non free-
      const defNonFree = session.user.permissions
        ? session.user.permissions.showPaid
        : undefined;
      storage.setJs("defaultIsNonFree", defNonFree ? defNonFree : false, true);
      if (
        !ctrl.initialShowPaidEvent &&
        (defNonFree === true || defNonFree === "true")
      ) {
      ...
      const orgOwned = structuredQuery.some(
        (item) => item.value === ctrl.maybeOrgOwnedValue,
      );
