...
class CropStore(config: CropperConfig) extends S3ImageStorage(config) {
  ...
  def getSecureCropUri(uri: URI): Option[URL] =
    config.imgPublishingSecureHost.map(
      new URI("https", _, uri.getPath, uri.getFragment).toURL
    )
  ...
  )(implicit requestContext: RequestLoggingContext): Future[Asset] = {
  ...
  def listCrops(id: String): Future[List[Crop]] = {
    list(config.imgPublishingBucket, id).map { crops =>
      crops
        .foldLeft(Map[String, Crop]()) {
          case (map, (s3Object)) => {
            val filename :: containingFolder :: _ =
              s3Object.uri.getPath.split("/").reverse.toList
            ...
            val updatedCrop = for {
              ...
              x <- getOrElseOrNone(userMetadata, "bounds-x", "bounds_x").map(
                _.toInt
              )
              y <- getOrElseOrNone(userMetadata, "bounds-y", "bounds_y").map(
                _.toInt
              )
              w <- getOrElseOrNone(userMetadata, "bounds-width", "bounds_w")
                .map(_.toInt)
              h <- getOrElseOrNone(userMetadata, "bounds-height", "bounds_h")
                .map(_.toInt)
              width <- userMetadata.get("width").map(_.toInt)
              height <- userMetadata.get("height").map(_.toInt)

              cid = s"$id-$x-$y-$w-$h"
              ratio = getOrElseOrNone(
                userMetadata,
                "aspect-ratio",
                "aspect_ratio"
              )
              ...
              exportType = userMetadata
                .get("type")
                .map(ExportType.valueOf)
                .getOrElse(ExportType.default)
              cropSource = CropSpec(
                source,
                Bounds(x, y, w, h),
                ratio,
              ...
              lastCrop = map.getOrElse(
                cid,
                Crop.createFromCropSource(author, date, cropSource)
              )
              lastSizings = lastCrop.assets

              currentSizings =
                if (isMaster) lastSizings else lastSizings :+ sizing
              masterSizing = if (isMaster) Some(sizing) else lastCrop.master
            } yield cid -> Crop.createFromCropSource(
              author,
              date,
              cropSource,
              masterSizing,
