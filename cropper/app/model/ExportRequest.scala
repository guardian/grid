...
case class CropRequest(uri: String, bounds: Bounds, aspectRatio: Option[String])
...
object ExportRequest {
  ...
  implicit val readExportRequest: Reads[ExportRequest] =
    Reads[ExportRequest](jsValue =>
  ...
  def boundsFill(dimensions: Dimensions): Bounds =
    Bounds(0, 0, dimensions.width, dimensions.height)

  def toCropSpec(cropRequest: ExportRequest, dimensions: Dimensions): CropSpec =
    cropRequest match {
      case FullExportRequest(uri) =>
        CropSpec(
          ...
          AspectRatio
            .calculate(dimensions.width, dimensions.height)
            .map(_.friendly),
          FullExport
        )
      // Map "crop" that covers the whole image to a "full" export
      case CropRequest(uri, bounds, ratio)
          if bounds == boundsFill(dimensions) =>
        CropSpec(uri, boundsFill(dimensions), ratio, FullExport)
      case CropRequest(uri, bounds, ratio) =>
        CropSpec(uri, bounds, ratio, CropExport)
    }
