...
class AuthController(
  ...
  def session = auth { request =>
    ...
    request.user match {
      ...
      case MachinePrincipal(accessor, _) =>
        respond(
          Json.obj(
            "api-key" ->
              Json.obj(
  ...
  def isValidDomain(inputUri: String): Boolean = {
    ...
    if (!success)
      logger.warn(s"Provided login redirect URI is invalid: $inputUri")
    success
  }
  ...
  def doLogin(redirectUri: Option[String] = None) = Action.async {
    implicit req =>
      ...
      providers.userProvider.sendForAuthentication match {
        case Some(authCallback) =>
          authCallback(req).map(
            _.addingToSession(
              checkedRedirectUri.map(REDIRECT_SESSION_KEY -> _).toSeq: _*
            )
          )
        case None =>
          Future.successful(
            InternalServerError(
              "Login not supported by configured authentication provider"
            )
          )
      }
  }
  ...
  def oauthCallback = Action.async { implicit request =>
    providers.userProvider.sendForAuthenticationCallback match {
      case Some(callback) =>
        ...
        callback(request, maybeRedirectUri).map(
          _.removingFromSession(REDIRECT_SESSION_KEY)
        )
      case None =>
        Future.successful(
          InternalServerError(
            "No callback for configured authentication provider"
          )
        )
    }
  }
  ...
  def logout = Action { implicit request =>
    val result: Result = providers.userProvider.flushToken match {
      ...
      case None =>
        InternalServerError(
          "Logout not supported by configured authentication provider"
        )
    }
