...
class UsageTable(config: UsageConfig)
    extends DynamoDB(config, config.usageRecordTable)
    with GridLogging {
  ...
  def queryByUsageId(id: String): Future[Option[MediaUsage]] = Future {
    UsageTableFullKey
      .build(id)
      .flatMap((tableFullKey: UsageTableFullKey) => {
        val keyAttribute: KeyAttribute =
          new KeyAttribute(hashKeyName, tableFullKey.hashKey)
        val rangeKeyCondition: RangeKeyCondition =
          new RangeKeyCondition(rangeKeyName).eq(tableFullKey.rangeKey)
  ...
  def queryByImageId(id: String): Future[Set[MediaUsage]] = Future {
    ...
    val fullSet =
      queryResult.asScala.map(ItemToMediaUsage.transform).toSet[MediaUsage]

    hidePendingIfPublished(hidePendingIfRemoved(fullSet))
  }

  def hidePendingIfRemoved(usages: Set[MediaUsage]): Set[MediaUsage] =
    usages.filterNot((mediaUsage: MediaUsage) => {
  ...
  def hidePendingIfPublished(usages: Set[MediaUsage]): Set[MediaUsage] = usages
    .groupBy(_.grouping)
    .flatMap { case (grouping, groupedUsages) =>
  ...
  def matchUsageGroup(usageGroup: UsageGroup): Observable[UsageGroup] = {
    ...
    Observable.from(Future {
      ...
      logger.info(
        s"Built matched UsageGroup ${usageGroup.grouping} (${usages.size})"
      )
  ...
  def deleteRecord(mediaUsage: MediaUsage) = {
    ...
    logger.info(
      s"deleting usage ${mediaUsage.usageId} for media id ${mediaUsage.mediaId}"
    )
  ...
  def updateFromRecord(record: UsageRecord): Observable[JsObject] = Observable
    .from(Future {
