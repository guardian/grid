...
class UsageApi(
  ...
  def deleteUsages(mediaId: String) = AuthenticatedAndAuthorisedToDelete.async { req =>
    ...
    var photoSalesUsage = false
    var filteredUsages = ListBuffer[MediaUsage]()

    usageTable.queryByImageId(mediaId)
      .map(usages => {
        usages.foreach(usage => {
          if (usage.syndicationUsageMetadata.exists(_.partnerName == "Capture")) {
            photoSalesUsage = true
          } else {
            filteredUsages.+=(usage)
            usageTable.deleteRecord(usage)
          }
        })
        if (!photoSalesUsage) {
