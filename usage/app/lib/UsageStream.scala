...
class UsageStream(usageGroup: UsageGroupOps) {
  ...
  val previewContentStream: Observable[ContentContainer] =
    PreviewCrierContentStream.observable
  val liveContentStream: Observable[ContentContainer] =
    LiveCrierContentStream.observable

  val previewObservable: Observable[UsageGroup] = getObservable(
  ...
  private def getObservable(contentStream: Observable[ContentContainer]) = {
    contentStream.flatMap((container: ContentContainer) => {
      val usageGroupOption: Option[UsageGroup] = usageGroup.build(
        container.content,
