 ...
 * not "in" the map.
 */
final class SimpleConfig implements com.gu.typesafe.config.Config, MergeableValue, Serializable {
    ...
    @Override
    public com.gu.typesafe.config.ConfigOrigin origin() {
        return object.origin();
    }

    @Override
    public SimpleConfig resolve() {
        return resolve(com.gu.typesafe.config.ConfigResolveOptions.defaults());
    }

    @Override
    public SimpleConfig resolve(com.gu.typesafe.config.ConfigResolveOptions options) {
        return resolveWith(this, options);
    }

    @Override
    public SimpleConfig resolveWith(com.gu.typesafe.config.Config source) {
        return resolveWith(source, com.gu.typesafe.config.ConfigResolveOptions.defaults());
    }

    @Override
    public SimpleConfig resolveWith(com.gu.typesafe.config.Config source, ConfigResolveOptions options) {
        com.gu.typesafe.config.impl.AbstractConfigValue resolved = ResolveContext.resolve(object, ((SimpleConfig) source).object, options);
    ...
    private com.gu.typesafe.config.ConfigValue hasPathPeek(String pathExpression) {
        Path path = Path.newPath(pathExpression);
        com.gu.typesafe.config.ConfigValue peeked;
        try {
    ...
    public boolean hasPath(String pathExpression) {
        com.gu.typesafe.config.ConfigValue peeked = hasPathPeek(pathExpression);
        return peeked != null && peeked.valueType() != com.gu.typesafe.config.ConfigValueType.NULL;
    }

    @Override
    public boolean hasPathOrNull(String path) {
        com.gu.typesafe.config.ConfigValue peeked = hasPathPeek(path);
        return peeked != null;
    }
    ...
    private static void findPaths(Set<Map.Entry<String, com.gu.typesafe.config.ConfigValue>> entries, Path parent,
                                  AbstractConfigObject obj) {
        for (Map.Entry<String, com.gu.typesafe.config.ConfigValue> entry : obj.entrySet()) {
            String elem = entry.getKey();
            com.gu.typesafe.config.ConfigValue v = entry.getValue();
            Path path = Path.newKey(elem);
            if (parent != null)
                path = path.prepend(parent);
            if (v instanceof AbstractConfigObject) {
                findPaths(entries, path, (AbstractConfigObject) v);
            } else if (v instanceof com.gu.typesafe.config.impl.ConfigNull) {
                // nothing; nulls are conceptually not in a Config
            } else {
                entries.add(new AbstractMap.SimpleImmutableEntry<String, com.gu.typesafe.config.ConfigValue>(path.render(), v));
            }
        }
    }
    ...
    public Set<Map.Entry<String, com.gu.typesafe.config.ConfigValue>> entrySet() {
        Set<Map.Entry<String, com.gu.typesafe.config.ConfigValue>> entries = new HashSet<Map.Entry<String, com.gu.typesafe.config.ConfigValue>>();
        findPaths(entries, null, object);
        return entries;
    }

    static private com.gu.typesafe.config.impl.AbstractConfigValue throwIfNull(com.gu.typesafe.config.impl.AbstractConfigValue v, com.gu.typesafe.config.ConfigValueType expected, Path originalPath) {
        if (v.valueType() == com.gu.typesafe.config.ConfigValueType.NULL)
            throw new com.gu.typesafe.config.ConfigException.Null(v.origin(), originalPath.render(),
                    expected != null ? expected.name() : null);
        else
            return v;
    }

    static private com.gu.typesafe.config.impl.AbstractConfigValue findKey(AbstractConfigObject self, String key,
                                                                           com.gu.typesafe.config.ConfigValueType expected, Path originalPath) {
        return throwIfNull(findKeyOrNull(self, key, expected, originalPath), expected, originalPath);
    }

    static private com.gu.typesafe.config.impl.AbstractConfigValue findKeyOrNull(AbstractConfigObject self, String key,
                                                                                 com.gu.typesafe.config.ConfigValueType expected, Path originalPath) {
        com.gu.typesafe.config.impl.AbstractConfigValue v = self.peekAssumingResolved(key, originalPath);
        if (v == null)
            throw new com.gu.typesafe.config.ConfigException.Missing(self.origin(), originalPath.render());

        if (expected != null)
            v = DefaultTransformer.transform(v, expected);

        if (expected != null && (v.valueType() != expected && v.valueType() != com.gu.typesafe.config.ConfigValueType.NULL))
            throw new com.gu.typesafe.config.ConfigException.WrongType(v.origin(), originalPath.render(), expected.name(),
                    v.valueType().name());
        else
            return v;
    }

    static private com.gu.typesafe.config.impl.AbstractConfigValue findOrNull(AbstractConfigObject self, Path path,
                                                                              com.gu.typesafe.config.ConfigValueType expected, Path originalPath) {
        try {
            ...
            if (next == null) {
                ...
                AbstractConfigObject o = (AbstractConfigObject) findKey(self, key,
                        com.gu.typesafe.config.ConfigValueType.OBJECT,
    ...
    com.gu.typesafe.config.impl.AbstractConfigValue find(Path pathExpression, com.gu.typesafe.config.ConfigValueType expected, Path originalPath) {
        return throwIfNull(findOrNull(object, pathExpression, expected, originalPath), expected, originalPath);
    }

    com.gu.typesafe.config.impl.AbstractConfigValue find(String pathExpression, com.gu.typesafe.config.ConfigValueType expected) {
        Path path = Path.newPath(pathExpression);
        return find(path, expected, path);
    }

    private com.gu.typesafe.config.impl.AbstractConfigValue findOrNull(Path pathExpression, com.gu.typesafe.config.ConfigValueType expected, Path originalPath) {
        return findOrNull(object, pathExpression, expected, originalPath);
    }

    private com.gu.typesafe.config.impl.AbstractConfigValue findOrNull(String pathExpression, com.gu.typesafe.config.ConfigValueType expected) {
        Path path = Path.newPath(pathExpression);
        return findOrNull(path, expected, path);
    }

    @Override
    public com.gu.typesafe.config.impl.AbstractConfigValue getValue(String path) {
        return find(path, null);
    }

    @Override
    public boolean getIsNull(String path) {
        com.gu.typesafe.config.impl.AbstractConfigValue v = findOrNull(path, null);
        return (v.valueType() == com.gu.typesafe.config.ConfigValueType.NULL);
    }

    @Override
    public boolean getBoolean(String path) {
        com.gu.typesafe.config.ConfigValue v = find(path, com.gu.typesafe.config.ConfigValueType.BOOLEAN);
        return (Boolean) v.unwrapped();
    }

    private com.gu.typesafe.config.impl.ConfigNumber getConfigNumber(String path) {
        com.gu.typesafe.config.ConfigValue v = find(path, com.gu.typesafe.config.ConfigValueType.NUMBER);
        return (com.gu.typesafe.config.impl.ConfigNumber) v;
    }
    ...
    public int getInt(String path) {
        com.gu.typesafe.config.impl.ConfigNumber n = getConfigNumber(path);
    ...
    public String getString(String path) {
        com.gu.typesafe.config.ConfigValue v = find(path, com.gu.typesafe.config.ConfigValueType.STRING);
    ...
    @Override
    public com.gu.typesafe.config.ConfigList getList(String path) {
        com.gu.typesafe.config.impl.AbstractConfigValue v = find(path, com.gu.typesafe.config.ConfigValueType.LIST);
        return (ConfigList) v;
    }

    @Override
    public AbstractConfigObject getObject(String path) {
        AbstractConfigObject obj = (AbstractConfigObject) find(path, com.gu.typesafe.config.ConfigValueType.OBJECT);
        return obj;
    }
    ...
    public Object getAnyRef(String path) {
        com.gu.typesafe.config.ConfigValue v = find(path, null);
        return v.unwrapped();
    }
    ...
    private BigInteger getBytesBigInteger(String path) {
        ...
        } catch (com.gu.typesafe.config.ConfigException.WrongType e) {
            bytes = parseBytes((String) v.unwrapped(),
                v.origin(), path);
        }
        if (bytes.signum() < 0)
            throw new com.gu.typesafe.config.ConfigException.BadValue(v.origin(), path,
    ...
    private List<BigInteger> getBytesListBigInteger(String path){
        ...
        for (com.gu.typesafe.config.ConfigValue v : list) {
            BigInteger bytes;
            if (v.valueType() == com.gu.typesafe.config.ConfigValueType.NUMBER) {
                bytes = BigInteger.valueOf(((Number) v.unwrapped()).longValue());
            } else if (v.valueType() == com.gu.typesafe.config.ConfigValueType.STRING) {
    ...
    @Override
    public com.gu.typesafe.config.ConfigMemorySize getMemorySize(String path) {
        return com.gu.typesafe.config.ConfigMemorySize.ofBytes(getBytesBigInteger(path));
    }
    ...
    public TemporalAmount getTemporal(String path){
        ...
        } catch (com.gu.typesafe.config.ConfigException.BadValue e){
    ...
    private <T> List<T> getHomogeneousUnwrappedList(String path,
            com.gu.typesafe.config.ConfigValueType expected) {
    ...
    public List<Boolean> getBooleanList(String path) {
        return getHomogeneousUnwrappedList(path, com.gu.typesafe.config.ConfigValueType.BOOLEAN);
    }

    @Override
    public List<Number> getNumberList(String path) {
        return getHomogeneousUnwrappedList(path, com.gu.typesafe.config.ConfigValueType.NUMBER);
    }

    @Override
    public List<Integer> getIntList(String path) {
        List<Integer> l = new ArrayList<Integer>();
        List<com.gu.typesafe.config.impl.AbstractConfigValue> numbers = getHomogeneousWrappedList(path, com.gu.typesafe.config.ConfigValueType.NUMBER);
        for (com.gu.typesafe.config.impl.AbstractConfigValue v : numbers) {
            l.add(((com.gu.typesafe.config.impl.ConfigNumber) v).intValueRangeChecked(path));
        }
        return l;
    }
    ...
    public List<String> getStringList(String path) {
        return getHomogeneousUnwrappedList(path, com.gu.typesafe.config.ConfigValueType.STRING);
    }

    @Override
    public <T extends Enum<T>> List<T> getEnumList(Class<T> enumClass, String path) {
        List<ConfigString> enumNames = getHomogeneousWrappedList(path, com.gu.typesafe.config.ConfigValueType.STRING);
    ...
    private <T extends Enum<T>> T getEnumValue(String path, Class<T> enumClass, com.gu.typesafe.config.ConfigValue enumConfigValue) {
        ...
        try {
            ...
            throw new com.gu.typesafe.config.ConfigException.BadValue(
    ...
    @SuppressWarnings("unchecked")
    private <T extends com.gu.typesafe.config.ConfigValue> List<T> getHomogeneousWrappedList(
            String path, com.gu.typesafe.config.ConfigValueType expected) {
        List<T> l = new ArrayList<T>();
        List<? extends com.gu.typesafe.config.ConfigValue> list = getList(path);
        for (com.gu.typesafe.config.ConfigValue cv : list) {
            // variance would be nice, but stupid cast will do
    ...
    @Override
    public List<com.gu.typesafe.config.ConfigObject> getObjectList(String path) {
        return getHomogeneousWrappedList(path, com.gu.typesafe.config.ConfigValueType.OBJECT);
    }

    @Override
    public List<? extends com.gu.typesafe.config.Config> getConfigList(String path) {
        List<com.gu.typesafe.config.ConfigObject> objects = getObjectList(path);
        List<com.gu.typesafe.config.Config> l = new ArrayList<com.gu.typesafe.config.Config>();
    ...
    public List<Long> getBytesList(String path) {
        com.gu.typesafe.config.ConfigValue v = find(path, com.gu.typesafe.config.ConfigValueType.LIST);
        return getBytesListBigInteger(path).stream()
            .map(bytes -> toLong(bytes, v.origin(), path))
            .collect(Collectors.toList());
    }

    private Long toLong(BigInteger value, com.gu.typesafe.config.ConfigOrigin originForException,
        String pathForException){
        if (value.bitLength() < 64) {
            return value.longValue();
        } else {
            throw new com.gu.typesafe.config.ConfigException.BadValue(originForException, pathForException,
                "size-in-bytes value is out of range for a 64-bit long: '" + value + "'");
        }
    }

    @Override
    public List<com.gu.typesafe.config.ConfigMemorySize> getMemorySizeList(String path) {
        return getBytesListBigInteger(path).stream()
     ...
     * @throws com.gu.typesafe.config.ConfigException
     ...
     *             if string is invalid
     */
    public static BigInteger parseBytes(String input, com.gu.typesafe.config.ConfigOrigin originForException,
        String pathForException) {
        String s = ConfigImplUtil.unicodeTrim(input);
    ...
    private com.gu.typesafe.config.impl.AbstractConfigValue peekPath(Path path) {
        return root().peekPath(path);
    }

    private static void addProblem(List<com.gu.typesafe.config.ConfigException.ValidationProblem> accumulator, Path path,
                                   com.gu.typesafe.config.ConfigOrigin origin, String problem) {
        accumulator.add(new com.gu.typesafe.config.ConfigException.ValidationProblem(path.render(), origin, problem));
    }

    private static String getDesc(com.gu.typesafe.config.ConfigValueType type) {
        return type.name().toLowerCase();
    }

    private static String getDesc(com.gu.typesafe.config.ConfigValue refValue) {
    ...
    private static void addMissing(List<com.gu.typesafe.config.ConfigException.ValidationProblem> accumulator,
                                   String refDesc, Path path, com.gu.typesafe.config.ConfigOrigin origin) {
        addProblem(accumulator, path, origin, "No setting at '" + path.render() + "', expecting: "
                   + refDesc);
    }

    private static void addMissing(List<com.gu.typesafe.config.ConfigException.ValidationProblem> accumulator,
                                   com.gu.typesafe.config.ConfigValue refValue, Path path, com.gu.typesafe.config.ConfigOrigin origin) {
        addMissing(accumulator, getDesc(refValue), path, origin);
    }

    // JavaBean stuff uses this
    static void addMissing(List<com.gu.typesafe.config.ConfigException.ValidationProblem> accumulator,
                           com.gu.typesafe.config.ConfigValueType refType, Path path, com.gu.typesafe.config.ConfigOrigin origin) {
        addMissing(accumulator, getDesc(refType), path, origin);
    }

    private static void addWrongType(List<com.gu.typesafe.config.ConfigException.ValidationProblem> accumulator,
                                     String refDesc, com.gu.typesafe.config.impl.AbstractConfigValue actual, Path path) {
    ...
    private static void addWrongType(List<com.gu.typesafe.config.ConfigException.ValidationProblem> accumulator,
                                     com.gu.typesafe.config.ConfigValue refValue, com.gu.typesafe.config.impl.AbstractConfigValue actual, Path path) {
        addWrongType(accumulator, getDesc(refValue), actual, path);
    }

    private static void addWrongType(List<com.gu.typesafe.config.ConfigException.ValidationProblem> accumulator,
                                     com.gu.typesafe.config.ConfigValueType refType, com.gu.typesafe.config.impl.AbstractConfigValue actual, Path path) {
        addWrongType(accumulator, getDesc(refType), actual, path);
    }

    private static boolean couldBeNull(com.gu.typesafe.config.impl.AbstractConfigValue v) {
        return DefaultTransformer.transform(v, com.gu.typesafe.config.ConfigValueType.NULL)
                .valueType() == com.gu.typesafe.config.ConfigValueType.NULL;
    }

    private static boolean haveCompatibleTypes(com.gu.typesafe.config.ConfigValue reference, com.gu.typesafe.config.impl.AbstractConfigValue value) {
        if (couldBeNull((com.gu.typesafe.config.impl.AbstractConfigValue) reference)) {
    ...
    private static boolean haveCompatibleTypes(com.gu.typesafe.config.ConfigValueType referenceType, com.gu.typesafe.config.impl.AbstractConfigValue value) {
        if (referenceType == com.gu.typesafe.config.ConfigValueType.NULL || couldBeNull(value)) {
            // we allow any setting to be null
            return true;
        } else if (referenceType == com.gu.typesafe.config.ConfigValueType.OBJECT) {
            if (value instanceof AbstractConfigObject) {
                return true;
            } else {
                return false;
            }
        } else if (referenceType == com.gu.typesafe.config.ConfigValueType.LIST) {
        ...
        } else if (referenceType == com.gu.typesafe.config.ConfigValueType.STRING) {
    ...
    private static void checkValidObject(Path path, AbstractConfigObject reference,
            ...
            List<com.gu.typesafe.config.ConfigException.ValidationProblem> accumulator) {
        for (Map.Entry<String, com.gu.typesafe.config.ConfigValue> entry : reference.entrySet()) {
    ...
    private static void checkListCompatibility(Path path, SimpleConfigList listRef,
            SimpleConfigList listValue, List<com.gu.typesafe.config.ConfigException.ValidationProblem> accumulator) {
        if (listRef.isEmpty() || listValue.isEmpty()) {
            // can't verify type, leave alone
        } else {
            com.gu.typesafe.config.impl.AbstractConfigValue refElement = listRef.get(0);
            for (com.gu.typesafe.config.ConfigValue elem : listValue) {
                com.gu.typesafe.config.impl.AbstractConfigValue e = (com.gu.typesafe.config.impl.AbstractConfigValue) elem;
    ...
    static void checkValid(Path path, com.gu.typesafe.config.ConfigValueType referenceType, com.gu.typesafe.config.impl.AbstractConfigValue value,
                           List<com.gu.typesafe.config.ConfigException.ValidationProblem> accumulator) {
        if (haveCompatibleTypes(referenceType, value)) {
            if (referenceType == com.gu.typesafe.config.ConfigValueType.LIST && value instanceof SimpleConfigObject) {
                // attempt conversion of indexed object to list
                com.gu.typesafe.config.impl.AbstractConfigValue listValue = DefaultTransformer.transform(value,
                        com.gu.typesafe.config.ConfigValueType.LIST);
    ...
    private static void checkValid(Path path, com.gu.typesafe.config.ConfigValue reference, com.gu.typesafe.config.impl.AbstractConfigValue value,
                                   List<com.gu.typesafe.config.ConfigException.ValidationProblem> accumulator) {
    ...
    @Override
    public void checkValid(com.gu.typesafe.config.Config reference, String... restrictToPaths) {
        ...
        if (restrictToPaths.length == 0) {
            ...
            for (String p : restrictToPaths) {
                ...
                com.gu.typesafe.config.impl.AbstractConfigValue refValue = ref.peekPath(path);
                if (refValue != null) {
                    com.gu.typesafe.config.impl.AbstractConfigValue child = peekPath(path);
                    if (child != null) {
