...
final class Path {
    ...
    Path(String... elements) {
        if (elements.length == 0)
            throw new com.gu.typesafe.config.ConfigException.BugOrBroken("empty path");
    ...
    Path subPath(int firstIndex, int lastIndex) {
        if (lastIndex < firstIndex)
            throw new com.gu.typesafe.config.ConfigException.BugOrBroken("bad call to subPath");
    ...
    public boolean equals(Object other) {
        if (other instanceof Path) {
            ...
            return this.first.equals(that.first)
                    && com.gu.typesafe.config.impl.ConfigImplUtil.equalsHandlingNull(this.remainder,
                            that.remainder);
        } else {
            return false;
        }
    ...
    static Path newPath(String path) {
        return com.gu.typesafe.config.impl.PathParser.parsePath(path);
    }
}
