 ...
 * {@link com.gu.typesafe.config.ConfigValue} instances. Values accessed through the
 ...
 * merging configs using {@link Config#withFallback(com.gu.typesafe.config.ConfigMergeable)}.
 *
 * <p>
 ...
 *
 * <p>
 * <code>Config</code> is a view onto a tree of {@link com.gu.typesafe.config.ConfigObject}; the
 ...
 *
 * <p>
 * Use {@link com.gu.typesafe.config.ConfigUtil#joinPath} and {@link com.gu.typesafe.config.ConfigUtil#splitPath} to convert
 ...
 * conceptually, {@code ConfigValue}s with a {@link com.gu.typesafe.config.ConfigValue#valueType()
 ...
 * {@link com.gu.typesafe.config.ConfigValue#valueType() valueType()} of {@link ConfigValueType#NULL
 * NULL}. Instead, they throw {@link com.gu.typesafe.config.ConfigException.Missing} if the value is
 * completely absent or set to null. If the value is set to null, a subtype of
 * {@code ConfigException.Missing} called {@link com.gu.typesafe.config.ConfigException.Null} will be
 * thrown. {@link com.gu.typesafe.config.ConfigException.WrongType} will be thrown anytime you ask for
 ...
 * to handle substitutions (though {@link com.gu.typesafe.config.ConfigFactory#load()} and similar
 ...
 * the associative operation {@link Config#withFallback(com.gu.typesafe.config.ConfigMergeable)}. If
 * you use {@link com.gu.typesafe.config.ConfigFactory#load()} (recommended), it merges system
 ...
 * {@link com.gu.typesafe.config.ConfigObject#render()} on the root object,
 * <code>myConfig.root().render()</code>. There's also a variant
 * {@link com.gu.typesafe.config.ConfigObject#render(com.gu.typesafe.config.ConfigRenderOptions)} which allows you to control
 ...
 *
 * <p>
 * As an alternative to {@link com.gu.typesafe.config.ConfigObject#render()}, the
 ...
 */
public interface Config extends com.gu.typesafe.config.ConfigMergeable {
    /**
     * Gets the {@code Config} as a tree of {@link com.gu.typesafe.config.ConfigObject}. This is a
     * constant-time operation (it is not proportional to the number of values
     * in the {@code Config}).
     *
     * @return the root object in the configuration
     */
    com.gu.typesafe.config.ConfigObject root();
     ...
     *
     * <p>
     * This method uses {@link com.gu.typesafe.config.ConfigResolveOptions#defaults()}, there is
     * another variant {@link Config#resolve(com.gu.typesafe.config.ConfigResolveOptions)} which lets
     ...
     *
     * <p>
     * Many methods on {@link com.gu.typesafe.config.ConfigFactory} such as
     ...
     * @throws com.gu.typesafe.config.ConfigException.UnresolvedSubstitution
     ...
     */
    Config resolve(com.gu.typesafe.config.ConfigResolveOptions options);
     ...
     * calling {@link Config#resolve(com.gu.typesafe.config.ConfigResolveOptions)} with
     ...
     * {@link com.gu.typesafe.config.ConfigResolveOptions#setAllowUnresolved(boolean)} so the partial
     * resolves don't fail).
     *
     * @param source
     ...
     * found. See {@link com.gu.typesafe.config.ConfigException.ValidationFailed#problems}. The
     ...
     * @throws com.gu.typesafe.config.ConfigException.ValidationFailed
     ...
     * @throws com.gu.typesafe.config.ConfigException.BugOrBroken
     ...
     * {@link com.gu.typesafe.config.ConfigObject}: it looks for a path expression, not a key; and it
     ...
     * (see {@link com.gu.typesafe.config.ConfigUtil#joinPath} and {@link com.gu.typesafe.config.ConfigUtil#splitPath}).
     *
     * @param path
     *            the path expression
     * @return true if a non-null value is present at the path
     * @throws com.gu.typesafe.config.ConfigException.BadPath
     ...
     * {@link com.gu.typesafe.config.ConfigUtil#splitPath}.
     * <p>
     * Because a <code>Config</code> is conceptually a single-level map from
     * paths to values, there will not be any {@link com.gu.typesafe.config.ConfigObject} values in the
     * entries (that is, all entries represent leaf nodes). Use
     * {@link com.gu.typesafe.config.ConfigObject} rather than <code>Config</code> if you want a tree.
     * (OK, this is a slight lie: <code>Config</code> entries may contain
     * {@link com.gu.typesafe.config.ConfigList} and the lists may contain objects. But no objects are
     * directly included as entry values.)
     *
     * @return set of paths with non-null values, built up by recursing the
     *         entire tree of {@link com.gu.typesafe.config.ConfigObject} and creating an entry for
     *         each leaf value.
     */
    Set<Map.Entry<String, com.gu.typesafe.config.ConfigValue>> entrySet();
     ...
     * @throws com.gu.typesafe.config.ConfigException.BadPath
     *             if the path expression is invalid
     * @throws com.gu.typesafe.config.ConfigException.Missing
     ...
     * @throws com.gu.typesafe.config.ConfigException.WrongType
     ...
     * @param path
     *            path expression
     * @return the {@link com.gu.typesafe.config.ConfigObject} value at the requested path
     * @throws com.gu.typesafe.config.ConfigException.Missing
     *             if value is absent or null
     * @throws com.gu.typesafe.config.ConfigException.WrongType
     *             if value is not convertible to an object
     */
    com.gu.typesafe.config.ConfigObject getObject(String path);
     ...
     * so on - see {@link com.gu.typesafe.config.ConfigValue#unwrapped()}).
     *
     * @param path
     *            path expression
     ...
     *             if value is absent or null
     */
    com.gu.typesafe.config.ConfigValue getValue(String path);
     ...
     * @throws com.gu.typesafe.config.ConfigException.BadValue
     ...
     *             if value cannot be parsed as a size in bytes
     */
    com.gu.typesafe.config.ConfigMemorySize getMemorySize(String path);
    ...
    /**
     * Gets a list value (with any element type) as a {@link com.gu.typesafe.config.ConfigList}, which
     * implements {@code java.util.List<ConfigValue>}. Throws if the path is
     * unset or null.
     *
     * @param path
     *            the path to the list value.
     * @return the {@link com.gu.typesafe.config.ConfigList} at the path
     * @throws com.gu.typesafe.config.ConfigException.Missing
     *             if value is absent or null
     * @throws com.gu.typesafe.config.ConfigException.WrongType
     ...
     * "unwrapped" (see {@link com.gu.typesafe.config.ConfigValue#unwrapped()}).
     *
     * @param path
     *            the path to the list value.
     * @return the list at the path
     ...
     * {@link com.gu.typesafe.config.ConfigUtil#joinPath} and {@link com.gu.typesafe.config.ConfigUtil#splitPath}).
     *
     * @param key
     ...
     * <p>
     * Note that path expressions have a syntax and sometimes require quoting
     * (see {@link com.gu.typesafe.config.ConfigUtil#joinPath} and {@link ConfigUtil#splitPath}).
     *
     * @param path
