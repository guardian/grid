...
final class ResolveSource {
    ...
    static private ResultWithPath findInObject(AbstractConfigObject obj, com.gu.typesafe.config.impl.ResolveContext context, Path path)
        ...
        com.gu.typesafe.config.impl.ResolveResult<? extends com.gu.typesafe.config.impl.AbstractConfigValue> partiallyResolved = context.restrict(path).resolve(obj,
                new ResolveSource(obj));
        com.gu.typesafe.config.impl.ResolveContext newContext = partiallyResolved.context.restrict(restriction);
        if (partiallyResolved.value instanceof AbstractConfigObject) {
            ...
            return new ResultWithPath(com.gu.typesafe.config.impl.ResolveResult.make(newContext, pair.value), pair.pathFromRoot);
        } else {
            throw new com.gu.typesafe.config.ConfigException.BugOrBroken("resolved object to non-object " + obj + " to " + partiallyResolved);
        }
    }
    ...
    static private ValueWithPath findInObject(AbstractConfigObject obj, Path path, Node<Container> parents) {
        ...
        com.gu.typesafe.config.impl.AbstractConfigValue v = obj.attemptPeekWithPartialResolve(key);
    ...
    ResultWithPath lookupSubst(com.gu.typesafe.config.impl.ResolveContext context, SubstitutionExpression subst,
                               int prefixLength)
            throws NotPossibleToResolve {
        if (ConfigImpl.traceSubstitutionsEnabled())
    ...
    ResolveSource pushParent(Container parent) {
        if (parent == null)
            throw new com.gu.typesafe.config.ConfigException.BugOrBroken("can't push null parent");
        ...
        if (pathFromRoot == null) {
            if (parent == root) {
                ...
                if (ConfigImpl.traceSubstitutionsEnabled()) {
                    ...
                    if (root.hasDescendant((com.gu.typesafe.config.impl.AbstractConfigValue) parent))
            ...
            if (ConfigImpl.traceSubstitutionsEnabled()) {
                // this hasDescendant check is super-expensive so it's a
                // trace message rather than an assertion
                if (parentParent != null && !parentParent.hasDescendant((com.gu.typesafe.config.impl.AbstractConfigValue) parent))
    ...
    private static Node<Container> replace(Node<Container> list, Container old, com.gu.typesafe.config.impl.AbstractConfigValue replacement) {
        Container child = list.head();
        if (child != old)
            throw new com.gu.typesafe.config.ConfigException.BugOrBroken("Can only replace() the top node we're resolving; had " + child
        ...
        if (replacement == null || !(replacement instanceof Container)) {
            if (parent == null) {
                 ...
                 */
                com.gu.typesafe.config.impl.AbstractConfigValue newParent = parent.replaceChild((com.gu.typesafe.config.impl.AbstractConfigValue) old, null);
            ...
            if (parent == null) {
                ...
                com.gu.typesafe.config.impl.AbstractConfigValue newParent = parent.replaceChild((com.gu.typesafe.config.impl.AbstractConfigValue) old, replacement);
    ...
    ResolveSource replaceCurrentParent(Container old, Container replacement) {
        ...
        if (old == replacement) {
            ...
            Node<Container> newPath = replace(pathFromRoot, old, (com.gu.typesafe.config.impl.AbstractConfigValue) replacement);
            if (ConfigImpl.traceSubstitutionsEnabled()) {
            ...
            if (old == root) {
                ...
                throw new com.gu.typesafe.config.ConfigException.BugOrBroken("attempt to replace root " + root + " with " + replacement);
                // return this;
            }
        }
    }
    ...
    ResolveSource replaceWithinCurrentParent(com.gu.typesafe.config.impl.AbstractConfigValue old, com.gu.typesafe.config.impl.AbstractConfigValue replacement) {
        if (ConfigImpl.traceSubstitutionsEnabled())
        ...
        if (old == replacement) {
            ...
            com.gu.typesafe.config.impl.AbstractConfigValue newParent = parent.replaceChild(old, replacement);
    ...
    static final class ValueWithPath {
        final com.gu.typesafe.config.impl.AbstractConfigValue value;
        final Node<Container> pathFromRoot;

        ValueWithPath(com.gu.typesafe.config.impl.AbstractConfigValue value, Node<Container> pathFromRoot) {
            this.value = value;
            this.pathFromRoot = pathFromRoot;
        }
    ...
    static final class ResultWithPath {
        final com.gu.typesafe.config.impl.ResolveResult<? extends com.gu.typesafe.config.impl.AbstractConfigValue> result;
        final Node<Container> pathFromRoot;

        ResultWithPath(com.gu.typesafe.config.impl.ResolveResult<? extends com.gu.typesafe.config.impl.AbstractConfigValue> result, Node<Container> pathFromRoot) {
            this.result = result;
            this.pathFromRoot = pathFromRoot;
        }
