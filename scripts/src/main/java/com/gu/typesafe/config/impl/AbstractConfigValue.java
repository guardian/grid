 ...
 *
 */
abstract class AbstractConfigValue implements com.gu.typesafe.config.ConfigValue, MergeableValue {

    final private SimpleConfigOrigin origin;

    AbstractConfigValue(com.gu.typesafe.config.ConfigOrigin origin) {
    ...
    protected static List<AbstractConfigValue> replaceChildInList(List<AbstractConfigValue> list,
        ...
        while (i < list.size() && list.get(i) != child)
            ++i;
        if (i == list.size())
            throw new com.gu.typesafe.config.ConfigException.BugOrBroken("tried to replace " + child + " which is not in " + list);
    ...
    protected abstract class NoExceptionsModifier implements Modifier {
        ...
        public final AbstractConfigValue modifyChildMayThrow(String keyOrNull, AbstractConfigValue v)
            ...
            try {
                ...
                throw new com.gu.typesafe.config.ConfigException.BugOrBroken("Unexpected exception", e);
            }
        }
    ...
    protected abstract AbstractConfigValue newCopy(com.gu.typesafe.config.ConfigOrigin origin);
    ...
    protected AbstractConfigValue constructDelayedMerge(com.gu.typesafe.config.ConfigOrigin origin,
    ...
    @Override
    public AbstractConfigValue withOrigin(com.gu.typesafe.config.ConfigOrigin origin) {
    ...
    public AbstractConfigValue withFallback(ConfigMergeable mergeable) {
        if (ignoresFallbacks()) {
            ...
            com.gu.typesafe.config.ConfigValue other = ((MergeableValue) mergeable).toFallbackValue();
    ...
    protected boolean canEqual(Object other) {
        return other instanceof com.gu.typesafe.config.ConfigValue;
    }

    @Override
    public boolean equals(Object other) {
        // note that "origin" is deliberately NOT part of equality
        if (other instanceof com.gu.typesafe.config.ConfigValue) {
            return canEqual(other)
                    && (this.valueType() ==
                            ((com.gu.typesafe.config.ConfigValue) other).valueType())
    ...
    public String toString() {
        ...
        render(sb, 0, true /* atRoot */, null /* atKey */, com.gu.typesafe.config.ConfigRenderOptions.concise());
        return getClass().getSimpleName() + "(" + sb.toString() + ")";
    }

    protected static void indent(StringBuilder sb, int indent, com.gu.typesafe.config.ConfigRenderOptions options) {
    ...
    protected void render(StringBuilder sb, int indent, boolean atRoot, String atKey, com.gu.typesafe.config.ConfigRenderOptions options) {
        if (atKey != null) {
            String renderedKey;
            if (options.getJson())
    ...
    protected void render(StringBuilder sb, int indent, boolean atRoot, com.gu.typesafe.config.ConfigRenderOptions options) {
        Object u = unwrapped();
    ...
    public final String render() {
        return render(com.gu.typesafe.config.ConfigRenderOptions.defaults());
    }
    ...
    SimpleConfig atKey(com.gu.typesafe.config.ConfigOrigin origin, String key) {
